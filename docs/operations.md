...
Cold archives may be compacted with `python3 scripts/compress_recovery.py --archive PATH.tar --expected-sha256 ORIGINAL_SHA --max-output-gib 16 --replace-verified-raw`. This is a bounded lossless representation change, not a new backup or service restart. It removes the raw representation only after the compressed file, exact decompressed-byte hash, safe member inventory and private manifest are durable. Preserve the compressed archive, both recorded hashes and historical recovery evidence. `restore-review-prepare` accepts the resulting `.tar.gz` directly and must receive its **compressed artifact SHA**, not the original raw SHA. See the recovery record for actual compacted paths and measured results.
...
## Earlier construction cargo release checkpoint
...
## Earlier native starter release checkpoint

Matched worlda962/client2f5c is now installed through the managed additive publication and immutable client activation path. The07:10 cold archive retained the complete standalone, including the ordinary-account native review fixture. Actual browser reconnect after the51.863-second managed restart preserved both carried and stored inventory, location and game-owned access; normal publication preserved all existing character/ship/item/container/appearance records. Exact artifacts and final public browser status are in [the native starter release record](handoffs/native_starter_release_20260910.md). Existing ships have not been silently converted; their explicit revisioned refit remains separate work.


## Current refit, airlock and loading release checkpoint

Matched world `2ade8d75` / client `f9e32399` is installed. The isolated source candidate excluded concurrent external renderer edits and passed check/build/art/Python, exact-module smoke, native refit/liquid UI and immutable loading failure/retry review. The11:33 private cold archive took32.161seconds of managed database interruption; review state was exact across30tables, and live character/item/container state was conserved. Only already-moving ship poses advanced between snapshots. Exact hashes, archive, browser evidence and remaining limitations are in [the release ledger](handoffs/refit_airlock_release_20260910.md). Final ordinary-provider public login/full rendered game acceptance passed. Existing ships are converted only through the explicit revisioned refit action; login does not perform a refit.

To stage an independently built client, use `public-client-stage --client-artifact ABSOLUTE_DIRECTORY --client-artifact-sha256 TREE_SHA256`; both arguments are required and permitted only for staging. The managed helper validates the exact complete tree, rejects symlinks/internal documents and never builds or stops the live service for this path. Use `public-client-activate --expected-live-client-sha256 CURRENT_SHA256 --expected-staged-client-sha256 REVIEWED_SHA256` for coordinated activation. Both digests are required together; mismatched live/staged metadata or current bytes reject before stopping. Managed stage/activation operations share a release lock. World publication remains separately additive through its managed path; verify its actual module bytes rather than the historical initial-program API field.


## Independent native asset delivery

Use `python3 scripts/dev.py public-client-delivery-stage|public-client-delivery-activate|public-client-delivery-rollback` as individual commands to manage the pinned GLB HTTP middleware independently of application and world artifacts. Staging performs no build. Activation verifies the exact existing client and previous delivery metadata, then restarts only the public frontend; a concurrent client release requires restaging. Future ordinary client activations keep the verified active delivery implementation and use the newly selected immutable client directory. Delivery rollback does not roll back the client or database.

The first installed delivery520de preserves client8246/world2ade and compresses native GLBs without changing decoded bytes or asset MIME. Actual public GETs, conditional requests and ranges passed; the two measured GLBs transfer86.05% fewer bytes. Exact hashes, protocol limits, proof commands and the independent recovery path are in [the delivery release record](handoffs/glb_delivery_release_20260910.md). No database restart or archive operation is required for this delivery-only switch.

## Start the existing database without publication

`python3 scripts/dev.py database-up` starts the configured existing database if it is stopped and waits for readiness. If it is already running it only checks readiness. This command never publishes the working world module, stops a running database, resets data or creates a backup. Use it for service recovery before a frontend-only release; `up`/`up-client` retain their existing development publication behavior when they start the database themselves. A command-dispatch regression is in `scripts/test_database_start_only.py`.
