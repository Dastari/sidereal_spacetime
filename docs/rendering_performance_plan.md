...
### R14. Planets (Phase 7, reference iteration reopened 2026-09-14)

The owner reopened planetary reference alignment on2026-09-14, requiring iterative actual-image comparison by the author and an independent Astra reviewer for every remaining family. Follow [the active reference iteration record](planet_reference_iteration_20260914.md) and [LOD authoring contract](planet_lod_authoring.md). This supersedes the planet-art pause for that work, while unrelated post-processing/quality experiments remain deferred. No new family visual acceptance is implied.

Historical deferred suggestions:
...
### R15. Planet LOD transition stalls (bounded owner authorization, 2026-09-11)

Status: layered path deployed with partial hardware evidence; strict transition acceptance not passed (2026-09-11). Native volcanic follow-up implements the same scheduling contract. This opens only transition scheduling/resource reuse; R14 visual changes remain deferred.

Owner requirement: move layered terrain generation into the voxel-worker pattern, build ahead before existing LOD thresholds, keep the previous visible node until the next is ready, swap within one frame, retain lower LODs per body, share/precompile materials, select resolution from the quad cap before generation, and expose last planet build milliseconds/pending builds in F3. Preserve existing geometry, materials, thresholds, atmosphere, weather and simulation semantics.

Acceptance: approach a temperate planet from seated Flight through full detail on hardware, F3 open, no transition frame above 20 ms and no frame without the planet. Save screenshots/JSON under `output/playwright/render-plan/R15/`; add NullEngine swap-order/retention coverage, run check/build and record measured versus outstanding results. This authorization does not reopen R9 or R11–R14 generally.
...
### 2026-09-11 — R0/R1 owner-priority live retry: steady Deck interval, then RAF stall

After the owner closed competing browser sessions, the public preview reopened as tab_d; tab_e no longer existed. RTX 4080 Laptop, WebGL, actual 1574 × 907, MSAA4, F3 open, on-foot Deck, all normal visuals enabled. Twenty one-second diagnostic samples are saved under output/playwright/render-plan/live-retry-20260911/, with a normally rendered canvas JPEG. The first fourteen samples advanced from frame16911 to18970 with 955 calls, 409 active / 1,372 total meshes, 528 materials, 147–165 displayed FPS and Render CPU5.76–6.23ms. This provides a useful short steady hardware interval. Radius42.52m and current population differ from the historical baseline, so it is not per-item timing attribution.

The engine then stopped at frame19023. Subsequent samples repeated stale F3 values despite document visibility remaining visible. A separate requestAnimationFrame probe also received zero callbacks; timers and evaluation still ran. The GL context was not lost, sampled GL errors were zero, one render loop remained registered, and no scene assets were pending; scene readiness reported false. Reopening the pane and clicking the canvas did not immediately resume the pending RAF. These observations narrow the stall to an interval without animation callbacks, but do not identify its root cause or prove a renderer defect. No game frames were manually submitted and no readiness state was bypassed. No new isolation or seated-Flight result is claimed from stale counters. The prior transient INVALID_OPERATION did not recur in these samples.

No source implementation changed. Reused the deployed candidate's passing check/build; documentation validation rerun. This retry improves Deck timing evidence but does not complete R1 close-up/hidden-deck acceptance or the plan.

### 2026-09-11 — R15 layered planet build-ahead and retained LODs, implemented, unmeasured

Reverified R7/R14 and the synchronous LOD replacement in environment/index.ts and layered-planet.ts. A changed LOD previously disposed its visible node before synchronous terrain generation, decoration/weather generation, GPU upload and new material compilation. The browser path now uses one module worker with request IDs; removed/revised bodies reject late results. Terrain, forest, crystals, initial clouds/smoke and prepared transferable vertex/index buffers are generated off-thread. Weather generation also uses the worker so a freshly activated LOD does not immediately regenerate clouds synchronously. No authority, simulation, content or authored asset input changed. Native ice/volcanic renderers and gas rendering are unchanged; this bounded implementation covers the layered terrain path used by temperate planets, not a general reopening of R14.

LOD1 prebuild starts at projected radius38px before its existing50px threshold; LOD0 prebuild starts at140px before190px. Original hysteresis is unchanged. Every completed level remains owned by its body, disabled when inactive. A ready-only cache commits enabling the new node and disabling the old node synchronously in update; asynchronous completion never hides the current planet. One shared scheduler admits one upload/compile continuation per browser frame across all bodies. Material instances are shared within each body and precompiled on actual replacement meshes before publication. Original per-body axial/weather transforms, palette conversion, winding, core sphere, atmosphere, effect geometry and PBR values are retained.

Resolution is selected before geometry allocation from exact recipe quad counts against the110,000 cap. A provably bounded small resolution skips counting; larger candidates use a count-only prepass without vertex/index buffers and preserve the historical admitted resolution sequence. This avoids generating/discarding over-cap meshes; a universal worst-case resolution clamp would change recipes that already fit and violate the no-visual-change requirement. Pelagic seed17 still uses resolutions16/32/96. Retained LODs intentionally increase allocated resources; the ship-loader budget bounds are unchanged because this item neither reduces ship resources nor establishes a new measured whole-scene bound.

F3 adds Last planet build (worker milliseconds for terrain/cap planning/decorations/buffer preparation, dash until completion) and Pending planet builds (queued/generating/staging/precompiling LODs plus weather worker jobs). Worker build duration is not main-thread frame time. Files: environment planet-build, planet-worker/client, planet-build-scheduler, planet-lod-cache/runtime, extracted pure planet-glint-samples, and their tests; adaptations in environment/index, layered-planet, planet-terrain, planet-sparkles, planet-smoke and planet-lava-spill; render diagnostics/index and CanvasUI diagnostics plus a new focused diagnostics test. Existing unrelated edits in shared index/diagnostics files are excluded from this item.

NullEngine tests cover old-node visibility while pending, update-only swap, lower-LOD retention/re-entry, stale completion after disposal, shared body materials/precompilation/ownership, staged disabled geometry, exact packed positions/colors/winding and cap counting. A scheduler test verifies one continuation per frame across bodies; a CanvasUI test verifies the new F3 rows. Workspace npm run check passes273files/1,547tests/77documents. The isolated live candidate's final check passes246files/1,395tests/77documents, then full npm run build passes. Existing bundle warnings remain.

Published client7f11e0430c3edbc34a322d8c453507e3553327834ba6b30febd8c82846a1bd8c forward from5a4c31c9 under the owner's standing publication authorization. Public entry index-CMhPFBTJ.js has SHA256907927f719a26b888d23000c85d9b21494cbde65eb444ca31ad8826e937185bd; worker planet-worker-CxaZcxgh.js has SHA25622fb19c9008a9dec3bcc03fc030c0291a0c32f4008e3d3d7f32aee73d16a579d. HTTPS root/entry/worker/ping return200 and artifact bytes match. Source/asset manifests match the prior live source (118world/src,121sim/src,58content/src,103net/src and19,740asset files); only the client was activated. No world publish or service/database recovery was needed. The isolated checkout is .runtime/release-checkouts/render-plan-r15-20260911. Disk pressure interrupted its initial copy; the copy was completed and validated after removing only rebuildable public/dist outputs from this agent's prior checkout. Immutable live releases and assets were preserved.

The deployed worker was exercised directly in the authenticated RTX browser for the real Pelagic temperate recipe: LOD2/1/0 completed in50.1/454.5/2,570.2ms of worker time, at16/32/96 resolution, with3,447/10,232/85,306 terrain quads, valid transferred Float32/Uint32 buffers and no worker errors. This proves deployed worker execution only. It does not measure rendered transitions.

Hardware acceptance remains unmeasured: after reloading the new client, the shared automation context remains at engine frame0 with zero independent requestAnimationFrame callbacks, even visible/focused after reopening/clicking. No planet node exists yet; the startup gate has zero pending assets and unready equipment meshes. Snapshot and resize automation failed. No frames/readiness/authority state were forced. The owner has been asked whether their visible pane shows the game or the startup cover. Existing before reference is the earlier aggregate live Deck964calls/1,372meshes/528materials and seated Flight400/1,447/532 at different camera/population; there are no valid R15 after counters, new isolation sequences, approach screenshots or proof of the20ms/no-missing-planet criteria. JSON worker/startup evidence and check/build/release logs are under output/playwright/render-plan/R15/. R15 is not marked done; R2/R4 identity conflicts and other deferred items remain unchanged.


### 2026-09-11 — R15 Pelagic measurement and Cinder/Observe follow-up

The owner reached Pelagic in seated Flight and reported that the visible freeze was gone, then reported the same stall on Cinder Volcanic and asked whether Map → Observe drives LOD. Observe already positions the render camera at the selected body before environment admission/LOD selection. Projected size uses that camera, independently of ship distance; no view semantics or authority changes are needed. A new NullEngine environment regression moves only the camera toward a distant planet, verifies LOD2 → LOD0, and verifies unchanged authoritative body coordinates and ship origin.

The RTX 4080 Laptop recorder captured Pelagic reaching LOD0 at frame56379, camera radius217.10, seated Flight, document visible: swap interval16.5ms, scene render14.9ms, 7 active planet meshes. The LOD0 worker request took783.3ms off-thread. Exactly one LOD remained enabled throughout the 2,080-frame retained measurement window. **The strict20ms criterion did not pass**: after the request, preparation intervals reached34.7ms (scene32.3ms), with other intervals21.9/26ms; a72.5ms interval occurred113ms before the request. This is partial hardware evidence, not completion. 127 early-window frames had no active planet mesh despite an enabled LOD, so enabled-state continuity alone does not prove uninterrupted on-screen visibility; camera/frustum visibility needs a synchronized capture. Evidence: `R15/pelagic-transition.json` and `pelagic-transition-summary.json`. The earlier `flight-reconnected.jpg/json` is a reconnection image with F3 closed and stale cached diagnostics, not acceptance evidence or a comparable baseline.

Cinder uses the separate native volcanic renderer, which the first layered-only R15 release left synchronous. The follow-up moves the unchanged authored composition, lava texture pixels, UVs, contact occlusion, radiance attributes and smoke geometry into the existing planet worker. Upload and compilation yield between meshes through the shared frame scheduler. A per-body ready-only cache retains old LODs and shares/precompiles basalt, smoke and molten materials plus the lava texture. Vent and flowing-lava material states remain separate; preparing a new LOD does not reset the currently animated emissive colour. Native ice and all visual redesign work remain deferred.

Files: environment/index.ts, native-volcanic-planet.ts, new native-volcanic-build.ts and native-volcanic-lod-runtime.ts, planet-worker.ts/client, runtime.test.ts, planet-lod-runtime.test.ts and new native-volcanic-lod-runtime.test.ts. The native NullEngine test checks visibility until compilation, retained return levels, distinct/shared lava materials, unchanged animation during prefetch, mesh roles and owned-resource disposal. Ship-only render-budget bounds remain unchanged because this changes planet scheduling, not ship resources.

The shared automation pane subsequently reported hidden and returned the startup cover after a reconnect; reopening did not restore an advancing measurement context. No readiness gate or authority state was forced. Cinder/Observe hardware timing, screenshots, settled Deck/Flight counters and both isolation sequences remain unmeasured. Pelagic's preparation-frame cost remains open; the owner's positive visual report is recorded separately from the failed strict timing criterion.


Validation/release: `VITEST_MAX_WORKERS=2 npm run check` passed274 files/1,549tests/77documents; `npm run build` passed. The isolated candidate passed247files/1,397tests/77documents and the full build. An initial candidate build lacked the copied allowlisted `docs/public/shipyard.md`; restoring that unchanged source file resolved the packaging failure. The final build passed. Source identity checks preserve118world,121sim,58content,103net and19,740asset files exactly relative to the previous live source. No world module was published. The isolated client was staged and activated through dev.py with expected-live/staged digest guards: previous7f11e0430c3e → **f1ffe83c99c27d3b77129cfa5ade892d7f1721761eefaac4184ed9084443bfb1**. Public index, entry `index-SWT9d1FU.js` and worker `planet-worker-DWaXMmKJ.js` return HTTP200 and match built bytes. Evidence: `R15/cinder-{check,build,isolated-check,isolated-build,stage,activate}.log`, source-isolation/public-verification/browser-status JSON. Cinder is **implemented and deployed, unmeasured**; R15 remains open for the strict hardware acceptance above.


### 2026-09-11 — R15 native ice and fixed-detail gas transitions, implemented/deployed, unmeasured

The owner authorized fixing the remaining transitions and required future authoring guidance. Added the [planet LOD authoring contract](planet_lod_authoring.md): account for Flight approach/retreat and abrupt Map → Observe changes, preserve authored geometry/PBR/identity, prepare buffers in the worker, build ahead, retain ready levels, share/precompile compatible materials, yield uploads, reject stale results, dispose owned resources, and record the20ms/no-missing-planet hardware gate. References were also appended to the existing local Blender migration and planet iteration guides; those pre-existing untracked documents are not swept into this commit.

Native ice now sends its unchanged authored composition and both optical attributes through the planet worker. The existing ready-only cache handles build-ahead, atomic enabled-state swaps and lower-level retention; materials are shared per body and compiled before publication. Gas spheres, rings and atmosphere do not vary with LOD, so their visual revision now excludes the threshold index while retaining real recipe-change invalidation. No authored assets, simulation/authority, F3 controls or view semantics changed. Source changes: environment/index.ts, planet-worker.ts/client, new native-ice-build.ts/native-ice-lod-runtime.ts; tests in new native-ice-lod-runtime.test.ts plus runtime.test.ts and the existing worker stub. NullEngine verifies exact ice geometry/optics at GPU precision, retained lower levels/shared materials/disposal, and gas mesh/material identity across an Observe camera jump while genuine recipe changes still rebuild. Ship render-budget bounds remain unchanged.

Workspace check passed275files/1,552tests/77documents and full build passed. Isolated release check passed248files/1,400tests/77documents and full build passed. Exact source comparison preserves118world,121sim,58content,103net and19,740asset files from the prior live candidate. No world module was published. Managed guarded publication advanced f1ffe83c99c2 → **c2cb7dad161399f307b169347313a9c5e01748e5109cbfaeec9361a87eb6d03e**. Public index, entry `index-yjny0qNn.js` and worker `planet-worker-DxSVlAi9.js` return HTTP200 and match the candidate bytes.

Browser verification remains incomplete. The old preview intermittently advanced, but snapshots failed. After reloading the new release and reopening, it returned the startup cover with engineframe0 and zero independent animation callbacks despite document visibility. Frost Ice was present in the destination list and requested via the same focusBody action that Map → Observe invokes; no resulting rendered transition can be asserted. No readiness bypass or authority changes were used. No gas destination was listed in the available navigation menu. Hardware screenshots, settled Deck/Flight counters, scale2 and Glow/Shadows/Lighting isolation sequences remain unmeasured. Pelagic's previously recorded34.7ms preparation interval still prevents strict R15 acceptance; these fixes do not claim to resolve that remaining measured cost. Evidence is under `output/playwright/render-plan/R15/ice-gas-*` (checks, builds, source isolation, release identity and browser probe). R15 remains open for hardware acceptance.

### 2026-09-14 — R14 native reference candidates, selective Ice26 glass and bounded validation

Implemented in isolated reference tooling; production integration and hardware transition acceptance remain open. Nine main-world candidates have root/Astra morphology working passes. Sixteen of19 moon crops have individual working passes; Crystal1/2 require native face-orientation repair and Volcanic2 requires a localized warm material finish. Exact revisions, failed iterations, reference-scale corrections and reviews are in [the active iteration record](planet_reference_iteration_20260914.md). These working passes are not owner final sign-off.

The owner's requested selective transparent Ice shards are native Ice26 PBR transmission/volume, preserving geometry and opaque caps. Two glass materials share one scene refraction resource. The worker initializes once per body; ready-node upload/disposal ownership is explicit, and proven fixed-detail revisions do not rebuild identical geometry at thresholds. NullEngine tests cover ready swap/retention, pending disposal, material and refraction-target ownership. The fresh software1574×907 Ice26 route records34 frames, zero missing/overlapping visible nodes and one retained level with shared glass target. SwiftShader timing is not hardware acceptance. The RTX4080 preview reached4,813 frames but later reported hidden with no advancement; normal seated Flight/Map→Observe and F3 isolation measurements remain undone. No Deck/Flight before/after performance claim is added.

Files include scripts/art_library/planet_reference_candidate.ts, planet_reference_worker.ts, planet_reference_worker_lifetime.ts, planet_reference_transmission.ts, native composers/fixtures and dedicated tests, plus docs/planet_lod_authoring.md. The separate PR checkout passes32 reference test files/174 tests and18 Python tests with isolated dependencies and one4GiB-heap subprocess per test file after an interrupted run. Cumulative cgroupOOM kills remain3 across the successful retry; exact killed-process attribution is unavailable.

Required npm run check and npm run build were attempted in the branch. Both expose missing modules already imported by unchanged origin/main code: debug-collision-geometry.ts and scene-material-registration.ts; build reaches dashboard after world/client succeed. Shared-checkout successes do not waive the branch failure. No world/sim/content/render package source, authority, live asset publication or deferred quality work was changed by this art candidate task. Evidence/history: output/playwright/planet-reference-20260914/; PR preparation and failure logs in its pr-preparation directory.

### 2026-09-14 — R14 post-OOM recovery and final moon comparisons, implemented review candidates, hardware unmeasured

Recovered the interrupted native reference review with heavy authoring/capture/test jobs serialized. Kernel cgroup records3 OOM kills; exact prior victim/time is not proven. Isolated process-per-file checks pass34files/187tests plus18Python tests before final Crystal updates. Required whole-branch check/build were attempted from independent dependencies and reveal unchanged upstream-main imports of missing debug-collision-geometry.ts and scene-material-registration.ts; world/client build passes before dashboard fails on the latter. No unrelated construction fixes were copied into this art branch.

Volcanic moon2r006 now passes the actual two-angle and47px root+Astra working comparison. Crystal moon1/2r009 fix finite terminal fracture coverage and pass morphology at both actual angles and79/52px scales; full gates remain open for localized bright native mineral shoulders, with a material-only successor in progress. Nine main-world art/morphology gates and17/19 individual moon working gates pass at inspected views, with Ice cap-shadow and Gas ring cast-shadow limitations still explicit. Nine focused r009 source/NullEngine tests pass. Source, Blender/GLB parity, images, JSON and failed histories are preserved under output/playwright/planet-reference-20260914/. Runtime worker retains one explicitly qualified fixed-detail geometry per reviewed Crystal body; current captures show zero pending builds and one retained node.

No new baseline Deck/Flight or isolation counters are available. The recovered shared public preview reportsGT730 rather than baselineRTX4080, stops advancing at27frames and has conflicting document/tool visibility. SwiftShader captures prove appearance/counts only, never timing. Hardware transition/whole-scene optical cost remains unmeasured; no R14/R15 completion or production activation is claimed. Draft PR2 is https://github.com/Dastari/sidereal_spacetime/pull/2; current files/tests/history and remaining work are enumerated in docs/planet_reference_iteration_20260914.md and docs/handoffs/planet_reference_recovery_20260914.md.

### 2026-09-14 — R14 final isolated working visual coverage, hardware unmeasured

Crystal moon1/2r010 now pass root and independent Astra actual two-angle plus79/52px reference comparisons. Selected existing bright mineral roles expose27/15 source triangles on10/6 traced polygons, preserving all9 geometry, PBR definitions, maps and placement IDs. All19 individual moons and nine main worlds have working art/morphology assessments at inspected views; Ice cap-shadow, Gas ring cast-shadow, all-seed/hemisphere and hardware gates remain separate. Canonical Crystal16 archives414 evidence files with previous histories/approvals/mappings intact and mainpin13.

Final isolated validation passes39TypeScript testfiles/216tests plus18Python tests, including NullEngine native surface/identity/retention and resource lifetime checks. Exact scripts, native source revisions, tests, actual screenshots/JSON and separate root/Astra reports are in the R14 evidence collection. No new Deck/Flight/scale2/Glow–Shadows–Lighting isolation measurements are claimed; both final moon captures retain one fixed-detail node with zero pending builds under SwiftShader, which proves appearance/counts only. R14 remains a review-candidate implementation, hardware unmeasured and not activated in the normal game. PR2 carries the scoped changes; hardware timing and final owner approval remain open.
...
Genesis follow-up evidence: Gas5, Temperate3 and Volcanic23 actual page screenshots are saved. Volcanic UI wheel zoom records ready LOD0 → ready LOD2 → retained LOD0, with retained nodes[0,2] and smoke restored; these are SwiftShader behavior/count checks only. Initial60s software waits for Volcanic and moon preparation are preserved, not treated as hardware timings. PR: https://github.com/Dastari/sidereal_spacetime/pull/6 (open, not merged).
...
2026-09-15 owner merge acceptance: the GT730 machine is not a suitable RTX4080 benchmark, but the owner accepts the observed performance and explicitly authorizes PR6 merge. The release is accepted; RTX4080 transition timing remains unmeasured. Pre-merge canvas-ui workspace pin correction passes npm ci dry-run; full check/build retain the previously documented baseline blockers.

### 2026-09-15 — Owner-directed yellow main sequence star, native r007

Implemented in isolated review, reference working acceptance by root and independent Astra. Native Blender photosphere and shared authored flare geometry/materials; runtime metadata/placement identity, cancel-safe GLB ownership, outward-axis animation and fixed-quad plasma fringe. Three NullEngine tests pass. Actual browser images, JSON and six-second playback under `output/playwright/render-plan/yellow-star-system/r007/`; software counts63 draws,388 meshes including358 instances,180 flare nodes. No hardware timing or Deck/Flight result is claimed. Whole-project check/build attempted; missing upstream render helpers and local Spacetime tool prevent full pass (logs retained). Real-game activation and celestial migration are the next stage of the owner request. Existing unrelated construction/authority changes remain outside this branch.


### 2026-09-15 — Owner-directed dynamic star r013 / plasma-r018 and live solar system

Implemented and deployed; full hardware performance acceptance remains unmeasured. Closed native honeycomb tiles share four PBR materials and bounded GPU height/spot animation, avoiding per-frame geometry generation. Expanded orange corona and rare curling eruptions use one shader quad. Root and independent Astra qualify playback, alternate/close views and actual Genesis plus production Observe integration against the original Yellow Main Sequence reference and later owner direction. Observe frames the2.1r visual envelope without changing physical body radii; landscape/portrait NullEngine projection tests pass. Native7,884,752bytes stays below8MiB; rejected oversized revisions are preserved.

Star isolated software counts reduced from r007's63draws/388meshes to r013/plasma-r018's17draws/7meshes (6draws bloom off); scenes differ in retired flare actors, so this is a scoped star comparison, not a whole-game benchmark. Production-renderer static fixture at1574x907: Deck1502draws, Flight957draws; original owner's2-ship RTX4080 baseline is not comparable to this fixture. Deck scale2=1489; Glow off1332; then Shadows off450; then Lighting off450. Settled Flight scale2=957; Glow off839; then Shadows off426; then Lighting off426. These are SwiftShader counts only. The first Flight sequence sampled its baseline during camera transition and is preserved as an unsuccessful settled baseline; use flight-settled-isolation.json.

Final phase3+solar migration passed fresh isolated baseline/update/persistence verification before authorized live publication. Live33bodies, applied receipt and exact seed hash verified; all3ship,3character and21inventory-item records unchanged. Compatible public client and5173preview use exact native star; Genesis continues to show r013. GT730 actual authenticated public Deck screenshot and Genesis ready/advancing-state evidence are saved, but automated F3/resize is unreliable and no baseline-resolution hardware timing is claimed. No unrelated IFCS phases4–6 were deployed.

Files: stellar surface/corona/eruption/runtime/catalog and tests; native Blender revision/script/ledger; solar content/migration/render adapter; independent asset packaging; Observe framing/test. Full check:287suites/1440pass/2skip, TypeScript pass, inherited missing documentation links still fail final docs stage. Full build passes. Compatible release TypeScript/26focused tests/client build pass. Evidence: output/playwright/render-plan/yellow-star-system/{plasma-r018,genesis-r013,system-r013}/. Remaining: hardware performance validation and owner final art/PR approval; no further star-art iteration requested by either reviewer.
