...
## Qualified uniform-bone follow-up

The TAA adapter now qualifies small native rigs (1–32 bones) for uniform current/previous bone matrices, reserving 512 vertex uniform vectors for other material work plus eight per bone, including the identity entry. It warms hidden cloned mesh/skeleton variants before changing the original palette storage. Original meshes remain visible during bounded warmup. Material attributes are explicitly dirtied on transition; restoring texture storage also clears the stale Babylon bone-velocity define. Off, disposal, hidden rigs and material changes release or requalify this presentation-only state. Unsupported rigs retain the conservative history guard.

Actual WebGL2 native-crew evidence is recorded in `antialiasing_uniform_bone_evidence.json`: all 37 visible skinned shaders enabled bone velocity after ten warmup frames, with no missing-character frame. During 178 changed matrix values, the actual TAA shader accumulation factor remained 0.05 across all 12 sampled animated frames. Off restored texture palettes and zero bone-velocity defines; disposal left zero warmup meshes, skeletons and camera passes. Every sampled GL error was zero. `output/playwright/antialiasing-review/uniform-animated.png` was visually reviewed. This fixture proves the exact native shader/storage transition, not an FPS improvement or full game acceptance; the release owner performs the combined normal-game review before publication.
