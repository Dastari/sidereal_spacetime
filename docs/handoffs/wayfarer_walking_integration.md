...
Offline quarter-metre reachability found 922 standing grid points, of which 816 connect to the selected spawn. Main, aft and forward corridor probes are reachable. Some room openings are conservatively blocked by whole-object covers. The bridge sill is a real 31.25 mm step, with 2.09375 m clearance above it; the later qualified threshold authority now steps over its exact authored support (see `wayfarer_threshold_integration.md`). The pilot seat's center is correctly occupied geometry, not an ordinary walking destination. A seated approach/support transition needs separate qualification.
...
Room-specific floor colouring is not present in the pinned floor variants; the authored grey mapped surfaces are retained. This is a future semantic theme/decal feature, not lost imported material. The later qualified threshold pass opens the exact 31.25 mm bridge sill while preserving its jambs, headroom and raised markers. Complete visuals and ordinary corridor walking are accepted for this static review slice; functional cargo, piloting, physical airlocks and complete multi-deck gameplay are not claimed.
