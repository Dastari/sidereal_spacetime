...
## Actual reconnect defect correction

The parent browser review found that an unacknowledged old-socket control release could block the serialized queue forever. The helper now bounds release acknowledgement at500ms and claim acknowledgement at1500ms. An uncertain timed-out claim queues a conditional release before retry. Late acknowledgements cannot restore local control after replacement/disposal; timeout does not cancel an already-sent server reducer. Same-connection adapters must enqueue requests in call order, as the SDK does.

An old socket's unresolved intent call also no longer blocks a replacement connection. The transmitter invalidates that logical pending attempt on connection change; late completion/rejection is ignored. Same-connection acknowledgement waits are bounded to1000ms by the next sampler offer, with at most one new retry attempt per second while acknowledgements remain unresolved. There can be uncertain older server requests after a timeout; monotonic per-connection sequences prevent them from superseding newer accepted intent. Disposal suppresses deferred sends that have not been issued yet.

The four focused helper suites now pass31 tests, including permanently pending release/claim/send, new-socket takeover, late rejection, disposal and same-connection release/reclaim ordering. Root owns renewed actual-browser acceptance of this corrected candidate.
