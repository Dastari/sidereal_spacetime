...
Status: native and integrated Deck/Flight browser evidence passed. Candidate13990 was held by the publication guard when the independent character/backpack release advanced; its rendering changes are being combined into a new source snapshot based on that accepted release. No world authority, local glass, art revision or public release approval changes are implied.
...
## Integrated review and forward reconciliation

The ordinary dedicated review account used the exact13990 built entry through browser response routing. Its existing actor and ship were converted through the normal explicit Ship refit button, preserving all99 items,18 containers, two weapon states, five hotbar rows, appearance, station identity and actor local position. The two expected supply/fuel container placement changes are recorded separately in `.runtime/multiship-integrated-review/conservation.json`; this was not the owner's Dastari account.

Actual Deck right-drag orbit changed azimuth by about95 degrees while all423 wall meshes remained enabled. Switching with the actual Flight button disabled all102 floor,131 equipment and420 cargo meshes, plus the avatar, while retaining423 walls and enabling179 roof meshes. Accepted metrics and the complete Flight image are in `output/playwright/multiship-integrated-review/`. The attempted orbit screenshot timed out and a later capture contained a loading cover, so that PNG is not visual acceptance evidence. Long paused review sessions also encountered legitimate authentication expiry/reconstruction; no clean-renewal or zero-error claim is made for this run. The browser was blanked and closed.

The13990 activation guard refused to replace another agent's staged release. The external exact e1e51225 client and fdd790fd authority subsequently became live. The new `.runtime/release-checkouts/combined-faces-render-20260910` snapshot preserves that source and its native r009 faces/backpack behavior, adding the accepted remote/wall changes, neutral portrait loading and separately qualified AA work. No world publication or repeated backup is part of this client integration. Its gates and exact pins are recorded independently before any activation.
