...
### Refit, native airlock and loading release — 2026-09-10
...
### Current independent native GLB delivery — 2026-09-10

The active client advanced independently to `8246fafc6e13048bd1eaabd5a63d6c06dc9ca1ba983de4bdb6e466de676c4a35`, entry `/assets/index-B4bNcXSU.js`; the delivery integration preserves that newer artifact. World remains2ade above. Independently pinned delivery `520de12b8dada34e0016524008d5e098af7b5cd1ac42a89f7875024898e5678d` now negotiates gzip for native GLBs, retaining exact decoded hashes and MIME, cache validators and range semantics. Actual public GETs reduced the two measured crew/parts GLBs from38.68MB to5.39MB (86.05%). This is a delivery measurement, not a model/rendering change.

### Whole-tree asset delivery — 2026-09-13

`scripts/glb_delivery.mjs` (`assetDelivery`) now covers every published type under `/assets/`: GLB, JSON, PNG, SVG, HDR and Vite's own bundle files. Compressible types prefer the `.br` sidecar written at build time, then the `.gz` sidecar, then on-the-fly gzip; PNG and fonts always go out as identity. Every response carries a strong sha256 identity ETag (weak `-br`/`-gzip` variants for encoded bodies), `Last-Modified`, full conditional handling and identity byte ranges. Vite bundle files directly under `/assets/` are content hashed, so they are `public, max-age=31536000, immutable`; published runtime assets keep plain names and stay `no-cache` (revalidate) until asset URLs carry a content hash. Sidecars are only trusted when not older than their source, and are never served at their own URLs. This is a delivery change (`public-client-delivery-stage` / `-activate`), not a client or world publication; a client rebuild is needed before sidecars exist in the live release.

`public-client-delivery-stage`, `public-client-delivery-activate` and `public-client-delivery-rollback` manage that independent artifact under the same publication lock. Its current manifest is `.runtime/public-client/delivery.json`; the normal client manifest remains separate. Only the public frontend restarts, and rollback retains the current app/world. See [HTTP proof and recovery commands](handoffs/glb_delivery_release_20260910.md). The authenticated owner's separately requested [Dastari ship conversion](handoffs/dastari_owner_refit_20260910.md) is complete without another client/world publication.
