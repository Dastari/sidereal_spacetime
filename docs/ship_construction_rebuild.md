...
The pinned full262-placement Wayfarer document has qualified conservative collision, native floor/cockpit/roof/cutaway rendering, low-step support, per-instance inventory/equipment and occupied-station flight. Normal fresh accounts now receive independent authored Wayfarers in the shared world; public UI creation, reload and actual server-restart inventory persistence passed. Its source still contains one deck. Existing legacy ships are preserved until an explicit state-conserving refit; that adapter is in active integration. Supported cargo stacks, complete services, native damage and the complete semantic template remain separate gates. See [qualified Wayfarer walking](handoffs/wayfarer_walking_integration.md); none of this grants pressure, payload, health or armor ratings merely from the meshes.
...
## Owner fit correction and measured native audit — 2026-09-10

Model authoring is the primary compatibility boundary. Every exported surface must fit the asset's reserved polygon and height, including handles, bevels and trim. A 2 m reservation cannot export 2.04 m geometry; smaller or irregular assets keep explicit shapes and consistent origins. Structural walls occupy an outward reservation and may not consume floor area advertised as usable. Catalog admission and blueprint publication must enforce this contract; per-placement nudges must not conceal bad source interfaces.

The [native authoring audit](handoffs/wayfarer_authored_interface_audit.md) confirms all twelve r002 floor shapes fit their actual declared envelopes. The older wall assembly is incompatible with the usable-floor boundary: 21 boundary-bearing source groups intrude, with some mixed partition geometry needing separation. Six stable locker instances have measured native wall penetration. Their bodies fit their existing declared envelope; shifting them inward is not the chosen repair. Correct wall source datums and qualify equipment mounting/support interfaces at preserved IDs and transforms before a later validated refit. This audit does not invalidate or replace the independently qualified preserved-fuel mount.

Visual coplanarity, pressure enclosure, access and support remain separate gates. Local roof-seam and airlock chamber qualifications do not establish whole-main-hull sealing. Existing native source revisions, inventories and UUIDs remain unchanged while these new authoring revisions are qualified.
