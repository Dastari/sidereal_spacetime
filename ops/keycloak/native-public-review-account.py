"""CT116-only managed native-starter ordinary public-review account. Never resets existing users."""
import json
import os
import secrets
...
path = root / 'sidereal-native-public-review.json'
created = False
if path.exists():
    ...
    assert account['username'] == 'sidereal-native-public-review'
    assert account.get('managedBy', 'native-starter-public-acceptance-r001') == 'native-starter-public-acceptance-r001'
    if not actual.get('email'):
        request('/admin/realms/dastari/users/' + account['id'], {'email': 'sidereal-native-public-review@example.invalid', 'emailVerified': False}, 'PUT')
else:
    username = 'sidereal-native-public-review'
    ...
    status, response_headers, _ = request('/admin/realms/dastari/users', {
        ...
        'firstName': 'Native', 'lastName': 'Review',
        'email': 'sidereal-native-public-review@example.invalid',
        'attributes': {'sidereal_review_owner': ['native-starter-public-acceptance-r001']},
        'requiredActions': [],
        'credentials': [{'type': 'password', 'value': password, 'temporary': False}],
    }, 'POST')
    assert status == 201
    account = {'id': response_headers['Location'].rsplit('/', 1)[1], 'username': username, 'password': password, 'managedBy': 'native-starter-public-acceptance-r001'}
...
path.chmod(0o600)
print(json.dumps({'nativePublicReviewAccountCreated': created, 'distinctFromPrimary': True,
