...
export function HullSizePanel({
  ...
  return (
    <section className="layout-hull-summary" aria-label="Hull limits">
      <strong>{doc.structure?.hull.name ?? "No hull limits applied"}</strong>
      {doc.structure && (
        <span>
          {doc.structure.hull.width / 32} × {doc.structure.hull.length / 32} ×{" "}
          {doc.structure.hull.height / 32} m
        </span>
      )}
      <details className="editor-disclosure layout-hull-editor">
        <summary>Edit sizes</summary>
        <EditorSection title="Hull size catalogue">
          ...
          <details>
            ...
            <button
              className="layout-wide"
              onClick={editor.exportHullCatalogue}
            >
