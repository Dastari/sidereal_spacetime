...
2026-09-26 owner art-library storage direction ("a lot of the reference stuff should be moved into the wiki; be selective about what we push to git"): reference and review media (exact reference crops, boards, renders, comparison sheets, revision screenshots, review pages) no longer live in git. Web-sized copies with each design's ledger text are in the Sidereal wiki (`Art/Library/<design>`); exact originals are in `/root/sidereal-art-archive/` on CT107 with `MANIFEST.sha256`; [assets/art-library/ARCHIVED.json](assets/art-library/ARCHIVED.json) lists every moved file and hash, and `scripts/art_catalog.py check` accepts archived evidence by that hash. Preserving every exact crop and revision still applies: put new review media in the archive and wiki, not in git. Model sources, textures, ledgers and runtime inputs stay in the repository.
