...
import subprocess
import unittest
import checkout_guard  # Also protects in-process qualification reads.
...
class NativeAirlockGeometryTests(unittest.TestCase):
    def test_actual_attached_native_seal_support_and_continuous_door_motion(self):
        ...
        subprocess.run([sys.executable, str(Path(__file__).with_name('checkout_guard.py')), str(inlet.ROOT/'scripts/qualify_wayfarer_airlock_inlet.py'), str(motion.KIT.relative_to(inlet.ROOT))], cwd=inlet.ROOT, check=True, stdout=subprocess.DEVNULL)
        ...
        subprocess.run([sys.executable, str(Path(__file__).with_name('checkout_guard.py')), str(inlet.ROOT/'scripts/qualify_wayfarer_airlock_inlet_motion.py')], cwd=inlet.ROOT, check=True, stdout=subprocess.DEVNULL)
    ...
    def test_conservative_native_projection_preserves_aperture_and_authority_budget(self):
        subprocess.run([sys.executable, str(Path(__file__).with_name('checkout_guard.py')), str(inlet.ROOT/'scripts/qualify_wayfarer_airlock_walking.py')], cwd=inlet.ROOT, check=True, stdout=subprocess.DEVNULL)
