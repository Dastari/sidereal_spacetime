...
def run():
    ...
    faces={
      ...
      'front':cube([-7,5.4937,-1],[7,5.4937437,4]),
      'rear':cube([-7,-8.4937437,-1],[7,-8.4937,4]),
      'left':cube([-4.7827327,-10,-1],[-4.7827,7,4]),
      'right':cube([4.7827,-10,-1],[4.7827327,7,4]),
    }
    scenarios={'control-all-caps':list(faces.values()),**{'native-'+omit:[v for k,v in faces.items()if k!=omit]for omit in faces}}
    result={}
...
if __name__=='__main__':
    report=run()
    path=ROOF/'closure-regions-a006.json'
