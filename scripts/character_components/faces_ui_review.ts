...
scene.clearColor = new Color4(0.022, 0.038, 0.067, 1);
...
camera.orthoLeft = -1.8;
camera.orthoRight = 1.8;
camera.minZ = 0.01;
...
new HemisphericLight("private-fill", Vector3.Up(), scene).intensity = 0.9;
const pivot = new TransformNode("private-mannequin", scene);
...
CanvasUI.prototype.paint = function () {
  if (this.canvas === canvas) ui = this;
  return originalPaint.call(this);
};
const state: GameUIState = {
  status: "ready",
  error: "",
  modelStatus: "Private local appearance preview; no server",
  hasActor: true,
  connected: false,
  actorName: "Preview mannequin",
  shipName: "Private review",
  seated: false,
  nearStation: false,
  interior: true,
  receipts: 0,
...
const actions: GameUIActions = {
  ...
  motion: inert,
  camera: inert,
  focusDestination: inert,
  dismiss: inert,
  retry: inert,
  crew: (next) => {
    appearance = { ...next };
    changes.push({ at: performance.now(), appearance });
    crew?.customize(appearance);
  },
};
const gameUI = createGameUI(canvas, scene, state, actions, [
  { id: "private", name: "Private review" },
]);
document.querySelector<HTMLInputElement>("#asset")!.onchange = async (
  event,
) => {
  ...
  crew.customize(appearance);
  for (const clip of scene.animationGroups) clip.stop();
  ui?.invalidate();
};
let disposed = false;
function dispose() {
  ...
  scene.dispose();
  engine.dispose();
  CanvasUI.prototype.paint = originalPaint;
}
