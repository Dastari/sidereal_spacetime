...
def run():
    ...
    for name,extra in scenarios.items():
        ...
        candidates=[]
        native=(box-solids).to_mesh64()
        mesh=trimesh.Trimesh(vertices=np.asarray(native.vert_properties)[:,:3],faces=np.asarray(native.tri_verts),process=False)
        area=mesh.area_faces;keep=area>0;print('ZERO AREA REMOVAL',len(area)-int(keep.sum()),flush=True)
        mesh.update_faces(keep);mesh.merge_vertices(merge_tex=True,merge_norm=True)
        normalized=m.Manifold(m.Mesh64(np.array(mesh.vertices,dtype=np.float64),np.array(mesh.faces,dtype=np.uint64)))
        print('NORMALIZED',normalized.status(),normalized.num_tri(),flush=True)
        for component in normalized.decompose():
...
if __name__=='__main__':
    report=run()
    path=ROOF/'closure-regions-a008.json'
