...
for(const rightHanded of [false,true])for(const look of ['captain','marine'] as const){
 ...
 for(const [asset,profile,directed] of cases){
  ...
  for(const yaw of [0,-20,20,-45,45,-70,70,-90,90,-135,135])for(const pitch of [0,-20,20,-40,40,-60,60]){
   for(let frame=0;frame<20;frame++){scene.onBeforeAnimationsObservable.notifyObservers(scene);solver.update({yaw:yaw*Math.PI/180,pitch:pitch*Math.PI/180,facing:0,profile,active:true,reducedMotion:true,itemId:asset},.1);scene.onAfterAnimationsObservable.notifyObservers(scene);}
   const d=solver.diagnostics;costs.push(d.cpuMs);maxPrimary=Math.max(maxPrimary,d.primaryErrorM);maxSupport=Math.max(maxSupport,d.supportErrorM);maxPenetration=Math.max(maxPenetration,d.penetrationM);if(d.status!=='solved'&&d.status!=='fallback')failed++;
   results.push({rightHanded,look,asset,profile,directed:!!directed,yaw,pitch,status:d.status,primaryM:d.primaryErrorM,supportM:d.supportErrorM,shoulderM:d.shoulderErrorM,eyeM:d.eyeErrorM,penetrationM:d.penetrationM,upperArmPenetrationM:d.upperArmPenetrationM,aimErrorDegrees:d.aimErrorDegrees,iterations:d.iterations});
  }
...
costs.sort((a,b)=>a-b);if(existsSync(folder+'/grid-audit.json'))copyFileSync(folder+'/grid-audit.json',folder+'/grid-audit-'+Date.now()+'.json');writeFileSync(folder+'/grid-audit.json',JSON.stringify({note:'Actual staged GLB rig and equipment; two handedness paths, rotated parent, 77 yaw/pitch combinations per profile. Failures retained, not acceptance.',samples:results.length,cpuMs:{median:costs[Math.floor(costs.length*.5)],p95:costs[Math.floor(costs.length*.95)],max:costs.at(-1)},results},null,2));
