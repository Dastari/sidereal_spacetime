...
for (const id of ids) {
  ...
  it("retains complete native regional surfaces UVs normals and identities at every LOD", () => {
    const a = composeCrystalMoonReference(kit, 38, 0),
      b = composeCrystalMoonReference(kit, 38, 2);
  ...
  it("NullEngine sees embedded deep crater floor before the cleared native substrate", () => {
    ...
    try {
      const meshes = composeCrystalMoonReference(kit, 38, 0, true).map(
        (b, i) => {
      ...
      expect(hits[0].mesh.metadata.materialRole).toBe(1);
      ...
      const east = Vector3.Cross(Vector3.Up(), n).normalize(),
        ...
        ph = meshes
          ...
          .filter((v) => v.hit.hit && v.hit.pickedPoint)
          .sort((a, b) => a.hit.distance - b.hit.distance);
      expect(ph[0].mesh.metadata.materialRole).toBe(1);
      ...
      const sd = n
        ...
        sh = meshes
          ...
          .filter((v) => v.hit.hit && v.hit.pickedPoint)
          .sort((a, b) => a.hit.distance - b.hit.distance);
      expect(sh[0].mesh.metadata.materialRole).toBe(1);
...
it("keeps the large crater floor exposed in all16 seeded regional placements", () => {
  ...
  try {
    for (const id of ids) {
      ...
      const meshes = composeCrystalMoonReference(kit, 38, 0).map((b, i) => {
      ...
      for (let i = 0; i < kit.compositionRecipe.scales.length; i++) {
        const y = 1 - (2 * (i + 0.5)) / kit.compositionRecipe.scales.length,
          ...
          hits = meshes
            ...
            .filter((v) => v.hit.hit && v.hit.pickedPoint)
            .sort((a, b) => a.hit.distance - b.hit.distance);
        expect(hits[0].mesh.metadata.materialRole, `${id}/crust-${i}`).toBe(1);
      }
...
it("retains authored optical materials while adding dense blunt districts and an asymmetric moon2 shell", () => {
  for (const id of ids) {
    ...
    const batches = composeCrystalMoonReference(kit, 38, 0),
      districts = new Set(
        batches.flatMap((b) =>
          b.ranges
            .filter((r) => r.partId.includes("/native-crystal-district-"))
            .map((r) => r.partId),
        ),
      );
    expect(districts.size).toBeGreaterThan(35);
    for (const name of [
      "colossal-cluster",
      "leaning-colossal-cluster",
      "medium-cluster",
    ])
      expect(kit.variants.find((v: any) => v.name === name)).toEqual(
        old.variants.find((v: any) => v.name === name),
      );
    if (id.endsWith("-2")) {
      const ground = kit.variants.find((v: any) => v.name === "ground-sphere"),
        radii = [];
      for (let i = 0; i < ground.positions.length; i += 3)
        radii.push(Math.hypot(...ground.positions.slice(i, i + 3)));
