...
code=f'''async (page)=>{{
 ...
 await page.evaluate(()=>{{const r=window.planetReview;r.engine.stopRenderLoop();{'for(const light of r.scene.lights){const shadow=light.getShadowGenerator();if(shadow)shadow.bias='+str(a.depth_bias)+';}' if a.depth_bias is not None else ''}r.camera.radius={a.radius};{'r.camera.target.set('+','.join(map(str,a.target))+');' if a.target else ''}{'r.camera.alpha='+str(a.alpha)+';' if a.alpha is not None else ''}{'r.camera.beta='+str(a.beta)+';' if a.beta is not None else ''}r.scene.render();}});await page.screenshot({{path:{q(str(paths[0]))}}});
 await page.evaluate(()=>{{const r=window.planetReview;r.camera.alpha+=1.6;r.scene.render();}});await page.screenshot({{path:{q(str(paths[1]))}}});
'''
if a.reference_scale:code+=f'await page.evaluate(()=>{{const r=window.planetReview;r.camera.radius={a.reference_radius};r.scene.render();}});await page.screenshot({{path:{q(str(paths[2]))}}});\n'
code+='}'
...
expression='JSON.stringify({viewport:{width:innerWidth,height:innerHeight},canvas:{width:window.planetReview.engine.getRenderWidth(),height:window.planetReview.engine.getRenderHeight()},stats:window.planetReview.stats(),shadowsEnabled:window.planetReview.scene.shadowsEnabled,camera:{alpha:window.planetReview.camera.alpha,beta:window.planetReview.camera.beta,radius:window.planetReview.camera.radius},shadows:window.planetReview.scene.lights.filter(l=>l.isEnabled()&&l.getShadowGenerator()).map(l=>({casters:l.getShadowGenerator().getShadowMap().renderList?.length,normalBias:l.getShadowGenerator().normalBias,bias:l.getShadowGenerator().bias}))})'
