...
for (const kind of ["whole", "bare"]) {
  const result = await build({
    absWorkingDir: root,
    entryPoints: [entry],
    outfile: path.join(review, kind + ".bundle.js"),
    bundle: true,
    format: "esm",
    platform: "browser",
    metafile: true,
        ...
        setup(builder) {
          builder.onResolve({ filter: /^\.\.\/\.\.\/packages\// }, (arg) => {
            ...
            return builder.resolve(
              ...
              {
                kind: arg.kind,
                resolveDir: candidate,
              },
            );
          });
          builder.onLoad(
            { filter: /framed-wayfarer-visuals\.json$/ },
            async () => ({
              contents: await readFile(
                path.join(review, kind + "-bindings.json"),
                "utf8",
              ),
              loader: "json",
            }),
