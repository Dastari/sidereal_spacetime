...
export function composeSolidMoonReference(
  kit: NativePlanetKit,
  seed: number,
  lod: 0 | 1 | 2,
  diagnostic = false,
  ...
  if (
    !recipe ||
    !recipe.referenceId.startsWith("planets--") ||
    !recipe.referenceId.includes("-moon-") ||
    !Number.isInteger(recipe.layoutSeed) ||
    recipe.scales.length !== 12 ||
    recipe.scales.some((v) => !Number.isFinite(v) || v < 0.3 || v > 0.46)
  )
    throw new Error("Explicit solid moon composition recipe required");
