...
for name,layout in layouts:
 ...
 bpy.ops.object.select_all(action='DESELECT');obj.select_set(True);bpy.context.view_layer.objects.active=obj;bpy.ops.object.convert(target='MESH');obj=bpy.context.object;obj.name='GEO-'+name;obj['role']='planet';obj['authoring']='native-toxic-fog-r003';obj.data.materials.append(mat)
...
(out/'validation.json').write_text(json.dumps({'nativeManifold':True,'alphaMin':float(alpha.min()),'alphaMax':float(alpha.max()),'alphaMean':float(alpha.mean()),'triangles':{v['name']:len(v['indices'])//3 for v in kit['variants']},'scope':'Toxic-only isolated native translucent surface approximation; no true volumetric scattering or acceptance','sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest()for p in out.iterdir()if p.is_file()}},indent=2));print('TOXIC_FOG_R003_DONE')
