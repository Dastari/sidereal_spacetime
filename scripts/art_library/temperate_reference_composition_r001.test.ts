...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/temperate-r001/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
it("retains continental terrain forest and material attributes at every LOD", () => {
  ...
  expect([...ids].filter((id) => id.startsWith("continent-"))).toHaveLength(6);
  expect([...ids].filter((id) => id.startsWith("forest-"))).toHaveLength(72);
  expect(a.reduce((n, b) => n + b.indices.length / 3, 0)).toBeLessThan(120000);
  ...
  try {
    ...
    for (const direction of [n, n.negate()]) {
      ...
      if (direction === n) {
        ...
        expect(hits[0].hit.pickedPoint!.length()).toBeGreaterThan(1.18);
        const water = hits.find((v) => v.mesh.metadata.materialRole === 0)!;
        expect(water.hit.distance - hits[0].hit.distance).toBeGreaterThan(0.18);
      } else {
