...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/rocky-r001/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("retains exact major crater geometry, material roles and placement identity at each LOD", () => {
  const heroes = (level: (typeof levels)[number]) =>
    level.map((batch) =>
      batch.ranges
        .filter((r) => r.partId.startsWith("crater-"))
        .map((r) => ({
          id: r.partId,
  ...
  expect(
    new Set(
      levels[0].flatMap((b) =>
        b.ranges
          .filter((r) => r.partId.startsWith("crater-hero"))
          .map((r) => r.partId),
      ),
    ).size,
  ).toBe(7);
...
it("excavates the native substrate and lowers crater floors beneath the general surface", () => {
  ...
  expect(minimum).toBeLessThan(0.96);
  const dark = levels[0][2];
  let below = 0;
  for (const r of dark.ranges.filter((r) =>
    r.partId.startsWith("crater-hero"),
  )) {
...
it("clears the single diagnostic bowl floor with the actual triangulated substrate", () => {
  ...
  const ground = hits(level[0], "ground"),
    floor = hits(level[2], "crater-diagnostic");
  expect(ground).toBeGreaterThan(0.88);
  expect(floor).toBeGreaterThan(ground + 0.003);
  expect(floor).toBeLessThan(0.96);
  expect(new Set(level.flatMap((b) => b.ranges.map((r) => r.partId)))).toEqual(
    new Set(["ground", "crater-diagnostic"]),
  );
});
