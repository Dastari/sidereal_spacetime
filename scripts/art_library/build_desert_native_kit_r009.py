...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r009'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-desert-r009'
    return obj
...
for variant,perimeter in enumerate(regions):
    ...
    height=[.095,.08,.09][variant]
    parts=[cliff_solid(name+'-broad-backing',perimeter,height)]
    # Two asymmetric broad middle terraces divide the contiguous low field;
    # their opposed notches bound an inset erosion basin inside the patch.
    middle_a=[(-1.25,-.45),(-.97,-.45),(-.97,-.78),(-.58,-.78),(-.58,-.51),
      (-.28,-.51),(-.28,-.12),(-.49,-.12),(-.49,.12),(-.19,.12),(-.19,.54),
      (-.58,.54),(-.58,.79),(-1.07,.79),(-1.07,.42),(-1.25,.42)]
    middle_b=[(.04,-.68),(.47,-.68),(.47,-.44),(1.08,-.44),(1.08,-.07),
      (.83,-.07),(.83,.26),(1.18,.26),(1.18,.50),(.78,.50),(.78,.72),
      (.33,.72),(.33,.43),(.02,.43),(.02,.18),(.28,.18),(.28,-.09),(.04,-.09)]
    # Distinct offsets swap which broad terrace dominates each region.
    first=.30 if variant!=1 else .23;second=.20 if variant!=1 else .32
    parts.append(cliff_solid(name+'-west-middle-terrace',middle_a,first,cap=0))
    parts.append(cliff_solid(name+'-east-middle-terrace',middle_b,second,cap=5))
    connector=[(-.92,-.71),(-.52,-.71),(-.52,-.54),(.19,-.54),(.19,-.33),
       (-.20,-.33),(-.20,-.20),(-.57,-.20),(-.57,-.41),(-.92,-.41)]
    parts.append(cliff_solid(name+'-low-connecting-shelf',connector,.15,cap=1))
    ...
    for section,(start,end,drop) in enumerate([(0,9,.18),(10,17,.14),(len(perimeter)-8,len(perimeter),.20)]):
        ...
        parts.append(cliff_solid(name+'-branch-shelf-'+str(section),foot,max(.015,height-drop*.22),cap=5))
    ...
    for i,(x,y,w,d,h) in enumerate(column_specs[:13 if variant==0 else 6 if variant==1 else 0]):
        # Each region retains a visibly different skyline, never equal poles.
        x,y=(x,y) if variant==0 else (y,-x) if variant==1 else (-x,y)
        obj=pillar(name+'-monument-column-'+str(i),center[0]+x,center[1]+y,w,d,.29+(h-.40)*[.49,.31,.35][variant],0)
    ...
    if variant<2: parts.append(cliff_solid(name+'-eroded-upper-shoulder',shoulder,.34 if variant==0 else .27,cap=4))
    forms.append((name,parts))
