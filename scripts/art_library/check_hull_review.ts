...
const out='.runtime/art-library/hull/r001', read=(p:string)=>JSON.parse(readFileSync(p,'utf8'));
...
for(const component of components){
 ...
 catalog.assets.push({id:component.id,label:`Pilot ${component.slug} / r001`,category:component.category,nodes:[],bounds:component.bounds,thumbnail:`/assets/assembly/hull-review/${component.slug}/cutout.png`,visual:{url:`/assets/assembly/hull-review/${component.slug}/clean.glb`,sha256:createHash('sha256').update(readFileSync(`${out}/${component.slug}/clean.glb`)).digest('hex'),designId:'shipyard.hull.pilot-section',revision:1,bounds:component.bounds,damagePreview:'unsupported'}});
}
for(const id of ['part-c0b6b036f5b3dd8bd2f3','part-d9f37a5f7ea6e8d13254']){catalog.assets.push(original.assets.find(a=>a.id===id)!);library.volumes[id]=base.volumes[id];}
