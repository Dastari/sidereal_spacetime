...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r013'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-desert-r013'
    return obj
...
for variant in range(3):
 ...
 specs=[(-.22,.10,.40,.37,.64),(.03,.12,.43,.38,.74),(.27,.10,.34,.34,.62),
      (-.26,-.15,.36,.33,.55),(-.02,-.16,.39,.36,.67),(.22,-.15,.34,.33,.57),
      (-.11,-.37,.31,.28,.50),(.16,-.35,.29,.29,.53)]
 if variant<2:
    cx,cy=(-.67,-.05) if variant==0 else (.0,.65)
    parts.append(pillar(name+'-compact-shared-massif-foot',cx,cy,.98,.82,.55 if variant==0 else .39,0))
 for i,(x,y,w,d,h) in enumerate(specs[:8 if variant==0 else 4 if variant==1 else 0]):
    cx,cy=(-.67,-.05) if variant==0 else (.0,.65)
