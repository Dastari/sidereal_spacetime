...
for (const id of ["crystal-moon-1", "crystal-moon-2"])
  it(`${id} replaces selected shells while retaining every unselected native triangle exactly`, () => {
    const path = `output/playwright/planet-reference-20260914/${id}-r007/`,
      kit = JSON.parse(readFileSync(path + "kit.json", "utf8")),
      priorPath = path.replace("r007", "r005"),
...
it("r007 carries broad regional low/high surfaces and recessed runs across multiple source districts", () => {
  for (const id of ["crystal-moon-1", "crystal-moon-2"]) {
