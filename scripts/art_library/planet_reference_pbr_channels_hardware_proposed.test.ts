...
async function bindActualCandidate(scene:Scene,roles:ReferenceMaterialRole[]){
 const source=readFileSync('scripts/art_library/planet_reference_candidate_hardware_proposed.ts','utf8'),start=source.indexOf(' const texturePromises='),end=source.indexOf(' const nativeToxicFog=',start);
