...
it("exposes the authored hot fracture surfaces above final overlapping crust", () => {
  ...
  for (const id of ids) {
    ...
    try {
      ...
      expect(visible / samples, id).toBeGreaterThan(0.5);
      expect(results[id].visibleAreaEstimate, id).toBeGreaterThan(0.015);
    } finally {
      scene.dispose();
      engine.dispose();
    }
  ...
  writeFileSync(
    "output/playwright/planet-reference-20260914/volcanic-moons-r002-exposed-fracture-rays.json",
