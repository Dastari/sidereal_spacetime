...
export function composeVolcanicMoonReference(
  kit: NativePlanetKit,
  seed: number,
  lod: 0 | 1 | 2,
  diagnostic = false,
  ...
  const recipe = (
    kit as NativePlanetKit & {
      compositionRecipe?: {
        ...
        warmEdgeRegions?: number[];
      };
    }
  ).compositionRecipe;
  if (
    !recipe ||
    !["planets--volcanic-moon-1", "planets--volcanic-moon-2"].includes(
      recipe.referenceId,
    ) ||
    !Number.isInteger(recipe.layoutSeed) ||
    recipe.scales.length !== 12 ||
    recipe.scales.some((v) => !Number.isFinite(v) || v < 0.3 || v > 0.46)
  )
    throw new Error("Explicit volcanic moon composition recipe required");
  if (
    recipe.warmEdgeRegions &&
    (recipe.referenceId !== "planets--volcanic-moon-2" ||
      recipe.warmEdgeRegions.length !== 2 ||
      recipe.warmEdgeRegions[0] !== 2 ||
      recipe.warmEdgeRegions[1] !== 9)
  )
    throw new Error("Explicit bounded warm-edge placements required");
  ...
  function emit(
    ...
    if (
      ...
      throw new Error("Missing native rocky normals or UVs");
  ...
  if (diagnostic)
    ...
    for (let i = 0; i < 12; i++) {
      ...
      regions.push({
        ...
        name: recipe.warmEdgeRegions?.includes(i)
          ? "battered-region-b-edge"
          : (
                recipe.referenceId.endsWith("-2")
                  ? i % 4
                  : (i + recipe.variantOffset) % 2
              )
            ? "battered-region-b"
            : "battered-region-a",
        id: `${recipe.referenceId}/crust-${i}`,
      });
    }
