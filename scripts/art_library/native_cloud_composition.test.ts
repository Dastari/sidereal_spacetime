...
import {composeNativeClouds,NATIVE_CLOUD_BANK_LIMIT} from './native_cloud_composition';
const source=readFileSync('output/playwright/planet-reference-20260914/cloud-r001/kit.json','utf8');
...
describe('native cloud worker composition',()=>{
 ...
 it('bounds triangle count, full placement ranges and altitude for every bank',()=>{
  ...
  expect(batches.reduce((sum,b)=>sum+b.indices.length/3,0)).toBeLessThanOrEqual(24192);
  for(const b of batches){let end=0;for(const r of b.ranges){expect(r.firstTriangle).toBe(end);end+=r.triangleCount;}expect(end).toBe(b.indices.length/3);
   for(let i=0;i<b.positions.length;i+=3){const radius=Math.hypot(...b.positions.slice(i,i+3));expect(radius).toBeGreaterThanOrEqual(1.08);expect(radius).toBeLessThanOrEqual(1.14);}
