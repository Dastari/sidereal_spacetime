...
import {composeRockyReference} from './rocky_reference_composition_r003';
...
it('excavates the native substrate and lowers crater floors beneath the general surface',()=>{
 ...
 expect(minimum).toBeLessThan(.96);
...
it('clears the single diagnostic bowl floor with the actual triangulated substrate',()=>{
 ...
 expect(ground).toBeGreaterThan(.88);expect(floor).toBeGreaterThan(ground+.003);expect(floor).toBeLessThan(.96);
