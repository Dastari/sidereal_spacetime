...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r002'
    return obj
...
camera=bpy.context.object;camera.name='CAM-kit-review';camera.rotation_euler=(Vector((0,1.10,.15))-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.type='ORTHO';camera.data.ortho_scale=14.5
scene.camera=camera
