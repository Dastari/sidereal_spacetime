"""r015 editable thick snow masses and connected blue bluff feature kit."""
...
for variant in range(3):
 ...
 if variant>0: parts.append(box('GEO-local-broken-snow-shelf-%d'%variant,offset+Vector(([0,.18,-.24][variant],[0,-.13,.21][variant],.22)),([1,.67,.48][variant],[1,.59,.64][variant],[0,.24,.18][variant])))
 forms.append(parts)
for variant in range(3):
 offset=Vector(((variant+3)*3.,0,0));parts=[]
 height=[1.05,2.10,.55][variant]
 parts.append(prism('GEO-connected-blue-bluff-%d'%variant,offset+Vector((-.18,.08,0)),1.68,1.27,height,.08+variant*.13,variant))
 parts.append(prism('GEO-wide-embedded-ice-ledge-%d'%variant,offset+Vector((.39,-.18,0)),1.12,.96,height*.57,-.08+variant*.11,variant+1))
 if variant==0:
...
kit={'schema':'sidereal.native-planet-kit.v1','layout':'glacial-interior','authoringAxes':'BlenderZ-up local reusable volume forms; no visible quarterplates','materials':[{'name':n,'linearColor':list(c),'roughness':.82 if i==0 else .23} for i,(n,c) in enumerate(zip(names,colors))],'variants':[]}
