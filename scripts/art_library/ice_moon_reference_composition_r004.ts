...
export function composeIceMoonReference(
  kit: NativePlanetKit,
  seed: number,
  _lod: 0 | 1 | 2,
  diagnostic: boolean | "gorge" = false,
) {
  const recipe = (
    kit as NativePlanetKit & {
      compositionRecipe?: {
        ...
        layoutSeed: number;
        scales: number[];
        additionalRegion?: { normal: Vec; scale: number; angle: number };
      };
    }
  ).compositionRecipe;
  if (
    !recipe ||
    !["planets--ice-moon-1", "planets--ice-moon-2"].includes(
      recipe.referenceId,
    ) ||
    ![8, 10].includes(recipe.scales.length)
  )
    throw new Error("Explicit glacial moon recipe required");
  ...
  const variant = (name: string) => {
    ...
    if (!value) throw new Error(`Missing ice native variant ${name}`);
    return value;
  };
  function emit(
    name: string,
    ...
    if (
      ...
      throw new Error("Missing authored ice normals or UVs");
  ...
  const regions = diagnostic
    ? [
        {
          n: unit([0.452, 0.388, 0.794]),
          scale: 0.28,
          angle: 0,
          name: diagnostic === "gorge" ? "snow-open-gorge" : "snow-cut-region",
          id: "region-diagnostic",
        },
      ]
    : Array.from({ length: recipe.scales.length }, (_, i) => {
        const y = 1 - (2 * (i + 0.5)) / recipe.scales.length,
          r = Math.sqrt(1 - y * y),
          theta = phase + i * 2.39996323;
        return {
          n: [Math.cos(theta) * r, y, Math.sin(theta) * r] as Vec,
          scale: recipe.scales[i],
          angle: random() * Math.PI * 2,
          name: i % 2 ? "snow-open-gorge" : "snow-cut-region",
          id: `${recipe.referenceId}/region-${i}`,
        };
      });
  if (!diagnostic && recipe.additionalRegion) {
    const extra = recipe.additionalRegion;
    if (
      recipe.referenceId !== "planets--ice-moon-2" ||
      extra.normal.length !== 3 ||
      extra.normal.some((v) => !Number.isFinite(v)) ||
      !Number.isFinite(extra.scale) ||
      extra.scale < 0.05 ||
      extra.scale > 0.5 ||
      !Number.isFinite(extra.angle)
    )
      throw new Error("Invalid authored additional glacial region");
    regions.push({
      n: unit(extra.normal),
      scale: extra.scale,
      angle: extra.angle,
      name: "snow-open-gorge",
      id: `${recipe.referenceId}/blue-cut-hero`,
    });
  }
  ...
  function floorSampler(name: string) {
    ...
    const cells = new Map<number, FloorTriangle[]>(),
      cellSize = 0.12;
    let minX = Infinity,
      maxX = -Infinity,
      minY = Infinity,
    ...
    for (let i = 0; i < sourceRegion.indices.length; i += 3) {
      const [a, b, c] = [0, 1, 2].map(
        (k) =>
          sourceRegion.positions.slice(
            sourceRegion.indices[i + k] * 3,
            sourceRegion.indices[i + k] * 3 + 3,
          ) as Vec,
      );
      ...
      for (
        let x = Math.floor(Math.min(a[0], b[0], c[0]) / cellSize);
        x <= Math.floor(Math.max(a[0], b[0], c[0]) / cellSize);
        x++
      )
        for (
          let y = Math.floor(Math.min(a[1], b[1], c[1]) / cellSize);
          y <= Math.floor(Math.max(a[1], b[1], c[1]) / cellSize);
          y++
        ) {
          const key = x * 4096 + y;
    ...
    if ((maxY - minY) / cellSize >= 4094)
      throw new Error("Native floor index numeric cell stride exceeded");
    const floorAt = (x: number, y: number) => {
      if (
        ...
        return Infinity;
      let floor = Infinity;
      for (const { a, b, c, den } of cells.get(
        Math.floor(x / cellSize) * 4096 + Math.floor(y / cellSize),
      ) || []) {
        const u =
        ...
        if (u >= -1e-8 && v >= -1e-8 && w >= -1e-8)
          floor = Math.min(floor, a[2] * u + b[2] * v + c[2] * w);
      }
      return floor;
    };
    return floorAt;
  ...
  const samplers = new Map(
    [...new Set(regions.map((r) => r.name))].map((name) => [
      name,
      floorSampler(name),
    ]),
  );
  const groundSource = variant("ground-sphere");
  let groundRelief = 0;
  for (let i = 0; i < groundSource.positions.length; i += 3)
    groundRelief = Math.max(
      groundRelief,
      Math.hypot(
        groundSource.positions[i],
        groundSource.positions[i + 1],
        groundSource.positions[i + 2],
      ) - 1,
    );
  const substrate = (v: Vec): Vec => {
    ...
    for (const region of regions) {
      ...
      for (const [dx, dy] of stencil)
        floor = Math.min(floor, floorAt(x + dx, y + dy));
      if (Number.isFinite(floor))
        depression = Math.max(
          depression,
          Math.max(0, region.scale * (0.12 - floor) + groundRelief),
        );
    }
    const radius = Math.hypot(...v) - depression;
