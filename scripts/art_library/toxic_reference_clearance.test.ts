...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/toxic-r002/kit.json",
    "utf8",
  ),
);
it("NullEngine reproduces Toxic2 ground covering the basin and verifies native substrate clearance", () => {
  ...
  try {
    const n = new Vector3(0.452, 0.388, 0.794).normalize(),
      e = Vector3.Cross(Vector3.Up(), n).normalize(),
      north = Vector3.Cross(n, e),
      d = n
        .add(e.scale(-0.47 * 0.3))
        .add(north.scale(0.02 * 0.3))
        .normalize(),
      ray = new Ray(d.scale(2), d.negate(), 3);
    for (const clear of [false, true]) {
      const batches = toxicClearanceDiagnostic(kit, clear),
        meshes = batches.map((b, i) => {
          const mesh = new Mesh("toxic-clearance", scene);
          mesh.metadata = {
            role: "planet",
            materialRole: i,
            triangleRanges: b.ranges,
          };
          mesh.material = new PBRMaterial("toxic-native", scene);
      ...
      const part = hits[0].mesh.metadata.triangleRanges.find(
        (r: { firstTriangle: number; triangleCount: number }) =>
          hits[0].hit.faceId >= r.firstTriangle &&
          hits[0].hit.faceId < r.firstTriangle + r.triangleCount,
      )?.partId;
