...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ice-moon-2-r003/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
it("preserves every retained native variant and material from the frozen source", () => {
  const prior = JSON.parse(
    readFileSync(
      "output/playwright/planet-reference-20260914/ice-moon-2-r002/kit.json",
      "utf8",
    ),
  ) as NativePlanetKit;
  expect(kit.materials).toEqual(prior.materials);
  for (const variant of kit.variants)
    expect(JSON.stringify(variant)).toBe(
      JSON.stringify(prior.variants.find((v) => v.name === variant.name)),
    );
});
it("retains identical added-cut geometry and placement identities across all LODs", () => {
  ...
  expect(
    a.some((b) =>
      b.ranges.some((r) => r.partId === "planets--ice-moon-2/blue-cut-hero"),
    ),
  ).toBe(true);
  for (const lod of [1, 2] as const) {
    const b = composeIceMoonReference(kit, 38, lod);
    for (let i = 0; i < a.length; i++) {
      for (const field of ["positions", "normals", "uvs", "indices"] as const)
        expect(
          Buffer.from(a[i][field].buffer).equals(
            Buffer.from(b[i][field].buffer),
          ),
        ).toBe(true);
...
it("NullEngine resolves the exposed added glacial cut to its placement instead of the old white ground", () => {
  ...
  try {
    for (const b of composeIceMoonReference(kit, 38, 0)) {
      if (!b.indices.length) continue;
      const mesh = new Mesh("native", scene),
        v = new VertexData();
      v.positions = b.positions;
      v.normals = b.normals;
      v.uvs = b.uvs;
      v.indices = b.indices;
      v.applyToMesh(mesh);
    ...
    const normal = new Vector3(-0.86, 0.02, 0.51).normalize(),
      hit = scene.pickWithRay(new Ray(normal.scale(3), normal.negate(), 4));
    expect(hit?.hit).toBe(true);
    expect(hit!.faceId).toBeGreaterThanOrEqual(0);
    const range = hit!.pickedMesh!.metadata.trianglePlacementRanges.find(
