...
import { composeRockyReference } from "./rocky_reference_composition_r008";
import type { NativePlanetKit } from "../../packages/render/src/environment/native-planet-composition";
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/rocky-r008/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
it("retains complete native regional surfaces UVs normals and identities at every LOD", () => {
  const a = composeRockyReference(kit, 38, 0),
    b = composeRockyReference(kit, 38, 2);
  ...
  expect(a.reduce((n, b) => n + b.indices.length / 3, 0)).toBeLessThan(150000);
});
it("NullEngine sees embedded deep crater floor before the cleared native substrate", () => {
  ...
  try {
    const meshes = composeRockyReference(kit, 38, 0, true).map((b, i) => {
