...
for (const id of ids) {
  const kit = JSON.parse(
    readFileSync(
      `output/playwright/planet-reference-20260914/${id}-r004/kit.json`,
      "utf8",
    ),
  ) as NativePlanetKit;
...
it("exposes the authored hot fracture surfaces above final overlapping crust", () => {
  ...
  writeFileSync(
    "output/playwright/planet-reference-20260914/volcanic-moons-r004-exposed-fracture-rays.json",
...
it("preserves the complete cold foundation, placement recipe and native material definitions", () => {
  for (const id of ids) {
    ...
    expect(next.compositionRecipe).toEqual({
      ...old.compositionRecipe,
      warmEdgeRegions: [2, 9],
    });
    ...
    for (const material of old.materials)
      for (const field of ["baseColorTexture", "metallicRoughnessTexture"])
        if (material[field])
          expect(
            readFileSync(
              `output/playwright/planet-reference-20260914/${id}-r004/${material[field]}`,
            ).equals(
              readFileSync(
...
it("limits changed native placement geometry to the two explicit warm-edge region IDs", async () => {
  const { composeVolcanicMoonReference: previous } =
      await import("./volcanic_moon_reference_composition_r003"),
    old = JSON.parse(
      readFileSync(
        "output/playwright/planet-reference-20260914/volcanic-moon-2-r003/kit.json",
        "utf8",
      ),
    ),
    next = JSON.parse(
      readFileSync(
        "output/playwright/planet-reference-20260914/volcanic-moon-2-r004/kit.json",
        "utf8",
      ),
    );
  const a = previous(old, 38, 0),
    b = composeVolcanicMoonReference(next, 38, 0);
  let changed = new Set<string>();
  for (let role = 0; role < a.length; role++) {
    expect(a[role].ranges.map((r) => r.partId)).toEqual(
      b[role].ranges.map((r) => r.partId),
    );
    for (const range of a[role].ranges) {
      const other = b[role].ranges.find((r) => r.partId === range.partId)!;
      let equal = range.triangleCount === other.triangleCount;
      for (const field of ["positions", "normals", "uvs"] as const) {
        const stride = field === "uvs" ? 6 : 9;
        const oldBytes = a[role][field].slice(
            range.firstTriangle * stride,
            (range.firstTriangle + range.triangleCount) * stride,
          ),
          newBytes = b[role][field].slice(
            other.firstTriangle * stride,
            (other.firstTriangle + other.triangleCount) * stride,
          );
        equal =
          equal &&
          Buffer.from(oldBytes.buffer).equals(Buffer.from(newBytes.buffer));
      }
      if (!equal) changed.add(range.partId);
    }
  }
  expect([...changed].sort()).toEqual([
    "planets--volcanic-moon-2/crust-2",
    "planets--volcanic-moon-2/crust-9",
  ]);
});
