...
export async function createReferenceCandidate(scene:Scene,seed:number){
 ...
 const cache=createPlanetLODCache(async lod=>{
  const worker=new Worker(new URL('./planet_reference_worker.ts',import.meta.url),{type:'module'});
  const result=await new Promise<{batches:ReturnType<typeof composeDesertReference>;buildMs:number;smoke?:PackedPlanetGeometry;weather?:PlanetCloudGeometry & {uvs?:number[];ranges?:{firstTriangle:number;triangleCount:number;partId:string}[]}}>((resolve,reject)=>{worker.onmessage=e=>e.data.error?reject(new Error(e.data.error)):resolve(e.data);worker.onerror=e=>reject(new Error(e.message));worker.postMessage({kit,cloudKit,seed,lod,recipe,unit:new URLSearchParams(location.search).has("unit")});}).finally(()=>worker.terminate());
  ...
  for(const [index,batch] of result.batches.entries()){
   if(!batch.indices.length)continue;await nextFrame();
   ...
   await nextFrame();await mesh.material.forceCompilationAsync(mesh);
  }
  if(result.weather?.indices.length){
   await nextFrame();const mesh=new Mesh('reference-weather',scene),data=new VertexData();Object.assign(data,result.weather);data.applyToMesh(mesh);mesh.sideOrientation=Material.CounterClockWiseSideOrientation;mesh.material=weatherMaterial;mesh.parent=node;mesh.isPickable=false;mesh.metadata={role:'planet',style:recipe.style,planetWeather:true,trianglePlacementRanges:(result.weather.ranges??[{firstTriangle:0,triangleCount:result.weather.indices.length/3,partId:"weather"}]).map(r=>({...r,partId:`reference-review:${seed}:${r.partId}`}))};await nextFrame();await weatherMaterial.forceCompilationAsync(mesh);
  }
  if(result.smoke){await nextFrame();const smoke=smokeRuntime.attach(node,lod,result.smoke);if(smoke){await nextFrame();await smoke.material!.forceCompilationAsync(smoke);}}
  return {root:node};
 },v=>v.root.dispose(false,false));
 return {root,update(projected:number){previous=planetLOD(projected,previous);cache.update(fixedDetail?0:previous,projected);},stats:()=>({...cache.snapshot(),...smokeRuntime.stats(),requestedLOD:previous,buildMs,localLights:localLights?.lights.length??0}),dispose(){smokeRuntime.dispose();localLights?.dispose();cache.dispose();root.dispose(false,false);materials.forEach(m=>m.dispose());weatherMaterial.dispose();for(const pending of texturePromises.values())void pending.then(texture=>texture.dispose());}};
}
