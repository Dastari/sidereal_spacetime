...
window.study = {
  ...
  async load(slug) {
    current?.dispose();
    current = await SceneLoader.LoadAssetContainerAsync(
      "/__hull-study/panels/",
      slug + ".glb",
      scene,
    );
    current.addAllToScene();
    ...
    return {
      ...
      normalMapped: materials.filter((m) => m.bumpTexture).length,
      textureSizes: current.textures.map((t) => ({
        name: t.name,
        size: t.getSize(),
      })),
      webgl: engine.webGLVersion,
      drawCalls: engine._drawCalls?.current,
      geometry:
        "Actual exported study GLB in Babylon; isolated browser route, not live ship",
    };
  },
  ready: true,
};
