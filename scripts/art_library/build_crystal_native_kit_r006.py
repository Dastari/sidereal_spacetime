...
Run: blender -b --python scripts/art_library/build_crystal_native_kit_r006.py -- NEW_OUTPUT
"""
...
def make(name,verts,faces,roles):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=bm.faces);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-crystal-r006';return o
...
for role in [0,1,2,3,4,5,6,7]:
 rock=role<3;size=256;image=bpy.data.images.new('Crystal6-material-'+str(role),width=size,height=size,alpha=True);pixels=[]
 roughimage=bpy.data.images.new('Crystal6-roughness-'+str(role),width=size,height=size,alpha=True) if rock else None;roughpixels=[]
