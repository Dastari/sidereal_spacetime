...
export function composeToxicFog(kit:NativePlanetKit,seed:number,coverage:number,phase=0,anchors:readonly ToxicFogAnchor[]=[]){
 ...
 for(let index=0;index<count;index++){
  ...
  const scale=anchor.scale??(.13+random()*.065),rotation=random()*Math.PI*2,radius=Math.hypot(...anchor.position),place=tangent(unit(anchor.position),scale,rotation,radius-1,true,name==='toxic-fog-vent-plume'?.105:.060);
