...
Run: blender -b --python scripts/art_library/build_volcanic_native_kit_r023.py -- NEW_OUTPUT_DIR
...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-volcanic-r023'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=2):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-volcanic-r023'
    return obj
...
for variant in range(3):
 ...
 def channel(slug,points,width,top,role):
  ...
  obj=cliff_solid(name+'-'+slug,poly,top,bottom=-.27,cap=role)
  if role==3:obj['lavaPath']=json.dumps(points);obj['lavaWidth']=width
  parts.append(obj)
 ...
 # Two smaller connected native tributaries divide the largest quiet banks.
 for branch_id,route in enumerate([[(-.95,-.08),(-.87,.15),(-1.0,.39),(-.89,.65),(-1.02,.87)],[(-.46,-.10),(-.52,-.34),(-.38,-.55),(-.59,-.73),(-.53,-.90)]]):
  upper=[];lower=[]
  for k,(x,y) in enumerate(route):
   a,b=route[max(0,k-1)],route[min(len(route)-1,k+1)];dx,dy=b[0]-a[0],b[1]-a[1];length=math.hypot(dx,dy);w=[.065,.048,.075,.054,.04][k]
   upper.append((x-dy/length*w,y+dx/length*w));lower.append((x+dy/length*w,y-dx/length*w))
  cutter=cliff_solid(name+'-temporary-tributary-cut',lower+list(reversed(upper)),.65,bottom=-.24,cap=1)
  for target in [p for p in parts if 'quiet-basalt-plateau' in p.name]:
   bpy.context.view_layer.objects.active=target;mod=target.modifiers.new('Native branching cooling fracture','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=cutter;bpy.ops.object.modifier_apply(modifier=mod.name)
  bpy.data.objects.remove(cutter,do_unlink=True)
  channel('small-connected-tributary-'+str(branch_id),route,.041,.033,3)
 ...
 for j,(x,y,w,d,h) in enumerate([(-1.38,1.02,.67,.44,.19),(-.72,1.13,.54,.39,.17),(.93,1.09,.73,.46,.23),(-1.13,-1.10,.52,.37,.16),(-.28,-1.15,.77,.38,.18),(.85,-1.06,.64,.37,.17)]):
  ...
  parts.append(obj)
 # Authored small fracture relief seated on broad plateau surfaces. Fixed source
 # geometry survives every runtime LOD. Unequal chips leave quiet areas between.
 def inside(px,py,poly):
  odd=False
  for k,(ax,ay) in enumerate(poly):
   bx,by=poly[(k+1)%len(poly)]
   if (ay>py)!=(by>py) and px<(bx-ax)*(py-ay)/(by-ay)+ax:odd=not odd
  return odd
 rng=random.Random(2100+variant)
 for k in range(165):
  x,y=rng.uniform(-1.48,1.48),rng.uniform(-.9,.91)
  district=next((j for j,poly in enumerate([west,east,south]) if inside(x,y,poly)),None)
  if district is None:continue
  if variant!=2 and math.hypot(x-cx,y-cy)<.32:continue
  z=[.34,.26,.30][(district+variant)%3]
  w=rng.uniform(.055,.18);d=rng.uniform(.045,.14);h=rng.uniform(.016,.065)
  # Native low angular flakes, distinct from the sparse medium buttresses.
  poly=[(x-w*.5,y-d*.5),(x+w*.36,y-d*.5),(x+w*.52,y-d*.13),(x+w*.45,y+d*.48),(x-w*.48,y+d*.5)]
  if not all(inside(px,py,[west,east,south][district])for px,py in poly):continue
  parts.append(cliff_solid(name+'-fine-cooling-flake-'+str(k),poly,z+h,bottom=z-.012,cap=2 if k%4 else 1))
...
for role in [0,1,2,4]:
 image=bpy.data.images.new('Basalt23-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('Basalt23-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];roughpixels=[]
 for y in range(256):
  for x in range(256):
   ...
   rgb=[c*max(.3,grain-fracture) for c in palette[role][1]];rgb=[12.92*c if c<=.0031308 else 1.055*c**(1/2.4)-.055 for c in rgb];pixels.extend((*rgb,1));roughpixels.extend((1,.64+.24*grain,.18 if role<3 else 0,1))
...
# Native PBR lava flow texture: integrated narrow hot current, orange body,
# dark cooling fringe. UV follows the authored channel rather than world axes.
image=bpy.data.images.new('Lava23-flow',width=256,height=256,alpha=True);pixels=[]
for y in range(256):
 for x in range(256):
  u=x/255;v=y/255;center=.5+.09*math.sin(u*19)+.035*math.sin(u*51)
  distance=abs(v-center);core=math.exp(-(distance/.075)**2)*(.65+.35*math.sin(u*13)**2)
  edge=max(0,min(1,(.48-abs(v-.5))/.13))
  linear=[(.8+.2*core)*edge+.055*(1-edge),(.075+.62*core)*edge+.003*(1-edge),(.001+.13*core)*edge]
  rgb=[12.92*c if c<=.0031308 else 1.055*c**(1/2.4)-.055 for c in linear];pixels.extend((*rgb,1))
image.pixels.foreach_set(pixels);image.filepath_raw=str(out/'lava-flow.png');image.file_format='PNG';image.save();image.pack()
shader=materials[3].node_tree.nodes.get('Principled BSDF');tex=materials[3].node_tree.nodes.new('ShaderNodeTexImage');tex.image=image
materials[3].node_tree.links.new(tex.outputs['Color'],shader.inputs['Base Color']);materials[3].node_tree.links.new(tex.outputs['Color'],shader.inputs['Emission Color']);shader.inputs['Emission Strength'].default_value=4
kit['materials'][3].update(linearColor=[1,1,1],baseColorTexture='lava-flow.png',emissiveTexture='lava-flow.png',emissiveColor=[1,1,1],emissiveStrength=4,textureColorSpace='sRGB',invertY=False)
    ...
    for obj in parts:
        ...
        for p in mesh.polygons:
            ...
            for loop in p.loop_indices:
                q=mesh.vertices[mesh.loops[loop].vertex_index].co
                if 'lavaPath' in obj and p.material_index==3:
                    points=json.loads(obj['lavaPath']);best=(1e9,0,0);travel=0
                    for k in range(len(points)-1):
                        a,b=points[k],points[k+1];dx,dy=b[0]-a[0],b[1]-a[1];length=math.hypot(dx,dy);t=max(0,min(1,((q.x-a[0])*dx+(q.y-a[1])*dy)/(length*length)));px,py=a[0]+t*dx,a[1]+t*dy;distance=math.hypot(q.x-px,q.y-py)
                        if distance<best[0]:best=(distance,travel+t*length,((q.x-px)*(-dy)+(q.y-py)*dx)/length)
                        travel+=length
                    uv.data[loop].uv=(best[1]/max(travel,.001),.5+best[2]/(obj['lavaWidth']*2.6))
                else:uv.data[loop].uv=(q[axes[0]]*.45+.5,q[axes[1]]*.45+.5)
