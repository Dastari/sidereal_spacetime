...
for variant in range(5):
 ...
 cutters=[]
 if variant:
  # Different connected authored cavity footprints, no whole-sphere voxel remesh.
  centres=[(0,0)] if variant==1 else [(-.12,-.1),(.1,.08)] if variant==2 else [(-.12,.12),(.1,.08),(.12,-.13)] if variant==3 else [(-.12,-.13),(.12,.13)]
  for k,(cx,cy) in enumerate(centres):
   for layer,(width,depth) in enumerate([(.62,.045),(.49,.14),(.35,.255)]):
    wx=width if len(centres)==1 else width*.68;wy=width*.84 if variant!=2 else width*.55
    cut=cube('CUT-kit-%d-%d-%d'%(variant,k,layer),offset+Vector((cx,cy,(.15-depth)/2)),(wx,wy,.15+depth))
    bpy.context.view_layer.objects.active=body;mod=body.modifiers.new('Native stepped cavity','BOOLEAN');mod.operation='DIFFERENCE';mod.object=cut;mod.solver='EXACT';bpy.ops.object.modifier_apply(modifier=mod.name);cut.hide_render=True;cut.hide_set(True);cutters.append(cut)
...
kit={'schema':'sidereal.native-planet-kit.v1','authoringAxes':'Blender Z-up; local domain x/y[-.5,.5], topz0','materials':[{'name':m.name,'linearColor':list(c),'roughness':.78 if i==0 else .2 if i in [2,3] else .45} for i,(m,c) in enumerate(zip(materials,colors))],'variants':[]}
for index,objects in enumerate(variants):
 ...
 for ob in objects:
  ...
  ev.to_mesh_clear()
 kit['variants'].append({'name':['quiet-crust','deep-shaft','long-ravine','bent-ravine','broken-ice-field'][index],'positions':vs,'normals':ns,'indices':indices,'triangleMaterials':roles})
