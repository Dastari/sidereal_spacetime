...
Run: blender -b --python scripts/art_library/build_crystal_native_kit_r008.py -- NEW_OUTPUT
"""
...
def make(name,verts,faces,roles):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=bm.faces);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-crystal-r008';return o
...
rocky_path=Path('output/playwright/planet-reference-20260914/rocky-r009/kit.json')
rocky=json.loads(rocky_path.read_text());role_map=[0,2,1,0,8,2]
for source_name in ['battered-region-a','battered-region-b']:
 before=set(bpy.data.objects);bpy.ops.import_scene.gltf(filepath=str(rocky_path.parent/(source_name+'.glb')))
 imported=[o for o in bpy.data.objects if o not in before and o.type=='MESH'];parts=[]
 for obj in imported:
  mesh=obj.data;mesh.transform(obj.matrix_world);obj.parent=None;obj.matrix_world.identity()
  source_roles=[next(i for i,m in enumerate(rocky['materials'])if slot.name.split('.')[0]==m['name'])for slot in mesh.materials]
  roles=[role_map[source_roles[p.material_index]]for p in mesh.polygons]
  mesh.materials.clear()
  for material in materials:mesh.materials.append(material)
  for polygon,role in zip(mesh.polygons,roles):polygon.material_index=role
  obj.name='GEO-crystal-'+obj.name;obj['role']='planet';obj['authoring']='native-crystal-r008';parts.append(obj)
...
for role in [0,1,2,3,4,5,6,7]:
 rock=role<3;size=256;image=bpy.data.images.new('Crystal8-material-'+str(role),width=size,height=size,alpha=True);pixels=[]
 roughimage=bpy.data.images.new('Crystal8-roughness-'+str(role),width=size,height=size,alpha=True) if rock else None;roughpixels=[]
