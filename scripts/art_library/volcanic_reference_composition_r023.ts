...
export function composeVolcanicReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 const random=seeded(seed),phase=random()*Math.PI*2,count=diagnostic?1:18;
 for(let i=0;i<count;i++){
  const anchor:Vec=diagnostic?[.452,.388,.794]:globeAnchor(i,count,phase),scale=diagnostic?.26:[.32,.29,.35,.31,.30,.34][i%6],angle=diagnostic?0:random()*Math.PI*2;
