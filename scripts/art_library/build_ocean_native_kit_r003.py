...
Run: blender -b --python scripts/art_library/build_ocean_native_kit_r003.py -- NEW_OUTPUT
"""
...
def objmesh(name,verts,faces,roles):
 ...
 o['role']='planet';o['authoring']='native-ocean-r003';return o
...
def island(name,x,y,sx,sy,h,phase=0):
 ...
 for j,(scale,z) in enumerate(profile):
  for i,(px,py) in enumerate(outline):
   ...
   width=[.4,.5,1.0,1.5,1.3,.7,.14,.3,.75,1.4,1.2,.6,.2,.5,.3][i]
