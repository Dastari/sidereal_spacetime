...
root=Path(__file__).resolve().parents[2];out=root/'.runtime/art-library/hull/r002'
...
subprocess.run([config['art']['blender'],'--background','--threads','8','--python-exit-code','1','--python',str(root/'scripts/art_library/build_pilot_r002.py'),'--',str(out)],cwd=root,check=True)
