...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-toxic-r005'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-toxic-r005'
    return obj
...
for variant in range(3):
 ...
 for index,footprint in enumerate(basins+channels):
  ...
  if index<2:
   ...
   hole=cliff_solid(name+'-wet-transition-clearance',footprint,1.2,bottom=-.16,cap=3)
   bpy.context.view_layer.objects.active=wet;mod=wet.modifiers.new('Keep wet transition outside open basin','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=hole;bpy.ops.object.modifier_apply(modifier=mod.name);bpy.data.objects.remove(hole,do_unlink=True);parts.append(wet)
...
for role in [0,1,2,3,4,5]:
 ...
 for kind in ['albedo','normal','orm']:
  image=bpy.data.images.new(f'Toxic5-corrosion-{role}-{kind}',width=size,height=size,alpha=True);image.colorspace_settings.name='sRGB'if kind=='albedo'else'Non-Color';pixels=np.ones((size,size,4),dtype=np.float32)
  if kind=='albedo':
