...
Run: blender -b --python scripts/art_library/build_rocky_native_kit_r003.py -- NEW_OUTPUT
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-rocky-r003'
 return obj
...
def battered_region(name,phase):
 ...
 for j in range(len(profile)):
  for i in range(n):
   faces.append((j*n+i,j*n+(i+1)%n,(j+1)*n+(i+1)%n,(j+1)*n+i))
   roles.append(2 if j<3 else (1 if j in (3,4) else (4 if j==9 and 18<=i<=21 else (3 if (j==7 and 1<=i<=11) or (j==10 and 16<=i<=27) or (j==12 and 32<=i<=43) else 0))))
 ...
 return obj
forms.append(('battered-region-a',[battered_region('battered-region-a',0)]))
forms.append(('battered-region-b',[battered_region('battered-region-b',1.9)]))
