...
import {composeRockyReference} from './rocky_reference_composition_r005';
import type {NativePlanetKit} from '../../packages/render/src/environment/native-planet-composition';
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/rocky-r005/kit.json','utf8')) as NativePlanetKit;
...
it('retains exact major crater geometry, material roles and placement identity at each LOD',()=>{
 ...
 expect(new Set(levels[0].flatMap(b=>b.ranges.filter(r=>r.partId.startsWith('crater-hero')).map(r=>r.partId))).size).toBe(12);
...
it('excavates the native substrate and lowers crater floors beneath the general surface',()=>{
 ...
 for(const r of dark.ranges.filter(r=>r.partId.startsWith('crater-hero'))){
...
it('clears the single diagnostic bowl floor with the actual triangulated substrate',()=>{
 ...
 const ground=hits(level[0],'ground'),floor=hits(level[2],'crater-diagnostic');
 expect(ground).toBeGreaterThan(.88);expect(floor).toBeGreaterThan(ground+.003);expect(floor).toBeLessThan(.97);
 expect(new Set(level.flatMap(b=>b.ranges.map(r=>r.partId)))).toEqual(new Set(['ground','crater-diagnostic']));
});
