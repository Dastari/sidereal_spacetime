...
if (!["whole", "bare", "armor", "exploded"].includes(mode))
  throw Error("Unknown review mode");
...
if (mode !== "armor") {
  ...
  if (mode !== "whole") {
    ...
    for (const mesh of scene.meshes)
      if (mesh.metadata?.role === "hull") mesh.setEnabled(false);
  }
}
  ...
  for (const part of layout.parts) {
    ...
    if (!library) {
      const bytes = new Uint8Array(
        await (await fetch(part.visual.url)).arrayBuffer(),
      );
      if (constructionHash(bytes) !== part.visual.sha256)
        throw Error("Review native hash mismatch");
      ...
      library = container.meshes.filter(
    ...
    const selected = library.filter((m) =>
      nativeMeshInGroup(m.name, part.armorGroup),
    );
    if (!selected.length)
      throw Error("Missing native armor group: " + part.slug);
...
const views = {
  concept: [-2.75, 0.955, 0, 1, -1, 24],
  starboard: [-0.39, 0.955, 0, 1, -1, 24],
  gameplay: [-2.34, 0.955, 0, 1, -1, 27],
  bow: [-2.12, 1.02, 0, 1, -8, 11],
  side: [-Math.PI, 1.24, -5, 1.6, -1, 24],
  joined: [-2.9, 1.12, -5, 1.5, -1, 9],
  rear: [2.34, 1.05, 0, 1, 4, 28],
  top: [-Math.PI / 2, 0.025, 0, 0, 0, 28],
};
function metrics() {
  return {
