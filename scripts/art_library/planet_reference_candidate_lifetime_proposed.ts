import {createReferenceWorkerLifetime,createReferenceUploadLifetime,referenceFixedDetail} from './planet_reference_worker_lifetime_proposed';
...
export async function createReferenceCandidate(scene:Scene,seed:number){
 ...
 const fixedDetail=referenceFixedDetail(String(kit.layout),new URLSearchParams(location.search).get('kit'));
