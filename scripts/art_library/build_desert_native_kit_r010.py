...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r010'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-desert-r010'
    return obj
...
for variant in range(3):
 ...
 specs=[(-.18,.06,.18,.21,.84),(.02,.11,.19,.20,1.02),(.19,.07,.16,.20,.73),
      (-.23,-.12,.16,.19,.62),(-.05,-.09,.17,.18,.79),(.14,-.12,.16,.17,.63),
      (-.14,-.25,.14,.15,.56),(.05,-.24,.15,.17,.67)]
