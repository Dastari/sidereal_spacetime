...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ice-r026/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("changes only selected tall shard side material roles, keeping every authored surface and snow cap intact", () => {
  ...
  expect(kit.materials.slice(0, 5)).toEqual(old.materials);
  let glass = 0,
    snow = 0;
  for (let i = 0; i < old.variants.length; i++) {
    const before = old.variants[i],
      after = kit.variants[i];
    for (const field of ["positions", "normals", "uvs", "indices"])
      expect((after as any)[field]).toEqual(before[field]);
    for (let t = 0; t < before.triangleMaterials.length; t++) {
      const role = after.triangleMaterials[t];
      if (before.triangleMaterials[t] === 0) {
        expect(role).toBe(0);
        snow++;
      }
      if (role >= 5) {
        expect([2, 3, 4]).toContain(before.triangleMaterials[t]);
        glass++;
      }
      if (before.name.startsWith("ground"))
        expect(role).toBe(before.triangleMaterials[t]);
    }
  }
  expect(glass).toBeGreaterThan(100);
  expect(glass).toBeLessThan(2500);
  expect(snow).toBeGreaterThan(5000);
});
it("exports actual native glTF transmission IOR and volume with opaque caps, not mesh-wide alpha", () => {
  ...
  for (const name of ["snow-cut-region", "snow-open-gorge"])
    for (const prefix of ["", "blender-raw/"]) {
      const g = parse(
        `output/playwright/planet-reference-20260914/ice-r026/${prefix}${name}.glb`,
      );
      for (const role of [5, 6]) {
        const def = kit.materials[role] as any,
          m = g.materials.find((m: any) => m.name === def.name),
          e = m.extensions;
        expect(e.KHR_materials_transmission.transmissionFactor).toBeCloseTo(
          def.transmissionFactor,
          6,
        );
        expect(e.KHR_materials_ior.ior).toBeCloseTo(1.31, 6);
        for (const field of ["thicknessFactor", "attenuationDistance"])
          expect(e.KHR_materials_volume[field]).toBeCloseTo(def[field], 6);
        for (let c = 0; c < 3; c++)
          expect(e.KHR_materials_volume.attenuationColor[c]).toBeCloseTo(
            def.attenuationColor[c],
            6,
          );
        expect(m.alphaMode ?? "OPAQUE").toBe("OPAQUE");
      }
      const snow = g.materials.find((m: any) => m.name === "powder-snow");
      expect(snow.extensions?.KHR_materials_transmission).toBeUndefined();
    }
});
