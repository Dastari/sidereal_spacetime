...
import { composeVolcanicReference } from "./volcanic_reference_composition_r020";
...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/volcanic-r020/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
it("retains every regional surface and placement at all LODs within a bounded triangle budget", () => {
  const levels = ([0, 1, 2] as const).map((lod) =>
    composeVolcanicReference(kit, 38, lod),
  );
  const geometry = (batch: (typeof levels)[number][number]) =>
    batch.ranges
      .filter((r) => r.partId !== "volcanic-ground")
      .map((r) => ({
        id: r.partId,
  ...
  for (let m = 0; m < kit.materials.length; m++) {
    expect(geometry(levels[1][m])).toEqual(geometry(levels[0][m]));
    expect(geometry(levels[2][m])).toEqual(geometry(levels[0][m]));
  }
  expect(levels[0].reduce((n, b) => n + b.indices.length / 3, 0)).toBeLessThan(
    153000,
  );
  expect(levels[2].reduce((n, b) => n + b.indices.length / 3, 0)).toBeLessThan(
    76000,
  );
...
it("NullEngine rays see recessed native lava before the substrate at every LOD", () => {
  ...
  try {
    for (const lod of [0, 1, 2] as const) {
      const meshes = composeVolcanicReference(kit, 38, lod, true)
        .map((b, i) => {
          const mesh = new Mesh(`volcanic-${i}`, scene);
          mesh.metadata = {
            role: "planet",
            materialRole: i,
            triangleRanges: b.ranges,
          };
          mesh.material = new PBRMaterial(`volcanic-role-${i}`, scene);
      ...
      const transform = tangent([0.452, 0.388, 0.794], 0.26, 0, 0, true, 0.26);
      for (const point of [
        [0.19, 0.39, 0],
        [0.12, 0.59, 0],
        [0.22, 0.81, 0],
      ] as [number, number, number][]) {
        const d = Vector3.FromArray(transform(point)).normalize(),
          ray = new Ray(d.scale(2), d.scale(-1), 3),
          hits = meshes
        ...
        const substrate = hits.find((v) => v.mesh.metadata.materialRole === 0)!;
        expect(substrate.hit.distance - hits[0].hit.distance).toBeGreaterThan(
          0.005,
        );
      }
...
it("rejects missing native forms and invalid numeric input", () => {
  expect(() =>
    composeVolcanicReference({ ...kit, variants: [] }, 38, 0),
  ).toThrow("Missing volcanic native variant");
  expect(() => composeVolcanicReference(kit, NaN, 0)).toThrow(
    "Invalid volcanic composition",
  );
});
