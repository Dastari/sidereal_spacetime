...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/volcanic-r020/kit.json','utf8')) as NativePlanetKit;
it('retains every regional surface and placement at all LODs within a bounded triangle budget',()=>{
 ...
 expect(levels[0].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(153000);
 expect(levels[2].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(76000);
