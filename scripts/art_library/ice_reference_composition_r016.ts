...
export function composeIceReference(
  ...
  if (diagnostic)
    ...
    for (let i = 0; i < 10; i++) {
      ...
      regions.push({
        ...
        scale: [0.28, 0.24, 0.27, 0.23, 0.29, 0.25, 0.26, 0.24, 0.28, 0.23][i],
        angle: random() * Math.PI * 2,
        name: i % 2 ? "snow-shaft-region-b" : "snow-shaft-region-a",
        id: `region-${i}`,
      });
    }
  const substrate = (v: Vec): Vec => {
    ...
    for (const region of regions) {
      const distance =
        ...
        region.scale;
      const t = Math.max(0, Math.min(1, (distance - 0.49) / 0.34));
      depression = Math.max(
        depression,
        region.scale * 0.78 * (1 - t * t * (3 - 2 * t)),
      );
    }
    return [
      n[0] * (1 - depression),
  ...
  for (const region of regions) {
    ...
    if (!diagnostic)
      for (let j = 0; j < 2; j++) {
        const localX = (j ? -0.43 : 0.46) * region.scale,
          localY = (j ? 0.12 : -0.08) * region.scale,
          up = unit([
            n[0] + east[0] * localX + north[0] * localY,
            n[1] + east[1] * localX + north[1] * localY,
            n[2] + east[2] * localX + north[2] * localY,
          ]),
          e = unit(cross(Math.abs(up[1]) > 0.94 ? [1, 0, 0] : [0, 1, 0], up)),
          nn = cross(up, e),
          scale = j ? 0.055 : 0.07;
        emit("glacial-buttress-group", `${region.id}-bluewall-${j}`, (v) => [
          up[0] * (1.012 + v[2] * scale) +
            e[0] * v[0] * scale +
            nn[0] * v[1] * scale,
          up[1] * (1.012 + v[2] * scale) +
            e[1] * v[0] * scale +
            nn[1] * v[1] * scale,
          up[2] * (1.012 + v[2] * scale) +
            e[2] * v[0] * scale +
            nn[2] * v[1] * scale,
        ]);
      }
