...
export function composeIceReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 if(diagnostic)regions.push({n:unit([.452,.388,.794]),scale:.28,angle:0,name:'snow-shaft-region-a',id:'region-diagnostic'});
 else for(let i=0;i<10;i++){
  const y=1-2*(i+.5)/10,angle=phase+i*2.39996323,r=Math.sqrt(1-y*y);
  regions.push({n:[Math.cos(angle)*r,y,Math.sin(angle)*r],scale:[.28,.24,.27,.23,.29,.25,.26,.24,.28,.23][i],angle:random()*Math.PI*2,name:i%2?'snow-shaft-region-b':'snow-shaft-region-a',id:`region-${i}`});
 }
 const substrate=(v:Vec):Vec=>{
  ...
  for(const region of regions){
   ...
   const t=Math.max(0,Math.min(1,(distance-.49)/.34));
   depression=Math.max(depression,region.scale*.78*(1-t*t*(3-2*t)));
  }
 ...
 for(const region of regions){
  ...
  if(!diagnostic)for(let j=0;j<2;j++){
   const localX=(j?-.43:.46)*region.scale,localY=(j?.12:-.08)*region.scale,up=unit([n[0]+east[0]*localX+north[0]*localY,n[1]+east[1]*localX+north[1]*localY,n[2]+east[2]*localX+north[2]*localY]),e=unit(cross(Math.abs(up[1])>.94?[1,0,0]:[0,1,0],up)),nn=cross(up,e),scale=j?.055:.070;
   emit('glacial-buttress-group',`${region.id}-bluewall-${j}`,v=>[up[0]*(1.012+v[2]*scale)+e[0]*v[0]*scale+nn[0]*v[1]*scale,up[1]*(1.012+v[2]*scale)+e[1]*v[0]*scale+nn[1]*v[1]*scale,up[2]*(1.012+v[2]*scale)+e[2]*v[0]*scale+nn[2]*v[1]*scale]);
  }
