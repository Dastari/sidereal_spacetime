...
export function composeCrystalMoonReference(
  ...
  for (let i = 0; i < source.indices.length; i += 3) {
    ...
    const part = parts.find(
      (p) =>
        i / 3 >= p.firstTriangle && i / 3 < p.firstTriangle + p.triangleCount,
    );
    if (!part) throw new Error("Native triangle part missing");
    const last = batch.ranges.at(-1);
    if (last?.partId === part.partId) last.triangleCount++;
    else
      batch.ranges.push({
        partId: part.partId,
        firstTriangle: batch.indices.length / 3,
        triangleCount: 1,
      });
    for (let k = 0; k < 3; k++) {
