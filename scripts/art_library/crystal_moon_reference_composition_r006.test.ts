...
for (const id of ["crystal-moon-1", "crystal-moon-2"])
  it(`${id} preserves exact parent surfaces and identifies new native parts at every LOD`, () => {
    const path = `output/playwright/planet-reference-20260914/${id}-r006/`,
      kit = JSON.parse(readFileSync(path + "kit.json", "utf8")),
      prior = JSON.parse(
        readFileSync(path.replace("r006", "r005") + "kit.json", "utf8"),
    ...
    for (const channel of [
      ...
      expect(b[channel].slice(0, a[channel].length)).toEqual(a[channel]);
    const parts = kit.nativePartRanges;
    expect(parts[0].triangleCount).toBe(a.indices.length / 3);
    ...
    for (const seed of [38, 117, 904]) {
      ...
      const seen = new Set(
        levels[0].flatMap((b) => b.ranges.map((r) => r.partId)),
      );
      expect(seen).toEqual(new Set(parts.map((p: any) => p.partId)));
    }
  });
