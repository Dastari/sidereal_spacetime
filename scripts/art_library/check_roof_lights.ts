...
const out=process.argv[2]??'.runtime/art-library/roof/r004',c:PartCatalog=JSON.parse(readFileSync(out+'/ship-catalog.json','utf8')),d:AssemblyDocument=JSON.parse(readFileSync(out+'/ship-wayfarer.json','utf8'));
const e=new NullEngine(),s=new Scene(e),checks=[];
for(const p of d.parts){const a=c.assets.find(a=>a.id===p.assetId)!;if(a.category!=='roof'||!a.lights?.length)continue;const parent=new TransformNode(p.id,s),rig=createEquipmentLighting(s,parent,a.lights);checks.push({placement:p.id,described:a.lights.length,created:rig.lights.length,bounded:rig.lights.every(l=>l.range<=3&&l.intensity<=2),positions:rig.lights.map(l=>l.position.asArray()),directions:rig.lights.map(l=>l.direction.asArray())});rig.dispose();parent.dispose();}
