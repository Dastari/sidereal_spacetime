...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-toxic-r007'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-toxic-r007'
    return obj
...
for variant in range(3):
 ...
 crust=cliff_solid(name+'-continuous-corroded-crust',footprint,.11+variant*.020,bottom=-.30,cap=0)
 parts=[crust]
 ...
 # Unequal native edge breaks avoid engineered hexagonal pool silhouettes.
 def weather_edge(poly, strength):
  result=[]
  for i,(x,y) in enumerate(poly):
   nx,ny=poly[(i+1)%len(poly)];dx,dy=nx-x,ny-y;length=math.hypot(dx,dy);bend=strength*[.7,-.45,1,-.6,.4,-.8][i%6]
   result.extend([(x,y),(x+dx*.38-dy/length*bend,y+dy*.38+dx/length*bend),(x+dx*.73+dy/length*bend*.45,y+dy*.73-dx/length*bend*.45)])
  return result
 # Native basin footprint redesign: broad unequal connected wetlands replace
 # tiny deep wells. Existing chimney solids and anchors are preserved.
 expanded=[]
 for k,p in enumerate(basins):
  cx=sum(x for x,y in p)/len(p);cy=sum(y for x,y in p)/len(p)
  expanded.append([(cx+(x-cx)*(1.62 if k==0 else 1.52),cy+(y-cy)*1.42)for x,y in p])
 basins=expanded
 enlarged=[]
 for k,p in enumerate(channels):
  cx=sum(x for x,y in p)/len(p);cy=sum(y for x,y in p)/len(p)
  enlarged.append([(cx+(x-cx)*(1.85 if k==1 else 1.12),cy+(y-cy)*(1.13 if k==1 else 1.9))for x,y in p])
 channels=enlarged
 basins=[weather_edge(p,.065) for p in basins]
 channels=[weather_edge(p,.032) for p in channels]
 for index,footprint in enumerate(basins+channels):
  ...
  if index<2:
   ...
   wet_outline=[(cx+(x-cx)*1.32,cy+(y-cy)*1.32)for x,y in footprint]
   wet=cliff_solid(name+'-broad-wet-olive-transition-'+str(index),wet_outline,.114+variant*.020,bottom=.080+variant*.020,cap=1)
   # Every native drainage cut also crosses the surrounding low wet crust.
   # The previous per-pool hole left ring bridges blocking adjacent channels.
   for cut_index,drainage in enumerate(basins+channels):
    hole=cliff_solid(name+'-wet-transition-drainage-'+str(cut_index),drainage,1.2,bottom=-.16,cap=3)
    bpy.context.view_layer.objects.active=wet;mod=wet.modifiers.new('Open connected wetland drainage','BOOLEAN');mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=hole;bpy.ops.object.modifier_apply(modifier=mod.name);bpy.data.objects.remove(hole,do_unlink=True)
   parts.append(wet)

  parts.append(cliff_solid(name+'-connected-chemical-liquid-'+str(index),footprint,.020+index*.001,bottom=-.15,cap=6 if index<2 else 7))
  if index<2:
   cx=sum(p[0]for p in footprint)/len(footprint);cy=sum(p[1]for p in footprint)/len(footprint)
   shallow=[(cx+(x-cx)*.88,cy+(y-cy)*.84)for x,y in footprint]
   parts.append(cliff_solid(name+'-olive-shallow-'+str(index),shallow,.034,bottom=-.14,cap=7))
   active=[(cx+(x-cx)*.36+.08,cy+(y-cy)*.32-.025)for x,y in footprint]
   parts.append(cliff_solid(name+'-small-active-chemical-vent-'+str(index),active,.042,bottom=-.13,cap=8))
 # One authored cracked intermediate field joins the two main pool banks.
 # Unequal low olive terraces leave the existing liquid course open between
 # them. Original pool outlines, source chimney heights and overall layout stay.
 field=[
 ([(-.45,.01),(-.25,-.03),(-.10,.015),(-.03,-.055),(.10,-.025),(.19,.045),(.08,.115),(-.07,.085),(-.18,.13),(-.32,.09)],.132,1),
 ([(-.37,.21),(-.20,.17),(-.10,.22),(.03,.18),(.14,.25),(.07,.33),(-.08,.31),(-.17,.38),(-.34,.32)],.158,1),
 ([(-.63,-.39),(-.48,-.42),(-.37,-.33),(-.43,-.26),(-.36,-.19),(-.46,-.15),(-.58,-.25)],.081,5),
 ([(.55,.57),(.70,.48),(.81,.52),(.86,.63),(.76,.73),(.62,.70),(.57,.64)],.102,1),
 ([(-.25,.045),(-.16,.00),(-.08,.035),(-.12,.09),(-.19,.10)],.172,4)]
 for k,(poly,top,role)in enumerate(field):parts.append(cliff_solid(name+'-connected-cracked-wet-field-'+str(k),poly,top+variant*.020,bottom=-.10,cap=role))
 ...
 for index,(cx,cy)in enumerate([(-1.04,.52),(.60,-.63),(.13,.87)]):
  buttress=[(cx-.27,cy-.17),(cx+.23,cy-.14),(cx+.28,cy+.04),(cx+.09,cy+.21),(cx-.24,cy+.15)]
  parts.append(cliff_solid(name+'-medium-corroded-buttress-'+str(index),buttress,.34+index*.035,bottom=.13,cap=1 if index==1 else 4))
 # Intermediate dark/olive crags join wetland banks to the retained heroes;
 # they are concentrated at transitions and do not cover the new liquid floor.
 for k,(x,y,w,d,h)in enumerate([(-1.29,-.63,.29,.22,.23),(-1.36,.14,.24,.24,.28),(-.25,-.66,.23,.21,.19),(.94,.70,.26,.24,.24),(1.24,.17,.22,.19,.31),(.24,.83,.21,.19,.23)]):
  parts.append(pillar(name+'-wetland-edge-crag-'+str(k),x,y,w,d,h,.13*(k%3-1),pale=k in(0,3)))
 forms.append((name,parts))

# Native cracked intermediate crust belongs to underlying ground, identified by
# actual source picks. Base radius remains1; only authored solids add local relief.
ground_fields=[]
left=[(-.25,-.18),(-.04,-.22),(.01,-.14),(-.045,-.07),(.025,.015),(-.015,.12),(-.10,.20),(-.27,.10)]
right=[(.04,-.17),(.18,-.16),(.27,-.045),(.18,.055),(.235,.16),(.07,.18),(.025,.10),(.055,.025)]
for i in range(38):
 z=1-2*(i+.5)/38;angle=i*2.399963229728653+.41;r=math.sqrt(1-z*z);n=Vector((math.cos(angle)*r,math.sin(angle)*r,z));east=n.cross(Vector((0,0,1))if abs(z)<.94 else Vector((0,1,0))).normalized();north=n.cross(east);rotation=i*.73;c=math.cos(rotation);sn=math.sin(rotation)
 scale=[.90,1.13,.97,1.04][i%4];height=[.036,.049,.028,.041][i%4]
 objects=[cliff_solid('ground-cracked-terrace-'+str(i)+'-a',[(x*scale,y*scale)for x,y in left],height,bottom=-.018,cap=0),cliff_solid('ground-cracked-terrace-'+str(i)+'-b',[(x*scale,y*scale)for x,y in right],height*.66,bottom=-.018,cap=0 if i%3 else 4)]
 if i%3==0:objects.append(cliff_solid('ground-unequal-step-'+str(i),[(-.20,-.03),(-.13,-.10),(-.075,-.065),(-.08,.055),(-.17,.08)],height+.021,bottom=.008,cap=4))
 for obj in objects:
  bm=bmesh.new();bm.from_mesh(obj.data);bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=2,use_grid_fill=True);bmesh.ops.triangulate(bm,faces=list(bm.faces));bm.to_mesh(obj.data);bm.free()
  for vertex in obj.data.vertices:
   x,y,h=vertex.co;direction=(n+east*(x*c-y*sn)+north*(x*sn+y*c)).normalized();vertex.co=direction*(1+h)
  bm=bmesh.new();bm.from_mesh(obj.data);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(obj.data);bm.free();obj.data.update();obj['groundPlacementId']='toxic-native-ground-field-'+str(i)
  ground_fields.append(obj)
for name, subdivisions in [('ground-sphere',6),('ground-sphere-medium',5),('ground-sphere-low',4)]:
    ...
    ground['role']='planet'
    forms.append((name,[ground,*ground_fields]))
...
# Fine irregular fracture network modulates native PBR finish without changing solids.
fracture=np.zeros((size,size))
for offset,slope in [(.19,.43),(.61,-.29),(.87,.71)]:
 distance=np.abs(((u+slope*v+offset+.009*np.sin(v*47))%1)-.5)
 fracture=np.maximum(fracture,np.exp(-(distance/.004)**2))
streak=streak*.82-fracture*.58
for role in [0,1,2,3,4,5]:
 ...
 for kind in ['albedo','normal','orm']:
  image=bpy.data.images.new(f'Toxic7-corrosion-{role}-{kind}',width=size,height=size,alpha=True);image.colorspace_settings.name='sRGB'if kind=='albedo'else'Non-Color';pixels=np.ones((size,size,4),dtype=np.float32)
  if kind=='albedo':
  ...
  image.pixels.foreach_set(pixels.reshape(-1));filename=f'corrosion-{role}-{kind}.png';image.filepath_raw=str(out/filename);image.file_format='PNG';image.save();image.pack()
  if kind=='albedo':
   reloaded=bpy.data.images.load(str(out/filename),check_existing=False);decoded=np.array(reloaded.pixels[:]).reshape(size,size,4)[:,:,:3];decoded=np.where(decoded<=.04045,decoded/12.92,((decoded+.055)/1.055)**2.4)
   if np.max(np.abs(decoded-linear))>.007:raise RuntimeError('Saved sRGB PNG does not reproduce authored linear palette')
...
for name,parts in forms:
    ...
    if not name.startswith('ground-sphere'):assert max(positions[2::3])<.82,'Unexpected Boolean cutter cap in '+name
