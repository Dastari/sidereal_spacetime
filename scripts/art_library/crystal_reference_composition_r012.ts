...
export function composeCrystalReference(
  ...
  if (diagnostic)
    ...
    for (let i = 0; i < 8; i++) {
      ...
      regions.push({
        ...
        scale: [0.34, 0.28, 0.31, 0.26, 0.33, 0.29, 0.32, 0.27][i],
  ...
  for (const region of regions) {
    ...
    const feet = [
      ...
      { x: -0.48, y: -0.61, hero: false },
      { x: 0.12, y: 0.73, hero: false },
    ];
    for (let j = 0; j < feet.length; j++) {
      const foot = feet[j],
        ...
        sn = Math.sin(angle),
        scale = foot.hero
          ? [0.27, 0.234, 0.306, 0.252][Math.floor(index / 2) % 4]
          : (j >= 3 ? 0.18 : 0.13) + (index % 3) * 0.015,
  ...
  const bridgeCandidates = Array.from({ length: 100 }, (_, i) => {
    const y = 1 - (2 * (i + 0.5)) / 100,
      t = phase + i * 2.39996323,
      r = Math.sqrt(1 - y * y);
    return { i, n: [Math.cos(t) * r, y, Math.sin(t) * r] as Vec };
  }).filter(
    (v) =>
      !regions.some(
        (region) =>
          Math.hypot(...v.n.map((x, k) => x - region.n[k])) <
          region.scale * 0.75,
      ),
  );
  const selected = bridgeCandidates.length
    ? [bridgeCandidates[(seed >>> 0) % bridgeCandidates.length]]
    : [];
  while (selected.length < 14 && selected.length < bridgeCandidates.length) {
    let best: (typeof bridgeCandidates)[number] | undefined,
      score = -1;
    for (const candidate of bridgeCandidates) {
      if (selected.includes(candidate)) continue;
      const d = Math.min(
        ...selected.map((p) =>
          Math.hypot(...candidate.n.map((x, k) => x - p.n[k])),
        ),
      );
      if (d > score) {
        score = d;
  ...
  const bridgeIds = new Set(selected.map((v) => v.i)),
    bridgeRequests: {
      i: number;
      n: Vec;
      east: Vec;
  ...
  for (let i = 0; i < (diagnostic ? 0 : 100); i++) {
    const y = 1 - (2 * (i + 0.5)) / 100,
      ...
      r = Math.sqrt(1 - y * y),
      n: Vec = [Math.cos(theta) * r, y, Math.sin(theta) * r],
      east = unit(cross(Math.abs(y) > 0.94 ? [1, 0, 0] : [0, 1, 0], n)),
      north = cross(n, east),
      rotation = random() * Math.PI * 2,
      c = Math.cos(rotation),
      s = Math.sin(rotation),
      scale = 0.21 + random() * 0.12;
    // Avoid filling the authored crater cavities themselves.
    if (
      regions.some(
        (region) =>
          Math.hypot(...n.map((v, k) => v - region.n[k])) < region.scale * 0.75,
      )
    )
      continue;
    emit(
      i % 4 === 0 ? "fracture-cliffs" : "low-fractured-shelf",
      `outcrop-${i}`,
      (v) => {
        const x = (v[0] * c - v[1] * s) * scale,
          z = (v[0] * s + v[1] * c) * scale,
          d = unit(n.map((q, k) => q + east[k] * x + north[k] * z) as Vec),
          h = 1 + v[2] * scale + 0.02;
        return d.map((q) => q * h) as Vec;
      },
    );
  ...
  for (const { i, n, east, north, c, s } of bridgeRequests) {
    ...
    let foot = -Infinity;
    for (const role of [0, 1, 2, 8]) {
      const batch = batches[role];
      for (let j = 0; j < batch.indices.length; j += 3) {
        const p = [0, 1, 2].map(
          (k) =>
            batch.positions.slice(
              batch.indices[j + k] * 3,
              batch.indices[j + k] * 3 + 3,
            ) as Vec,
        );
        if (p.every((v) => dot(v, n) <= 0)) continue;
        const q = p.map((v) => [dot(v, east), dot(v, north)]),
          [a, b, c] = q,
          den = (b[1] - c[1]) * (a[0] - c[0]) + (c[0] - b[0]) * (a[1] - c[1]);
        if (Math.abs(den) < 1e-12) continue;
        const u = ((b[1] - c[1]) * -c[0] + (c[0] - b[0]) * -c[1]) / den,
          v = ((c[1] - a[1]) * -c[0] + (a[0] - c[0]) * -c[1]) / den,
          w = 1 - u - v;
        if (u >= -1e-8 && v >= -1e-8 && w >= -1e-8)
          foot = Math.max(
            foot,
            u * dot(p[0], n) + v * dot(p[1], n) + w * dot(p[2], n),
          );
      }
    }
    if (!Number.isFinite(foot))
      throw new Error("Bridge crystal has no final native terrain support");
    const bridgeScale = 0.19 + (i % 3) * 0.018;
    emit("medium-cluster", `bridge-crystals-${i}`, (v) => {
      const x = (v[0] * c - v[1] * s) * bridgeScale,
        z = (v[0] * s + v[1] * c) * bridgeScale,
        h = foot + v[2] * bridgeScale;
