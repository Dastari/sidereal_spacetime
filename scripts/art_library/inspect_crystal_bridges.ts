...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/crystal-r012/kit.json",
    "utf8",
  ),
);
...
for (const b of batches)
  for (const r of b.ranges.filter((r) => r.partId.startsWith("bridge"))) {
...
console.log(
  JSON.stringify(
    [...centers].map(([id, p]) => ({
      ...
      p,
      dot:
        p.reduce((n, v, i) => n + v * [0.452, 0.388, 0.794][i], 0) /
        Math.hypot(...p),
    })),
    null,
    2,
