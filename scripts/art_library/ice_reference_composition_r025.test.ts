...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ice-r025/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("NullEngine clears each distinct native floor through variant-specific substrate indexing", () => {
  for (const mode of [true, "gorge"] as const) {
    ...
    try {
      const meshes = composeIceReference(kit, 38, 0, mode).flatMap((b, i) =>
        b.ranges.map((range) => {
          ...
          data.positions = b.positions.slice(
            range.firstTriangle * 9,
            (range.firstTriangle + range.triangleCount) * 9,
          );
          data.normals = b.normals.slice(
            range.firstTriangle * 9,
            (range.firstTriangle + range.triangleCount) * 9,
          );
          data.uvs = b.uvs.slice(
            range.firstTriangle * 6,
            (range.firstTriangle + range.triangleCount) * 6,
          );
          data.indices = Uint32Array.from(
            { length: range.triangleCount * 3 },
            (_, i) => i,
          );
      ...
      const ground = hits.find((v) => v.mesh.metadata.partId === "ground")!;
...
it("preserves corrected Ice21 material exports while replacing regional morphology", () => {
  ...
  for (let role = 0; role < 5; role++)
    for (const channel of ["albedo", "orm"])
      expect(
        readFileSync(
          `output/playwright/planet-reference-20260914/ice-r025/ice-${role}-${channel}.png`,
        ),
      ).toEqual(
        readFileSync(
...
it("replaces gorge cap coverage while preserving the reviewed shaft22 native variant exactly", () => {
  ...
  expect(kit.variants.find((v) => v.name === "snow-cut-region")).toEqual(
    old.variants.find((v) => v.name === "snow-cut-region"),
  );
  const snowArea = (source: NativePlanetKit) => {
    const v = source.variants.find((v) => v.name === "snow-open-gorge")!;
    let area = 0;
    for (let i = 0; i < v.indices.length; i += 3) {
      if (v.triangleMaterials[i / 3] !== 0) continue;
      const [a, b, c] = [0, 1, 2].map((k) =>
        Vector3.FromArray(v.positions, v.indices[i + k] * 3),
      );
      area += Math.max(0, Vector3.Cross(b.subtract(a), c.subtract(a)).z) / 2;
    }
    return area;
  };
  const ratio = snowArea(kit) / snowArea(old);
...
it("keeps native ground radius outside authored regions while clearing relief beneath open floors", () => {
  const old = JSON.parse(
    readFileSync(
      "output/playwright/planet-reference-20260914/ice-r023/kit.json",
      "utf8",
    ),
  ) as NativePlanetKit;
  for (const name of ["snow-cut-region", "snow-open-gorge"])
  ...
  const source = kit.variants.find((v) => v.name === "ground-sphere")!,
    batches = composeIceReference(kit, 38, 0, true),
    cursor = batches.map(
      (b) => b.ranges.find((r) => r.partId === "ground")?.firstTriangle! * 9,
    ),
    n = new Vector3(0.452, 0.388, 0.794).normalize();
  let untouched = 0;
  for (let i = 0; i < source.indices.length; i += 3) {
    const role = source.triangleMaterials[i / 3];
    for (let k = 0; k < 3; k++) {
      const p = Vector3.FromArray(source.positions, source.indices[i + k] * 3),
        output = Vector3.FromArray(batches[role].positions, cursor[role]);
      cursor[role] += 3;
      if (p.length() > 1.02 && Vector3.Dot(p.normalizeToNew(), n) < -0.3) {
        expect(output.length()).toBeCloseTo(p.length(), 6);
        untouched++;
      }
    }
  }
  ...
  expect(
    Math.max(
      ...source.positions
        .filter((_, i) => i % 3 === 0)
        .map((_, i) => Math.hypot(...source.positions.slice(i * 3, i * 3 + 3))),
    ),
...
it("authors thick white caps and exposed blue basal geometry instead of recoloring the previous white globe", () => {
  const native = kit.variants.find((v) => v.name === "ground-sphere")!;
  let lowBlue = 0,
    highSnow = 0;
  for (let i = 0; i < native.indices.length; i += 3) {
    const role = native.triangleMaterials[i / 3];
    for (let k = 0; k < 3; k++) {
      const index = native.indices[i + k] * 3,
        radius = Math.hypot(...native.positions.slice(index, index + 3));
      if (role === 2 && radius < 0.99) lowBlue++;
      if (role === 0 && radius > 1.025) highSnow++;
    }
  }
  expect(lowBlue).toBeGreaterThan(5000);
  expect(highSnow).toBeGreaterThan(5000);
  const old = JSON.parse(
    readFileSync(
      "output/playwright/planet-reference-20260914/ice-r024/kit.json",
      "utf8",
    ),
  ) as NativePlanetKit;
  expect(kit.materials).toEqual(old.materials);
