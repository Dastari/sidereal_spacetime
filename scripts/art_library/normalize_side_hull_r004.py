"""Derive normalized r004 from preserved editable Blender masters; no publication."""
from pathlib import Path
import sys, json, hashlib, subprocess, tomllib
ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT/'assets/art-library/designs/shipyard.hull.side-armor/revisions/r004'
SOURCE = ROOT/'assets/art-library/designs/shipyard.hull.side-armor/revisions/r003/blender-source.blend'
COMPONENTS = ROOT/'.runtime/art-library/side-hull/r003/components.json'
...
anchors={'HULL_ATTACH':[0,0,0], 'HULL_EDGE_START':[0,-1,0], 'HULL_EDGE_END':[0,1,0], 'HULL_TOP':[0,0,2.9375]}
records=[];exports=[];removed=0
for component in components:
    oldid=component['slug'];newid='part-'+hashlib.sha256((oldid+':normalized-side-hull-r004').encode()).hexdigest()[:20]
    ...
    for ob in objects:
        ...
        for v in ob.data.vertices:
            co=matrix@v.co;co.x=side*(co.x-mounting_x);v.co=co
    ...
    assert abs(bounds['min'][2])<1e-5 and abs(bounds['max'][2]-2.9375)<1e-5
    ...
    records.append({'assetId':newid,'previousAssetId':oldid,'variant':component['variant'],'previousSide':side,
       ...
       'triangles':triangles,'lights':lights,'legacyMountingX':mounting_x})
bpy.ops.object.select_all(action='DESELECT')
...
validation={'schema':'sidereal.side-hull-normalized.v1','source':str(SOURCE.relative_to(ROOT)),'sourceSha256':source_hash,'revision':4,
 'frame':{'x':'outward','y':'tangent','z':'up','mountingPlaneX':0,'spanMeters':2,'heightMeters':2.9375},
...
(OUT/'review.md').write_text('Normalized editable r003 side armor into r004 with no mesh scaling. Removed temporary legacy thruster cutouts; new asset IDs preserve installed r003 ships. Every carrier has local mounting X=0, tangent Y=-1..1 and deck base Z=0; retained actual height is 2.9375m. The white vent, red service and white plain-service contact row is an actual Blender render of these surfaces at the same mounting plane. No browser/game integration or artistic sign-off is claimed.\n')
