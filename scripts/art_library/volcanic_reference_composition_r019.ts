...
export function composeVolcanicReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 function emit(name:string,partId:string,transform:(p:Vec)=>Vec,smooth=false){
  const mesh=kit.variants.find(v=>v.name===name) as (NativePlanetKit['variants'][number]&{normals?:number[]})|undefined;
  ...
  for(let i=0;i<mesh.indices.length;i+=3){
   ...
   positions.forEach((p,k)=>{batch.positions.push(...p);batch.normals.push(...normals[k]);});batch.indices.push(start,start+1,start+2);
  }
 ...
 for(let i=0;i<count;i++){
  const anchor:Vec=diagnostic?[.452,.388,.794]:globeAnchor(i,count,phase),scale=diagnostic?.26:[.26,.23,.28,.25,.24,.27,.22,.26,.24,.28,.25,.23][i],angle=diagnostic?0:random()*Math.PI*2;
