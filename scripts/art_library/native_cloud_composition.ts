...
export function composeNativeClouds(
  ...
  const assembly = nativeAssembly(kit),
    random = seeded(seed),
    count = Math.ceil(
      Math.max(0, Math.min(1, coverage)) * NATIVE_CLOUD_BANK_LIMIT,
    );
  ...
  for (let i = 0; i < count; i++) {
    ...
    const scale = 0.15 + random() * 0.11,
      ...
      altitude = 0.096 + random() * 0.014;
    const name = BANKS[i % BANKS.length],
      heightScale = name === "cloud-towering-bank" ? 0.038 : 0.029;
    const place = tangent(
      globeAnchor(i, NATIVE_CLOUD_BANK_LIMIT, directionPhase),
      scale,
      rotation,
      altitude,
      true,
