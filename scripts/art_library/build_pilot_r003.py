...
for slug in slugs:
 ...
 if slug.startswith('floor'):
  ...
  cap=[(.09,.09),(1.91,.09),(1.91,1.46),(1.46,1.91),(.09,1.91)]
  loft('GEO-Sloping pale shoulder cap',[(x,y,height(y)-.015) for x,y in cap],[(x,y,height(y)+.065) for x,y in cap],pale)
  box('GEO-High aft shoulder spine',(.07,.08,2.60),(1.94,.34,2.72),dark,.02)
  # Vents follow the same sloping plane rather than floating above it.
  pane('GEO-Recessed sloping vent',(.98,.48,height(.48)+.072),(1.66,.48,height(.48)+.072),(1.66,1.42,height(1.42)+.072),(.98,1.42,height(1.42)+.072),dark,.018)
  for yy in [.55,.68,.81,.94,1.07,1.20,1.33]:beam('GEO-Slope vent louvre',(.99,yy,height(yy)+.09),(1.65,yy,height(yy)+.09),.028,.04,metal)
 ...
 visuals=active[:]
 for o in visuals:o['component_id']=slug;o['representation']='native Blender visual';o['review_revision']=3
 ...
 components.append({'slug':slug,'id':'part-'+hashlib.sha256(('hull-pilot-r3-'+slug).encode()).hexdigest()[:20],'category':'floor' if slug.startswith('floor') else 'roof' if slug=='pilot-roof' else 'wall','bounds':bounds,'nominal_dimensions_m':[bounds['max'][i]-bounds['min'][i] for i in range(3)],'collection':slug})
placements=[]
def place(slug,x,y,z=0,rotation=0,flipped=False):
 y+=4 # Assembly origin moves ahead of main hull; reusable meshes remain local.
 c=next(c for c in components if c['slug']==slug);pid=f'pilot-r003-{slug}-{len(placements):02}'
...
(OUT/'components.json').write_text(json.dumps(components,indent=2));(OUT/'wayfarer.json').write_text(json.dumps({'schema':'sidereal.assembly-draft.v1','id':'pilot-section-review-r003','name':'Compact swept pilot bay / unsigned r003','parts':placements},indent=2))
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'pilot-kit.blend'))
print('R003_COMPLETE',flush=True)
