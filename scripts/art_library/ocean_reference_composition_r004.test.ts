import {composeOceanReference as previousComposition} from './ocean_reference_composition_r003';
import {expect,it} from 'vitest';
import {readFileSync} from 'node:fs';
import {composeOceanReference} from './ocean_reference_composition_r004';
...
it('changes only transformed normals, preserving prior positions, indices and placement ranges',()=>{
 for(const lod of [0,1,2] as const){
  const previous=previousComposition(kit,38,lod),next=composeOceanReference(kit,38,lod);
