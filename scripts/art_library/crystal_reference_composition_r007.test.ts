...
it("keeps four distinct hero placements and medium bridges within a bounded geometry budget", () => {
  const b = composeCrystalReference(kit, 38, 0),
    centers = new Map<string, number[]>();
  for (const batch of b)
    for (const range of batch.ranges.filter((r) =>
      r.partId.startsWith("colossal"),
    )) {
      const p = batch.positions.slice(
        range.firstTriangle * 9,
        (range.firstTriangle + range.triangleCount) * 9,
      );
      const center = [0, 0, 0];
      for (let i = 0; i < p.length; i++) center[i % 3] += p[i] / (p.length / 3);
      centers.set(range.partId, center);
    }
  ...
  for (let i = 0; i < 4; i++)
    for (let j = i + 1; j < 4; j++)
      expect(
        Math.hypot(...values[i].map((v, k) => v - values[j][k])),
      ).toBeGreaterThan(0.5);
