...
it("qualifies only tested full native Ice moon and Crystal moon revisions for one retained geometry", () => {
  for (const revision of [
    "ice-moon-1-r002",
    "ice-moon-2-r002",
    "ice-moon-2-r003",
  ])
    expect(referenceFixedDetail("ice-moon-glacial", revision)).toBe(true);
  for (const revision of ["crystal-moon-1-r004", "crystal-moon-2-r004"])
    expect(referenceFixedDetail("crystal-moon-native-body", revision)).toBe(
      true,
    );
  expect(
    referenceFixedDetail("crystal-moon-geology", "crystal-moon-1-r004"),
  ).toBe(false);
  expect(referenceFixedDetail("ice-moon-glacial", "ice-moon-2-r004")).toBe(
    false,
  );
});

it("retains one geometry for exact native optical moon revisions and rejects unknown successors", () => {
  for (const revision of ["gas-giant-moon-1-r005", "gas-giant-moon-3-r005"])
    expect(referenceFixedDetail("solid-moon-craters", revision)).toBe(true);
  for (const revision of [
    "volcanic-moon-1-r003",
    "volcanic-moon-2-r004",
    "volcanic-moon-2-r005",
    "volcanic-moon-2-r006",
  ])
    expect(referenceFixedDetail("volcanic-moon-craters", revision)).toBe(true);
  for (const revision of [
    "crystal-moon-1-r005",
    "crystal-moon-2-r005",
    "crystal-moon-1-r008",
  ...
  expect(
    referenceFixedDetail("solid-moon-craters", "gas-giant-moon-1-r006"),
  ).toBe(false);
  expect(
    referenceFixedDetail("volcanic-moon-craters", "volcanic-moon-2-r007"),
  ).toBe(false);
});
