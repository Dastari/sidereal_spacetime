...
for (const id of ["crystal-moon-1", "crystal-moon-2"])
  it(`${id} replaces selected shells while retaining every unselected native triangle exactly`, () => {
    const path = `output/playwright/planet-reference-20260914/${id}-r009/`,
      kit = JSON.parse(readFileSync(path + "kit.json", "utf8")),
      priorPath = path.replace("r009", "r005"),
...
it("r009 carries broad regional low/high surfaces and recessed runs across multiple source districts", () => {
  for (const id of ["crystal-moon-1", "crystal-moon-2"]) {
    const plan = JSON.parse(
      readFileSync(
        `output/playwright/planet-reference-20260914/${id}-r009/native-fracture-plan.json`,
        "utf8",
      ),
    );
...
for (const id of ["crystal-moon-1", "crystal-moon-2"])
  it(`${id} preserves r008 interior parts and bounds actual occupied terminal surfaces`, () => {
    const root = `output/playwright/planet-reference-20260914/${id}-`,
      old = JSON.parse(readFileSync(root + "r008/kit.json", "utf8")),
      kit = JSON.parse(readFileSync(root + "r009/kit.json", "utf8")),
      a = old.variants[0],
      b = kit.variants[0];
    expect(kit.materials).toEqual(old.materials);
    for (const part of old.nativePartRanges) {
      if ([0, 11].includes(part.section)) continue;
      const next = kit.nativePartRanges.find(
        (p: any) => p.partId === part.partId,
      );
      expect(next).toBeDefined();
      expect(next.triangleCount).toBe(part.triangleCount);
      for (const [channel, stride] of [
        ["positions", 9],
        ["normals", 9],
        ["uvs", 6],
        ["triangleMaterials", 1],
        ...
        expect(
          ...
          a[channel].slice(
            part.firstTriangle * stride,
            (part.firstTriangle + part.triangleCount) * stride,
          ),
        );
    }
    for (const part of kit.nativePartRanges.slice(1)) {
      expect(part.runLimits[0]).toBeGreaterThan(-2);
      expect(part.runLimits[1]).toBeLessThan(2);
      const values: number[] = [],
        corners: number[][] = [];
      for (
        let t = part.firstTriangle;
        t < part.firstTriangle + part.triangleCount;
        t++
      )
        for (let k = 0; k < 3; k++) {
          const i = b.indices[t * 3 + k] * 3;
          corners.push(b.positions.slice(i, i + 3));
        }
      for (const p of corners) {
        const pairedBase = p.map(
          (v: number, j: number) =>
            v - part.authoredNormal[j] * (part.height + 0.23),
        );
        if (
          !corners.some((q) =>
            q.every(
              (v: number, j: number) => Math.abs(v - pairedBase[j]) < 1e-5,
            ),
          )
        )
          continue;
        values.push(
          p.reduce(
            (s: number, v: number, j: number) =>
              s + (v - part.authoredNormal[j] * part.height) * part.runAxis[j],
            0,
          ),
        );
      }
      expect(values.length).toBeGreaterThan(0);
      expect(Math.min(...values)).toBeGreaterThanOrEqual(
        part.runLimits[0] - 1e-5,
      );
      expect(Math.max(...values)).toBeLessThanOrEqual(part.runLimits[1] + 1e-5);
