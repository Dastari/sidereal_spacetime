...
export function composeOceanReference(
  ...
  const anchors: Vec[] = [];
  const layouts = [
    { name: "steep-island", scale: 0.27, y: 0.46, angle: 0, height: 0.65 },
    { name: "archipelago", scale: 0.23, y: -0.36, angle: 1.54, height: 0.56 },
    { name: "long-island", scale: 0.24, y: 0.13, angle: 3.48, height: 0.46 },
    { name: "steep-island", scale: 0.19, y: -0.58, angle: 4.77, height: 0.65 },
    { name: "lagoon-atoll", scale: 0.16, y: 0.74, angle: 2.25, height: 0 },
  ];
  for (let i = 0; i < layouts.length; i++) {
    const spec = layouts[i],
      angle = phase + spec.angle + (random() - 0.5) * 0.17,
      y = spec.y + (random() - 0.5) * 0.1,
      r = Math.sqrt(1 - y * y),
    ...
    if (spec.height && spec.name !== "archipelago") {
      ...
      const groves = [
        { x: -0.1, y: 0.08, scale: 0.06 },
        { x: 0.19, y: -0.08, scale: 0.046 },
      ];
      for (let g = 0; g < groves.length; g++) {
        ...
        emit(g === 0 ? "tree-grove" : "small-grove", `grove-${i}-${g}`, (v) => {
          const lx = grove.x * spec.scale + v[0] * grove.scale,
            ...
            x = lx * c - ly * ss,
            z = lx * ss + ly * c,
            h = 1 + spec.height * spec.scale + v[2] * grove.scale;
          return [
