...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/toxic-r007/kit.json','utf8')) as NativePlanetKit;
it('retains native corroded crust geometry, corner attributes and placement identities across all LODs',()=>{
 ...
 expect(levels[0].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(135000);
...
it('preserves all Toxic6 pool/chimney variants and PBR bytes while adding real native ground relief',()=>{
 const old=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/toxic-r006/kit.json','utf8'))as NativePlanetKit;expect(kit.materials).toEqual(old.materials);
 for(const name of ['toxic-basin-group-a','toxic-basin-group-b','toxic-basin-group-c'])expect(kit.variants.find(v=>v.name===name)).toEqual(old.variants.find(v=>v.name===name));
 for(let role=0;role<6;role++)for(const channel of ['albedo','normal','orm'])expect(readFileSync(`output/playwright/planet-reference-20260914/toxic-r007/corrosion-${role}-${channel}.png`)).toEqual(readFileSync(`output/playwright/planet-reference-20260914/toxic-r006/corrosion-${role}-${channel}.png`));
 const source=kit.variants.find(v=>v.name==='ground-sphere')!;let relief=0;for(let i=0;i<source.positions.length;i+=3)relief=Math.max(relief,Math.hypot(...source.positions.slice(i,i+3))-1);expect(relief).toBeGreaterThan(.06);expect(relief).toBeLessThan(.071);
 const before=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/toxic-r007/source-picks-before.json','utf8'))[0],after=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/toxic-r007/source-picks-after.json','utf8'))[0];expect(before.partId).toBe('ground');expect(after.partId).toBe('ground');expect(Math.hypot(...after.point)-Math.hypot(...before.point)).toBeGreaterThan(.025);
});
