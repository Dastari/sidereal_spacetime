...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r005'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-desert-r005'
    return obj

# Front path runs left to right. Midway inward bite is an eroded canyon notch.
front=[(-1.50,-.20),(-1.28,-.20),(-1.28,-.31),(-.94,-.31),(-.94,-.16),(-.69,-.16),(-.69,.09),(-.48,.09),(-.48,.29),(-.22,.29),(-.22,-.01),(.03,-.01),(.03,-.22),(.38,-.22),(.38,-.08),(.70,-.08),(.70,-.28),(1.07,-.28),(1.07,-.13),(1.50,-.13)]
for variant in range(3):
    name=['regional-cliff-head','regional-cliff-continuation','regional-cliff-end'][variant]
    path=[(x,y+(.11*math.sin(x*3.0) if variant==1 else 0)) for x,y in front]
    if variant==2:
        path=[(x*.80,y+.16*max(0,x)) for x,y in front]
    farleft=path[0][0];farright=path[-1][0]
    rear=path+[(farright,.76),(.55,.76),(.55,.90),(-.43,.90),(-.43,.70),(farleft,.70)]
    parts=[cliff_solid(name+'-high-landmass',rear,.34 if variant!=1 else .27)]
    # Two lower broken aprons follow the edge, offset unevenly. They terminate
    # against the cliff instead of surrounding the entire region with a border.
    lower=[(x,y-.20-.055*math.sin(i*1.7)) for i,(x,y) in enumerate(path)]
    lower_foot=lower+list(reversed([(x,y+.025) for x,y in path]))
    parts.append(cliff_solid(name+'-middle-eroded-shelf',lower_foot,.145,cap=5))
    # Disconnected sections make broken lower ledges, avoiding a uniform ring.
    for section,(start,end) in enumerate([(0,7),(11,len(path))]):
        sectionpath=lower[start:end]
        edge=[(x,y-.13-.04*math.cos(i)) for i,(x,y) in enumerate(sectionpath)]
        foot=edge+list(reversed([(x,y+.02) for x,y in sectionpath]))
        parts.append(cliff_solid(name+'-lower-shelf-'+str(section),foot,.025,cap=1))
    if variant==0:
        for i,values in enumerate([(1.08,.25,.38,.44,.84,0),(.75,.40,.31,.34,.61,0),(1.22,.52,.29,.26,.51,0)]):
            parts.append(pillar(name+'-summit-'+str(i),*values))
    elif variant==2:
        parts.append(pillar(name+'-broken-end',.71,.40,.39,.38,.46,0))
    forms.append((name,parts))
