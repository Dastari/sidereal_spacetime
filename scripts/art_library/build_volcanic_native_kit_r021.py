...
Run: blender -b --python scripts/art_library/build_volcanic_native_kit_r021.py -- NEW_OUTPUT_DIR
...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-volcanic-r021'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=2):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-volcanic-r021'
    return obj
...
for role in [0,1,2,4]:
 image=bpy.data.images.new('Basalt21-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('Basalt21-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];roughpixels=[]
 for y in range(256):
  for x in range(256):
