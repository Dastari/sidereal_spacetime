...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ocean-r001/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("places grove roots above water and keeps a conservative island-footprint bound", () => {
  ...
  expect(trunks.ranges).toHaveLength(6);
  for (const r of trunks.ranges) {
    ...
    for (let i = 0; i < p.length; i += 3)
      expect(Math.hypot(p[i], p[i + 1], p[i + 2])).toBeGreaterThan(1.08);
  }
  ...
  const footprints = [
    ...
    0.19 * 1.12,
    0.16 * 1.08,
    ...Array(8).fill(0.12 * 0.5),
  ];
  expect(
    footprints.reduce((a, r) => a + Math.PI * r * r, 0) / (4 * Math.PI),
  ).toBeLessThan(0.1);
