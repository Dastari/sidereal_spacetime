...
import {composeOceanReference} from './ocean_reference_composition';
import type {NativePlanetKit} from '../../packages/render/src/environment/native-planet-composition';
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ocean-r001/kit.json','utf8')) as NativePlanetKit;
...
it('places grove roots above water and keeps a conservative island-footprint bound',()=>{
 const trunks=levels[0][9];expect(trunks.ranges).toHaveLength(6);
 for(const r of trunks.ranges){const p=trunks.positions.slice(r.firstTriangle*9,(r.firstTriangle+r.triangleCount)*9);for(let i=0;i<p.length;i+=3)expect(Math.hypot(p[i],p[i+1],p[i+2])).toBeGreaterThan(1.08);}
 ...
 const footprints=[.27*1.12,.23*1.2,.24*1.16,.19*1.12,.16*1.08,...Array(8).fill(.12*.5)];
 expect(footprints.reduce((a,r)=>a+Math.PI*r*r,0)/(4*Math.PI)).toBeLessThan(.1);
