"""Native glacial shafts under thick irregular snow. Isolated r020 only.
Run: blender -b --python scripts/art_library/build_ice_native_kit_r020.py -- NEW_OUTPUT
"""
...
def mesh_object(name,verts,faces,roles,smooth=None):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=1,use_grid_fill=True);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-ice-r020';return o
...
for name,subdivisions in[('ground-sphere',5),('ground-sphere-medium',4),('ground-sphere-low',3)]:
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions,radius=1);o=bpy.context.object;o.name='GEO-'+name;o.data.materials.append(materials[0]);o['role']='planet';o['authoring']='native-ice-r020'
...
for role in range(5):
 image=bpy.data.images.new('Ice20-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('Ice20-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];rp=[]
 for y in range(256):
  for x in range(256):
   u=x/256;v=y/256
   ...
   pixels.extend((*(c*factor for c in palette[role][1]),1));rp.extend((1,rough,0,1))
