...
it("material-only finish preserves every native geometry/UV/normal and grove exactly", () => {
  for (const id of ids) {
    ...
    expect(after.compositionRecipe).toEqual(before.compositionRecipe);
    for (let i = 0; i < before.variants.length; i++)
      for (const channel of ["positions", "normals", "uvs", "indices"])
        expect(after.variants[i][channel]).toEqual(before.variants[i][channel]);
    const counts = (k: any, roles: number[]) =>
      k.variants
        .filter((v: any) => v.name.startsWith("battered-region-"))
        .reduce(
          (n: number, v: any) =>
            n +
            v.triangleMaterials.filter((r: number) => roles.includes(r)).length,
          0,
        );
    if (id.startsWith("temperate"))
      expect(counts(after, [6, 7])).toBeGreaterThan(
        counts(before, [6, 7]) * 1.7,
      );
    else expect(counts(after, [9])).toBeGreaterThan(1000);
  }
});
