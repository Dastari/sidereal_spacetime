...
import type { PlanetRecipe } from "../../packages/content/src/environment";
...
export function referenceVolcanicSmokeRuntime(
  scene: Scene,
  bodyId: string,
  recipe: PlanetRecipe,
) {
  let shared: Material | undefined;
  const meshes: Mesh[] = [];
  return {
    attach(
      parent: TransformNode,
      lod: 0 | 1 | 2,
      prepared: PackedPlanetGeometry | undefined,
    ) {
      if (recipe.style !== "volcanic" || !prepared) return undefined;
      const mesh = createPlanetSmoke(
        scene,
        `${bodyId}-lod-${lod}`,
        recipe,
        lod,
      ...
      mesh.metadata = {
        ...
        smokeSource: "existing-planet-smoke",
        trianglePlacementRanges: [
          {
            firstTriangle: 0,
            triangleCount: prepared.indices.length / 3,
            partId: `${bodyId}:${recipe.seed}:smoke`,
          },
        ],
      };
      meshes.push(mesh);
    ...
    stats() {
      const active = meshes.filter(
        (mesh) => !mesh.isDisposed() && mesh.isEnabled(),
      );
      return {
        smokeImplemented: true,
        smokeMeshes: active.length,
        smokeTriangles: active.reduce(
          (n, mesh) => n + mesh.getTotalIndices() / 3,
          0,
        ),
        retainedSmokeMeshes: meshes.filter((mesh) => !mesh.isDisposed()).length,
      };
    },
    dispose() {
