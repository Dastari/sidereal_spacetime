...
const out='.runtime/art-library/hull/r002', read=(p:string)=>JSON.parse(readFileSync(p,'utf8'));
...
for(const component of components){
 ...
 catalog.assets.push({id:component.id,label:`Pilot ${component.slug} / r002`,category:component.category,nodes:[],bounds:component.bounds,thumbnail:`/assets/assembly/hull-review/${component.slug}/cutout.png`,visual:{url:`/assets/assembly/hull-review/${component.slug}/clean.glb`,sha256:createHash('sha256').update(readFileSync(`${out}/${component.slug}/clean.glb`)).digest('hex'),designId:'shipyard.hull.pilot-section',revision:2,bounds:component.bounds,damagePreview:'unsupported'}});
}
