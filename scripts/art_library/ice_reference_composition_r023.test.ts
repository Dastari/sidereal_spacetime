...
import {composeIceReference} from './ice_reference_composition_r023';
import type {NativePlanetKit} from '../../packages/render/src/environment/native-planet-composition';
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ice-r023/kit.json','utf8')) as NativePlanetKit;
it('retains shaft and gorge native surfaces and optical channels identically across LODs',()=>{
 ...
 const ids=new Set(a.flatMap(b=>b.ranges.map(r=>r.partId)));expect(ids.size).toBe(13);expect(a.reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(185000);
});
...
it('preserves corrected Ice21 material exports and authored ground while replacing regional morphology',()=>{
 ...
 for(const name of ['ground-sphere','ground-sphere-medium','ground-sphere-low'])expect(kit.variants.find(v=>v.name===name)).toEqual(old.variants.find(v=>v.name===name));
 for(let role=0;role<5;role++)for(const channel of ['albedo','orm'])expect(readFileSync(`output/playwright/planet-reference-20260914/ice-r023/ice-${role}-${channel}.png`)).toEqual(readFileSync(`output/playwright/planet-reference-20260914/ice-r021/ice-${role}-${channel}.png`));
