...
import {composeIceReference} from './ice_reference_composition_r018';
import type {NativePlanetKit} from '../../packages/render/src/environment/native-planet-composition';
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ice-r018/kit.json','utf8')) as NativePlanetKit;
it('retains native shaft geometry, corner attributes and placement identities across all LODs',()=>{
 ...
 expect(levels[0].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(12000);
...
it('maps closed hero columns outward with substantial radial height and vertical wall normals',()=>{
 const batches=composeIceReference(kit,38,0,true);let highestBlue=0,verticalHighWall=0;
 for(const role of [2,3,4]){const b=batches[role];for(let i=0;i<b.positions.length;i+=3){const p=Vector3.FromArray(b.positions,i),radius=p.length();highestBlue=Math.max(highestBlue,radius);if(radius>1.15){const normal=Vector3.FromArray(b.normals,i);if(Math.abs(Vector3.Dot(p.normalize(),normal))<.2)verticalHighWall++;}}}
 expect(highestBlue).toBeGreaterThan(1.25);expect(verticalHighWall).toBeGreaterThan(30);
});
