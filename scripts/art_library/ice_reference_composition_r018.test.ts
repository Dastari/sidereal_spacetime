...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ice-r018/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("keeps the open glacial cut below surrounding crust with substrate behind its exposed native floor", () => {
  ...
  try {
    const level = composeIceReference(kit, 38, 0, true),
      meshes = level.map((b, i) => {
        const mesh = new Mesh(`ice-cut-${i}`, scene);
        mesh.metadata = {
          role: "planet",
          materialRole: i,
          triangleRanges: b.ranges,
        };
        mesh.material = new PBRMaterial(`cut-role-${i}`, scene);
        const data = new VertexData();
        data.positions = b.positions;
        data.normals = b.normals;
        data.uvs = b.uvs;
    ...
    const direction = n
      .add(east.scale(1.35 * 0.28))
      .add(north.scale(-0.62 * 0.28))
      .normalize();
...
it("maps closed hero columns outward with substantial radial height and vertical wall normals", () => {
  const batches = composeIceReference(kit, 38, 0, true);
  let highestBlue = 0,
    verticalHighWall = 0;
  for (const role of [2, 3, 4]) {
    const b = batches[role];
    for (let i = 0; i < b.positions.length; i += 3) {
      const p = Vector3.FromArray(b.positions, i),
        radius = p.length();
      highestBlue = Math.max(highestBlue, radius);
      if (radius > 1.15) {
        const normal = Vector3.FromArray(b.normals, i);
        if (Math.abs(Vector3.Dot(p.normalize(), normal)) < 0.2)
          verticalHighWall++;
      }
    }
  }
  expect(highestBlue).toBeGreaterThan(1.25);
  expect(verticalHighWall).toBeGreaterThan(30);
});
