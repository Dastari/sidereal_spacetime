...
import {composeIceMoonReference} from './ice_moon_reference_composition_r003';
...
it('retains shaft and gorge native surfaces and optical channels identically across LODs',()=>{
 ...
 const ids=new Set(a.flatMap(b=>b.ranges.map(r=>r.partId)));expect(ids.size).toBe(id.endsWith('-1')?11:9);expect(a.reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(350000);
});
...
it('keeps the large crater floor exposed in all18 seeded glacial moon placements',()=>{
 const engine=new NullEngine(),scene=new Scene(engine);scene.useRightHandedSystem=true;
 try{for(const id of ['ice-moon-1','ice-moon-2']){const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r002/kit.json`,'utf8'));const meshes=composeIceMoonReference(kit,38,0).map((b,i)=>{const mesh=new Mesh(`${id}-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial('moon-role',scene);const data=new VertexData();Object.assign(data,b);data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
...
for(const id of ['ice-moon-1','ice-moon-2'])for(const seed of [38,917])it(`matches every frozen output byte and placement range for ${id} seed${seed}`,()=>{
 const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r002/kit.json`,'utf8'));const before=frozen(kit,seed,0),after=composeIceMoonReference(kit,seed,0);
 for(let role=0;role<before.length;role++){for(const field of ['positions','normals','uvs','indices']as const)expect(Buffer.from(before[role][field].buffer).equals(Buffer.from(after[role][field].buffer)),`${id}/${seed}/${role}/${field}`).toBe(true);expect(after[role].ranges).toEqual(before[role].ranges);}
});
