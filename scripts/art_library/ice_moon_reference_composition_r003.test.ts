...
for (const id of ["ice-moon-1", "ice-moon-2"]) {
  ...
  it("retains shaft and gorge native surfaces and optical channels identically across LODs", () => {
    const a = composeIceMoonReference(kit, 38, 0),
      b = composeIceMoonReference(kit, 38, 2);
    ...
    expect(ids.size).toBe(id.endsWith("-1") ? 11 : 9);
  ...
  it("NullEngine clears each distinct native floor through variant-specific substrate indexing", () => {
    for (const mode of [true, "gorge"] as const) {
      ...
      try {
        const meshes = composeIceMoonReference(kit, 38, 0, mode).flatMap(
          (b, i) =>
            b.ranges.map((range) => {
        ...
        const ground = hits.find(
          (v) => v.mesh.metadata.partId === `planets--${id}/ground`,
        )!;
        expect(ground.hit.distance - hits[0].hit.distance).toBeGreaterThan(
          0.005,
        );
      } finally {
        scene.dispose();
...
it("keeps the large crater floor exposed in all18 seeded glacial moon placements", () => {
  ...
  try {
    for (const id of ["ice-moon-1", "ice-moon-2"]) {
      ...
      const meshes = composeIceMoonReference(kit, 38, 0).map((b, i) => {
      ...
      for (let i = 0; i < kit.compositionRecipe.scales.length; i++) {
        const y = 1 - (2 * (i + 0.5)) / kit.compositionRecipe.scales.length,
          ...
          hits = meshes
            ...
            .filter((v) => v.hit.hit && v.hit.pickedPoint)
            .sort((a, b) => a.hit.distance - b.hit.distance);
        expect(hits[0].mesh.metadata.materialRole, `${id}/crust-${i}`).toBe(4);
      }
...
for (const id of ["ice-moon-1", "ice-moon-2"])
  for (const seed of [38, 917])
    it(`matches every frozen output byte and placement range for ${id} seed${seed}`, () => {
      const kit = JSON.parse(
        readFileSync(
          `output/playwright/planet-reference-20260914/${id}-r002/kit.json`,
          "utf8",
      ...
      const before = frozen(kit, seed, 0),
        after = composeIceMoonReference(kit, seed, 0);
      for (let role = 0; role < before.length; role++) {
        for (const field of ["positions", "normals", "uvs", "indices"] as const)
          expect(
            Buffer.from(before[role][field].buffer).equals(
              Buffer.from(after[role][field].buffer),
            ),
            `${id}/${seed}/${role}/${field}`,
          ).toBe(true);
        expect(after[role].ranges).toEqual(before[role].ranges);
      }
    });
