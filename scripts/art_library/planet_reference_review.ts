...
const camera=new ArcRotateCamera('review',Math.atan2(4.3,2.45),Math.acos(2.1/Math.hypot(2.45,4.3,2.1)),referenceInitialRadius(query),Vector3.Zero(),scene);camera.fov=.52;camera.minZ=.01;camera.maxZ=10000;camera.attachControl(canvas,true);
...
const candidate=query.has("candidate")?await createReferenceCandidate(scene,seed,query.has('prepareShadows')?(mesh,nextFrame,opaqueRefraction)=>candidateShadows.prepare(mesh,nextFrame,{opaqueRefraction}):undefined):undefined;
