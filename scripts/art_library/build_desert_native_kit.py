...
palette = [
    ...
    ('mesa-rust', (.32,.080,.042), .84),
    ('canyon-burgundy', (.105,.023,.040), .88),
    ('mesa-cap', (.70,.31,.14), .83),
...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    rings = [(-.18,1.03),(height*.54,1.03),(height*.54,1.00),(height-.035,1.00),(height,.96)]
    verts=[]
    ...
    for ring in range(len(rings)-1):
        for j in range(n):
            ...
            roles.append((1 if pale else 2) if ring==1 else (5 if pale else 4) if ring==3 else (2 if j%4 in (0,1) else 3))
    ...
    obj['role']='planet'
    obj['authoring']='native-desert-r001'
    return obj
...
for name,layout in layouts:
    parts=[pillar(name+'-'+str(i),*values,pale=name=='sand-plateau') for i,values in enumerate(layout)]
