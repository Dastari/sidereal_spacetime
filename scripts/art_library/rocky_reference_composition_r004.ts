...
export function composeRockyReference(
  ...
  if (!diagnostic) {
    ...
    for (let i = 0; i < 7; i++) {
      const a = macroDirections[i],
        rotation = (phase - Math.PI) * 0.075 + (random() - 0.5) * 0.05,
        n = unit([
          a[0] * Math.cos(rotation) - a[2] * Math.sin(rotation),
          a[1] + (random() - 0.5) * 0.08,
          a[0] * Math.sin(rotation) + a[2] * Math.cos(rotation),
        ]);
      anchors.push(n);
      place(
