...
import { composeToxicFog } from "./toxic_fog_composition_r004";
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/toxic-fog-r004/kit.json",
    "utf8",
  ),
);
...
it("keeps three closed density layers bounded with air-like IOR and decreasing fringe opacity", () => {
  expect(kit.materials.map((m: { alpha: number }) => m.alpha)).toEqual([
    0.3, 0.56, 0.85,
  ]);
  ...
  for (const b of batches) {
    expect(b.ranges.map((r) => r.partId)).toEqual(
      batches[0].ranges.map((r) => r.partId),
    );
    expect(
