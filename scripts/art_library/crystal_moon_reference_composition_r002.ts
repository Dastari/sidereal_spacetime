...
export function composeCrystalMoonReference(
  ...
  if (
    ...
    ![6, 10].includes(recipe.scales.length)
  )
  ...
  if (!diagnostic) {
    const terrain = batches.map((b) => ({
      ...b,
      indices: b.ranges
        .filter(
          (r) => r.partId.endsWith("/ground") || r.partId.includes("/region-"),
        )
        .flatMap((r) =>
          b.indices.slice(
            r.firstTriangle * 3,
            (r.firstTriangle + r.triangleCount) * 3,
          ),
        ),
    }));
    const surface = nativeRadialSurface(
        terrain,
        terrain.map((_, i) => i),
      ),
      second = recipe.referenceId.endsWith("-2"),
      count = second ? 64 : 88;
    for (let i = 0; i < count; i++) {
      ...
      for (const region of regions) {
        if (
          Math.hypot(...n.map((v, k) => v - region.n[k])) <
          region.scale * 0.6
        ) {
          cavity = true;
          break;
        ...
        const east = unit(
            cross(
              Math.abs(region.n[1]) > 0.94 ? [1, 0, 0] : [0, 1, 0],
              region.n,
            ),
          ),
          north = cross(region.n, east),
        ...
        if (Math.hypot(...n.map((v, k) => v - pit[k])) < region.scale * 0.36) {
          cavity = true;
          break;
        }
      }
      if (cavity) continue;
      const height = surface(n),
        east = unit(cross(Math.abs(y) > 0.94 ? [1, 0, 0] : [0, 1, 0], n)),
        north = cross(n, east),
        angle = random() * Math.PI * 2,
        c = Math.cos(angle),
        s = Math.sin(angle),
        scale = second ? 0.28 + random() * 0.1 : 0.22 + random() * 0.065;
      if (!Number.isFinite(height))
        throw new Error(
          "Native crystalline district lacks final terrain support",
        );
      emit(
        second ? "angular-crystal-district" : "squat-crystal-district",
        `native-crystal-district-${i}`,
        (v) => {
          const x = (v[0] * c - v[1] * s) * scale,
            z = (v[0] * s + v[1] * c) * scale,
            h = height + v[2] * scale - 0.008;
