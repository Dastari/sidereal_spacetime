...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/temperate-r003/kit.json','utf8')) as NativePlanetKit;
it('retains continental terrain forest and material attributes at every LOD',()=>{
 ...
 expect(extract(a)).toEqual(extract(b));const ids=new Set(a.flatMap(b=>b.ranges.map(r=>r.partId)));expect([...ids].filter(id=>id.startsWith('continent-'))).toHaveLength(14);expect([...ids].filter(id=>id.startsWith('forest-'))).toHaveLength(196);expect(a.reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(251000);
 const ocean=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ocean-r007/kit.json','utf8'));expect(kit.materials.slice(0,ocean.materials.length)).toEqual(ocean.materials);
});
