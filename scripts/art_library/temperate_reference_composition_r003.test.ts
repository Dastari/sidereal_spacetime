...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/temperate-r003/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
it("retains continental terrain forest and material attributes at every LOD", () => {
  ...
  expect(a.reduce((n, b) => n + b.indices.length / 3, 0)).toBeLessThan(251000);
  ...
  expect(kit.materials.slice(0, ocean.materials.length)).toEqual(
    ocean.materials,
  );
});
