...
const kit = JSON.parse(
  readFileSync(
    "output/playwright/planet-reference-20260914/ocean-r002/kit.json",
    "utf8",
  ),
) as NativePlanetKit;
...
it("places grove roots above water and keeps a conservative island-footprint bound", () => {
  ...
  expect(trunks.ranges).toHaveLength(18);
  for (const r of trunks.ranges) {
    ...
    for (let i = 0; i < p.length; i += 3)
      expect(Math.hypot(p[i], p[i + 1], p[i + 2])).toBeGreaterThan(1.045);
  }
