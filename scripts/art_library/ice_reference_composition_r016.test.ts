...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ice-r016/kit.json','utf8')) as NativePlanetKit;
...
it('preserves exact authored UV seams by native material and refuses missing optical attributes',()=>{
 const composed=composeIceReference(kit,38,0,true),native=kit.variants.find(v=>v.name==='snow-shaft-region-a')! as NativePlanetKit['variants'][number]&{uvs:number[]};
