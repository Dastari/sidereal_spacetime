...
try {
  ...
  // Private (0600) owner token for isolated client screenshots of the boarded ship.
  writeFileSync(join(evidenceDirectory, "ship-wipe-owner.json"), JSON.stringify({ token: owner.token, characterId: actorX.id, shipId }), { mode: 0o600 });
} finally {
  x.disconnect();
}
process.exit(0);
