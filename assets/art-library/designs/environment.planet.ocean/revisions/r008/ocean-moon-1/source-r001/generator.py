...
palette=[('planets--ocean-moon-1-crust', (0.31, 0.4, 0.49), 0.91), ('planets--ocean-moon-1-rim', (0.65, 0.73, 0.77), 0.87), ('planets--ocean-moon-1-cavity', (0.018, 0.031, 0.045), 0.97), ('planets--ocean-moon-1-fracture-wall', (0.07, 0.13, 0.2), 0.92), ('planets--ocean-moon-1-cool-fault', (0.17, 0.34, 0.39), 0.91), ('planets--ocean-moon-1-pale-fracture', (0.78, 0.82, 0.8), 0.85), ('planets--ocean-moon-1-green-cap', (0.2, 0.4, 0.035), 0.9), ('planets--ocean-moon-1-sunlit-green-cap', (0.48, 0.6, 0.1), 0.88), ('planets--ocean-moon-1-blue-mineral-cover', (0.055, 0.3, 0.44), 0.88)]
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-planets--ocean-moon-1-r001'
 return obj
...
for form_name,parts in forms:
 ...
 for obj in parts:
  for face in obj.data.polygons:
   ...
   if field > 0.6599999999999999:
    face.material_index=7 if q.y>.74 or (q.x<-.95 and q.y<.1) else 6
   elif field > 0.6599999999999999-.20 and q.x>.30:face.material_index=8
...
for name,parts in forms:
 ...
 for gi,(x,y) in enumerate([(-1.25,.48),(-1.02,.64),(-1.40,.76)][:1]):
  best=-10
  for obj in parts:
...
kit={'schema':'sidereal.native-planet-kit.v1','layout':'hybrid-moon-craters','coveredReferenceIds':['planets--ocean-moon-1'],'compositionRecipe':{'referenceId': 'planets--ocean-moon-1', 'layoutSeed': 1069, 'scales': [0.4, 0.38, 0.42, 0.39, 0.41, 0.38, 0.43, 0.39, 0.4, 0.37, 0.42, 0.4], 'variantOffset': 1},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
