...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/ocean-r005/kit.json','utf8')) as NativePlanetKit;
...
it('keeps complete placement ranges, finite native CCW geometry and a bounded scene',()=>{
 for(const level of levels){expect(level.reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(111000);
...
it('widens native tangent footprints 1.4x while preserving every radial height and triangle identity',()=>{
 ...
 for(let i=0;i<next.length;i++){
  const a=next[i],b=previous[i];expect(a.indices).toEqual(b.indices);expect(a.ranges).toEqual(b.ranges);
  for(const range of a.ranges)for(let j=range.firstTriangle*9;j<(range.firstTriangle+range.triangleCount)*9;j+=3){
   expect(Math.hypot(a.positions[j],a.positions[j+1],a.positions[j+2])).toBeCloseTo(Math.hypot(b.positions[j],b.positions[j+1],b.positions[j+2]),6);
   if(range.partId==='ground')expect(a.positions.slice(j,j+3)).toEqual(b.positions.slice(j,j+3));
 ...
 const phase=((Math.imul(38,1664525)+1013904223)>>>0)/4294967296*Math.PI*2;
 const y=1-1/12,r=Math.sqrt(1-y*y),n=[Math.cos(phase)*r,y,Math.sin(phase)*r];
 const tangent=(p:Float32Array,j:number)=>{const dot=p[j]*n[0]+p[j+1]*n[1]+p[j+2]*n[2];return [p[j]/dot-n[0],p[j+1]/dot-n[1],p[j+2]/dot-n[2]];};
 let tested=0;
 for(let i=0;i<next.length;i++)for(const range of next[i].ranges.filter(r=>r.partId==='island-0'||r.partId.startsWith('grove-0-'))){
  for(let j=range.firstTriangle*9;j<(range.firstTriangle+range.triangleCount)*9;j+=3){
   const a=tangent(next[i].positions,j),b=tangent(previous[i].positions,j);
   for(let k=0;k<3;k++)expect(a[k]).toBeCloseTo(b[k]*1.4,6);tested++;
  }
 }
...
it('NullEngine resolves a widened native plateau ray hit to its stable island placement at every LOD',()=>{
 ...
 try {
  for(const level of levels){
   const b=level[6],range=b.ranges.find(r=>r.partId==='island-0')!;
   ...
   const data=new VertexData();data.positions=b.positions;data.normals=b.normals;data.indices=b.indices;data.applyToMesh(mesh);mesh.computeWorldMatrix(true);
