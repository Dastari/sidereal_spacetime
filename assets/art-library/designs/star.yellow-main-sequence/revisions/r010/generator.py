...
spots=[(Vector(v).normalized(),r) for v,r in [((.22,-.70,.70),.17),((-.54,-.82,.05),.21),((.68,-.65,-.35),.16),((.92,-.27,.05),.13),((-.75,.5,.4),.20),((.15,.95,-.1),.21),((-.6,-.4,-.7),.13)]]
...
# Coherent native material regions: neighbouring relief faces share plasma
# structures instead of independent random bright/dark assignments. Geometry
# and dark-complex locations retain the r007 sculpt.
for face in body.data.polygons:
 c=face.center.normalized();d=min((c-p).length/r for p,r in spots)
 d+=.12*math.sin(c.x*39+c.z*22)+.09*math.cos(c.y*43-c.z*31)
 flow=math.sin(c.x*11+2*math.sin(c.z*6))+math.sin(c.y*13+1.7*math.sin(c.x*7))+.55*math.sin(c.z*17+c.y*4)
 branch=math.sin(c.z*14+2*math.sin(c.y*8))+.7*math.sin(c.x*16-c.y*5)
 active=c.z>.06 or c.x>.48
 hot=(abs(flow)<(.145 if active else .055) and (active or math.sin(c.z*19+c.y*11)>.3)) or (abs(branch)<.075 and flow>.15 and active)
 grain=math.sin(c.x*57+c.z*17)*math.sin(c.y*61-c.x*13)+.4*math.cos(c.z*73+c.y*11)
 face.material_index=3 if d<.58 else 4 if d<.91 else 5 if hot or (.97<d<1.015 and grain>.1) else 1 if grain>.1 or abs(flow)<.25 else 0 if grain>-.65 else 2
