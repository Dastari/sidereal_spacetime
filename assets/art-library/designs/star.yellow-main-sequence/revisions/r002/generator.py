...
def mat(name,c,emission,strength):
 ...
 return m
mats=[mat('Golden photosphere',(.95,.46,.015),(1,.48,.018),.9),mat('Yellow raised granules',(1,.7,.04),(1,.66,.025),1.3),mat('Amber relief sides',(.55,.16,.002),(.95,.20,.001),.45),mat('Sunspot umbra',(.038,.009,.001),(.05,.009,.001),.22),mat('Sunspot penumbra',(.26,.065,.001),(.34,.06,.001),.4),mat('White yellow hot fissures',(1,.88,.32),(1,.81,.19),3.6),mat('Flare golden core',(1,.58,.035),(1,.47,.012),4.5),mat('Flare orange edge',(1,.20,.002),(1,.15,.001),1.8)]
 ...
 for sign in (-1,1):
  for j in range(N):
   for i in range(N):
    ...
    d=min((center-p).length/r for p,r in spots);noise=rng.random()
    role=3 if d<.68+noise*.22 else 4 if d<1.05+noise*.12 else 1 if noise>.43 else 0
    top=.984+rng.random()*.041 if role not in (3,4) else .962+rng.random()*.012
    corners=[]
    for du,dv in [(-1,-1),(1,-1),(1,1),(-1,1)]:
     q=Vector((0,0,0));q[axis]=sign;q[(axis+1)%3]=u+du*.965/N;q[(axis+2)%3]=v+dv*.965/N;corners.append(q.normalized())
    if sign<0:corners.reverse()
    tile(corners,top,.949,role,f'photosphere-{axis}-{sign}-{i}-{j}')
...
body=mesh('golden-photosphere',vertices,faces,mats,mi)
body['trianglePlacementRanges']=json.dumps(ranges,separators=(',',':'))
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=4,radius=.952);core=bpy.context.object;core.name='GEO-hot-fissure-interior';core.data.materials.append(mats[5]);core['role']='star';core['partId']='hot-fissure-interior'
# Short, tapered, bent prominences. Local Z is outward, rooted at the photosphere.
for i in range(24):
 phi=i*math.pi*(3-math.sqrt(5));z=1-2*(i+.5)/24;n=Vector((math.sqrt(1-z*z)*math.cos(phi),math.sqrt(1-z*z)*math.sin(phi),z))
 vs=[];fs=[];length=rng.uniform(.075,.20);width=rng.uniform(.016,.035)
 for k in range(6):
...
s.world.color=(.003,.003,.003);s.render.engine='CYCLES';s.cycles.samples=32;s.cycles.use_denoising=False;s.render.resolution_x=768;s.render.resolution_y=768;s.render.resolution_percentage=100;s.view_settings.view_transform='AgX'
...
(out/'validation.json').write_text(json.dumps({'schema':1,'files':files,'photosphereRadius':1,'meshes':sum(o.type=='MESH' for o in s.objects),'polygons':sum(len(o.data.polygons) for o in s.objects if o.type=='MESH'),'flareCount':24,'authoredPBR':True,'role':'star','seed':3901},indent=2)+'\n')
