...
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=4,radius=.963);core=bpy.context.object;core.name='GEO-hot-fissure-interior';core.data.materials.append(mats[5]);core['role']='star';core['partId']='hot-fissure-interior'
# Short, tapered, bent prominences. Local Z is outward, rooted at the photosphere.
for i in range(70):
 phi=i*math.pi*(3-math.sqrt(5));z=1-2*(i+.5)/70;n=Vector((math.sqrt(1-z*z)*math.cos(phi),math.sqrt(1-z*z)*math.sin(phi),z))
 vs=[];fs=[];length=rng.uniform(.025,.15);width=rng.uniform(.013,.028)
 for k in range(6):
...
(out/'validation.json').write_text(json.dumps({'schema':1,'files':files,'photosphereRadius':1,'meshes':sum(o.type=='MESH' for o in s.objects),'polygons':sum(len(o.data.polygons) for o in s.objects if o.type=='MESH'),'flareCount':70,'authoredPBR':True,'role':'star','seed':3901},indent=2)+'\n')
