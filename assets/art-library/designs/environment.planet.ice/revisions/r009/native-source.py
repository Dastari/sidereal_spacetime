...
colors=[(.78,.86,.97),(.20,.43,.76),(.004,.085,.42),(.005,.27,.72),(.002,.027,.11)]
materials=[]
...
for variant in range(6):
 ...
 if variant==5:
  for j in range(22):
   ...
   ob=ice_shaft('GEO-broad-blue-outcrop-%d'%j,offset+Vector((x,y,height/2-.035)),rng.uniform(.04,.085),rng.uniform(.035,.075),height,rng);ob.data.materials.append(materials[3 if j%3 else 2]);objects.append(ob)
