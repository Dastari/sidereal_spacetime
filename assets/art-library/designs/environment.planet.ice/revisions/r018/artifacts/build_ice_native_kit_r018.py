"""Native glacial shafts under thick irregular snow. Isolated r018 only.
Run: blender -b --python scripts/art_library/build_ice_native_kit_r018.py -- NEW_OUTPUT
"""
...
def mesh_object(name,verts,faces,roles,smooth=None):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=1,use_grid_fill=True);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-ice-r018';return o
...
def buttress(name,x,y,r,h,phase,sides=5):
 ...
 for z,scale in[(-.62,1.04),(h-.052,1),(h-.038,1.13),(h,1.08)]:
  for i in range(n):
   ...
   verts.append((x+math.cos(a)*r*scale*f,y+math.sin(a)*r*scale*.90*f,z))
...
def snow_shelf(name,outline,z,thickness):
 ...
 return mesh_object(name,verts,faces,roles)
parts=[region,
 buttress('hero-blue-column',.77,-.43,.20,1.01,.15,5),
 buttress('blue-column-companion',1.02,-.17,.17,.75,.42,6),
 buttress('blue-column-short',.59,-.75,.19,.54,.70,4),
...
for name,subdivisions in[('ground-sphere',5),('ground-sphere-medium',4),('ground-sphere-low',3)]:
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions,radius=1);o=bpy.context.object;o.name='GEO-'+name;o.data.materials.append(materials[0]);o['role']='planet';o['authoring']='native-ice-r018'
...
validation['sha256']={p.name:hashlib.sha256(p.read_bytes()).hexdigest()for p in out.iterdir()if p.is_file()};(out/'validation.json').write_text(json.dumps(validation,indent=2));print('ICE_R018_DONE',validation['variants'])
