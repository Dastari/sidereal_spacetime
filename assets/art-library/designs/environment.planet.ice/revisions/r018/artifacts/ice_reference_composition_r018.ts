...
export function composeIceReference(kit:NativePlanetKit,_seed:number,_lod:0|1|2,_diagnostic=true){
 ...
 const regions=[{n:unit([.452,.388,.794]),scale:.28,angle:0,name:'snow-cut-region',id:'region-diagnostic'}];
 ...
 const substrate=(v:Vec):Vec=>{
  ...
  for(const region of regions){
   ...
   if(dot<=0)continue;
   const x=(n[0]*east[0]+n[1]*east[1]+n[2]*east[2])/dot/region.scale,y=(n[0]*north[0]+n[1]*north[1]+n[2]*north[2])/dot/region.scale;
   ...
   if(Number.isFinite(floor))depression=Math.max(depression,region.scale*Math.max(0,.12-floor));
