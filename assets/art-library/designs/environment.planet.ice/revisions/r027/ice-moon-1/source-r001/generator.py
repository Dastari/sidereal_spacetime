...
def mesh_object(name,verts,faces,roles,smooth=None):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=1,use_grid_fill=True);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-planets--ice-moon-1-r001';return o
...
for fi,(region_name,region_parts)in enumerate(forms):
 ...
 for k in range(180):
  ...
  obj['role']='planet';obj['authoring']='native-planets--ice-moon-1-r001';region_parts.append(obj)
...
ground_districts=[]
for i in range(28):
 z=1-2*(i+.5)/28;theta=i*2.399963229728653+.37;r=math.sqrt(1-z*z);n=Vector((math.cos(theta)*r,math.sin(theta)*r,z));east=n.cross(Vector((0,0,1)) if abs(z)<.94 else Vector((0,1,0))).normalized();north=n.cross(east)
 width=[.29,.22,.34,.27,.30][i%5];depth=[.27,.33,.23,.30][i%4];height=[.052,.076,.037,.063][i%4]
...
for name,subdivisions in[('ground-sphere',5),('ground-sphere-medium',4),('ground-sphere-low',3)]:
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions,radius=.982);o=bpy.context.object;o.name='GEO-'+name;o.data.materials.append(materials[2]);o['role']='planet';o['authoring']='native-planets--ice-moon-1-r001'
 for p in o.data.polygons:p.use_smooth=True
 forms.append((name,[o,*ground_districts]))
kit={'schema':'sidereal.native-planet-kit.v1','layout':'ice-moon-glacial','coveredReferenceIds':['planets--ice-moon-1'],'compositionRecipe':{'referenceId': 'planets--ice-moon-1', 'layoutSeed': 1297, 'scales': [0.34, 0.29, 0.35, 0.3, 0.33, 0.28, 0.34, 0.31], 'variantOffset': 1},'uvConvention':'glTF corner UV, Blender V flipped; invertY=false','materials':definitions,'variants':[]}
