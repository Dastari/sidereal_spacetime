import {deformedNormalAt} from '/root/sidereal_spacetime/scripts/art_library/native_deformation_normals';
import type {NativePlanetKit} from '/root/sidereal_spacetime/packages/render/src/environment/native-planet-composition';
...
export function composeIceMoonReference(kit:NativePlanetKit,seed:number,_lod:0|1|2,diagnostic:boolean|'gorge'=false){
 const recipe=(kit as NativePlanetKit&{compositionRecipe?:{referenceId:string;layoutSeed:number;scales:number[]}}).compositionRecipe;
 ...
 const work={substrate:0,floorQueries:0,floorTriangles:0};
 (globalThis as any).__iceWork=work;
 ...
 const floorAt=(x:number,y:number)=>{work.floorQueries++;
  let floor=Infinity;
  for(const {a,b,c,den} of cells.get(`${Math.floor(x/cellSize)},${Math.floor(y/cellSize)}`)||[]){
   work.floorTriangles++;const u=((b[1]-c[1])*(x-c[0])+(c[0]-b[0])*(y-c[1]))/den,v=((c[1]-a[1])*(x-c[0])+(a[0]-c[0])*(y-c[1]))/den,w=1-u-v;
 ...
 const substrate=(v:Vec):Vec=>{work.substrate++;
