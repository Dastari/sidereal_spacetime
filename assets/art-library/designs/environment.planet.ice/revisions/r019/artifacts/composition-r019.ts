...
export function composeIceReference(kit:NativePlanetKit,seed:number,_lod:0|1|2,diagnostic=false){
 ...
 const regions=diagnostic?[{n:unit([.452,.388,.794]),scale:.28,angle:0,name:'snow-cut-region',id:'region-diagnostic'}]:Array.from({length:12},(_,i)=>{const y=1-2*(i+.5)/12,r=Math.sqrt(1-y*y),theta=phase+i*2.39996323;return {n:[Math.cos(theta)*r,y,Math.sin(theta)*r] as Vec,scale:[.35,.32,.38,.33][i%4],angle:random()*Math.PI*2,name:'snow-cut-region',id:`region-${i}`};});
