...
export function composeRockyReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 if(diagnostic)regions.push({n:unit([.452,.388,.794]),scale:.32,angle:0,name:'battered-region-a',id:'region-diagnostic'});
  ...
  regions.push({n:[Math.cos(angle)*r,y,Math.sin(angle)*r],scale:[.43,.38,.42,.37,.44,.39,.41,.38][i],angle:random()*Math.PI*2,name:i%2?'battered-region-b':'battered-region-a',id:`region-${i}`});
 }
