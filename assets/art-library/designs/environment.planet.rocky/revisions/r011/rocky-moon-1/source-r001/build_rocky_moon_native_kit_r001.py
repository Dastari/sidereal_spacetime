...
Run: blender -b --python scripts/art_library/build_rocky_moon_native_kit_r001.py -- NEW_OUTPUT
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-rocky-moon-r001'
 return obj
...
def inset_pit_district(name):
 ...
 return mesh_object(name,verts,faces,roles)
# Explicit lunar forms: retain the proven closed deep bowl, remove the main
# world's orange seam and broad district hierarchy. Short irregular lip shelves
# interrupt crater edges without crossing the opening.
for variant,phase in enumerate([0,1.9]):
 name='lunar-impact-'+('a'if variant==0 else'b');parts=[battered_region(name,phase)]
 for i,(x,y,sx,sy,h)in enumerate([(-.67,.35,.46,.39,.10),(.32,.62,.36,.29,.075),(-.26,-.67,.39,.24,.08)]):
  parts.append(shelf(name+'-broken-lip-'+str(i),x,y,sx,sy,h))
 for obj in parts:
...
forms=[(name,parts) for name,parts in forms if name.startswith('lunar-impact-')]
for obj in list(bpy.data.objects):
 if obj.type=='MESH' and not any(obj in parts for _,parts in forms):bpy.data.objects.remove(obj,do_unlink=True)
...
kit={'schema':'sidereal.native-planet-kit.v1','layout':'rocky-moon-craters','coveredReferenceIds':['planets--rocky-moon-1'],'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
...
for role in range(6):
 image=bpy.data.images.new('RockyMoon1-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('RockyMoon1-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];roughpixels=[]
 for y in range(256):
  for x in range(256):
