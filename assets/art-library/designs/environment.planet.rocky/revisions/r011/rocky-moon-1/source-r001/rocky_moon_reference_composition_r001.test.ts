...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/rocky-moon-r001/kit.json','utf8')) as NativePlanetKit;
it('retains complete native regional surfaces UVs normals and identities at every LOD',()=>{
 const a=composeRockyMoonReference(kit,38,0),b=composeRockyMoonReference(kit,38,2);
...
it('NullEngine sees embedded deep crater floor before the cleared native substrate',()=>{
 ...
 try{const meshes=composeRockyMoonReference(kit,38,0,true).map((b,i)=>{const mesh=new Mesh(`rocky-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial(`rocky-role-${i}`,scene);const data=new VertexData();data.positions=b.positions;data.normals=b.normals;data.uvs=b.uvs;data.indices=b.indices;data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
...
it('rejects missing authored optical attributes',()=>{expect(()=>composeRockyMoonReference({...kit,variants:kit.variants.map(v=>({...v,normals:undefined}))},38,0)).toThrow('Missing native rocky normals or UVs');});
