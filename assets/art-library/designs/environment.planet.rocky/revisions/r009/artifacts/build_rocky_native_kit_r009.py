...
Run: blender -b --python scripts/art_library/build_rocky_native_kit_r009.py -- NEW_OUTPUT
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-rocky-r009'
 return obj
...
def cut_small_pit(obj,x,y,r,depth):
 ...
 bpy.ops.mesh.primitive_cylinder_add(vertices=8,radius=r,depth=depth+.12,location=(x,y,top+.06-depth/2))
...
for variant,phase in enumerate([0,1.9]):
 ...
 parts.append(plate(name+'-warm-inset-mineral',[(-1.78,-.31),(-1.23,-.18),(-1.09,-.02),(-.78,.11),(-.75,.21),(-1.18,.08),(-1.34,-.07),(-1.81,-.19)],.22,4))
 parts.append(plate(name+'-warm-inset-mineral-rise',[(-1.23,-.18),(-1.10,-.18),(-.95,-.46),(-.76,-.61),(-.82,-.71),(-1.06,-.49)],.25,4))
...
for role in range(6):
 image=bpy.data.images.new('Rocky9-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('Rocky9-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];roughpixels=[]
 for y in range(256):
  for x in range(256):
