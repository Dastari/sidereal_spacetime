...
const ids=['rocky-moon-2','desert-moon-1','desert-moon-2','gas-giant-moon-1','gas-giant-moon-2','gas-giant-moon-3'];
for(const id of ids){
...
it('retains complete native regional surfaces UVs normals and identities at every LOD',()=>{
 const a=composeSolidMoonReference(kit,38,0),b=composeSolidMoonReference(kit,38,2);
...
it('NullEngine sees embedded deep crater floor before the cleared native substrate',()=>{
 ...
 try{const meshes=composeSolidMoonReference(kit,38,0,true).map((b,i)=>{const mesh=new Mesh(`rocky-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial(`rocky-role-${i}`,scene);const data=new VertexData();data.positions=b.positions;data.normals=b.normals;data.uvs=b.uvs;data.indices=b.indices;data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
...
it('rejects missing authored optical attributes',()=>{expect(()=>composeSolidMoonReference({...kit,variants:kit.variants.map(v=>({...v,normals:undefined}))},38,0)).toThrow('Missing native rocky normals or UVs');});

}
it('keeps the large crater floor exposed in all72 seeded regional placements',()=>{
 const engine=new NullEngine(),scene=new Scene(engine);scene.useRightHandedSystem=true;
 try{for(const id of ids){const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r001/kit.json`,'utf8'));const meshes=composeSolidMoonReference(kit,38,0).map((b,i)=>{const mesh=new Mesh(`${id}-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial('moon-role',scene);const data=new VertexData();Object.assign(data,b);data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
