...
import {composeToxicReference} from './toxic_reference_composition_r003';
import type {NativePlanetKit} from '../../packages/render/src/environment/native-planet-composition';
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/toxic-r003/kit.json','utf8')) as NativePlanetKit;
it('retains native corroded crust geometry, corner attributes and placement identities across all LODs',()=>{
 ...
 expect(levels[0].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(90000);
...
it('keeps a recessed liquid sample visible in every seeded full-body region',()=>{
 ...
 try{
  ...
  for(let i=0;i<12;i++){
   const y=1-2*(i+.5)/12,angle=phase+i*2.39996323,r=Math.sqrt(1-y*y),n=new Vector3(Math.cos(angle)*r,y,Math.sin(angle)*r),east=Vector3.Cross(Math.abs(n.y)>.94?Vector3.Right():Vector3.Up(),n).normalize(),north=Vector3.Cross(n,east),rotation=random()*Math.PI*2,scale=[.36,.30,.40,.33][i%4];
