...
def pillar(name, x, y, width, depth, height, rotation=0, pale=False):
    ...
    obj['role']='planet'
    obj['authoring']='native-toxic-r006'
    return obj
...
def cliff_solid(name, footprint, top, bottom=-.24, cap=0):
    ...
    for mat in materials:mesh.materials.append(mat)
    for poly,role in zip(mesh.polygons,roles):poly.material_index=role
    obj['role']='planet';obj['authoring']='native-toxic-r006'
    return obj
...
for role in [0,1,2,3,4,5]:
 ...
 for kind in ['albedo','normal','orm']:
  image=bpy.data.images.new(f'Toxic6-corrosion-{role}-{kind}',width=size,height=size,alpha=True);image.colorspace_settings.name='sRGB'if kind=='albedo'else'Non-Color';pixels=np.ones((size,size,4),dtype=np.float32)
  if kind=='albedo':
