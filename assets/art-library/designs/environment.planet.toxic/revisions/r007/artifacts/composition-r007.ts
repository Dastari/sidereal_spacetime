...
export function composeToxicReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 const batches=kit.materials.map(()=>({positions:[] as number[],normals:[] as number[],uvs:[] as number[],indices:[] as number[],ranges:[] as Range[]}));
 const variant=(name:string)=>{const value=kit.variants.find(v=>v.name===name);if(!value)throw new Error(`Missing toxic native variant ${name}`);return value;};
 ...
 function emit(name:string,id:string,transform:(v:Vec)=>Vec,_smooth=false){
  ...
  if(mesh.normals?.length!==mesh.positions.length||mesh.uvs?.length!==mesh.positions.length/3*2)throw new Error('Missing authored toxic normals or UVs');
 ...
 if(diagnostic)regions.push({n:unit([.452,.388,.794]),scale:.28,angle:0,name:'toxic-basin-group-a',id:'region-diagnostic'});
 else for(let i=0;i<12;i++){
  const y=1-2*(i+.5)/12,angle=phase+i*2.39996323,r=Math.sqrt(1-y*y);
  regions.push({n:[Math.cos(angle)*r,y,Math.sin(angle)*r],scale:[.43,.37,.46,.39][i%4],angle:random()*Math.PI*2,name:['toxic-basin-group-a','toxic-basin-group-b','toxic-basin-group-c'][i%3],id:`region-${i}`});
 }
 const floors=new Map(['toxic-basin-group-a','toxic-basin-group-b','toxic-basin-group-c'].map(name=>[name,nativeSurfaceFloor(variant(name))]));
 const sourceGround=variant('ground-sphere');let relief=0;for(let i=0;i<sourceGround.positions.length;i+=3)relief=Math.max(relief,Math.hypot(sourceGround.positions[i],sourceGround.positions[i+1],sourceGround.positions[i+2])-1);
 const substrate=(v:Vec):Vec=>{
  const n=unit(v);let depression=0;
  for(const region of regions){
   const dot=n[0]*region.n[0]+n[1]*region.n[1]+n[2]*region.n[2];if(dot<=0)continue;
   const east=unit(cross(Math.abs(region.n[1])>.94?[1,0,0]:[0,1,0],region.n)),north=cross(region.n,east),c=Math.cos(region.angle),s=Math.sin(region.angle);
   const tx=(n[0]*east[0]+n[1]*east[1]+n[2]*east[2])/dot/region.scale,ty=(n[0]*north[0]+n[1]*north[1]+n[2]*north[2])/dot/region.scale;
   const x=tx*c+ty*s,y=-tx*s+ty*c;if(Math.abs(x)>1.85||Math.abs(y)>1.5)continue;
   const floor=floors.get(region.name)!;let z=floor(x,y);
   for(const [dx,dy]of [[.12,0],[-.12,0],[0,.12],[0,-.12]])z=Math.min(z,floor(x+dx,y+dy));
   if(Number.isFinite(z))depression=Math.max(depression,.012+region.scale*(.07-z)+relief);
 ...
 for(const region of regions){
  ...
  emit(region.name,region.id,v=>{
   const x=(v[0]*c-v[1]*s)*region.scale,y=(v[0]*s+v[1]*c)*region.scale,d=unit([n[0]+east[0]*x+north[0]*y,n[1]+east[1]*x+north[1]*y,n[2]+east[2]*x+north[2]*y]),h=1-.012+v[2]*region.scale;
