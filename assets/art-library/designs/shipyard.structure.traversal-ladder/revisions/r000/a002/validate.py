...
for node in g['nodes']:
    ...
    match=Counter(map(triangle_key,actual))==Counter(map(triangle_key,expected[group]))
    checks.append({'name':'Exact authored-to-GLB triangle multiset '+group,'pass':match,'triangles':len(actual),'coordinateToleranceM':1e-6})
...
audit={'schema':'sidereal.native-traversal-audit.v1','adapterId':'shipyard.structure.traversal-ladder','revision':'r000-a002','qualification':{'nativeMeshMatch':all(c['pass'] for c in checks if 'triangle multiset' in c['name']),'physicalApertures':all(c['pass'] for c in checks if 'genuine2m' in c['name']),'capsuleSweepClear':all(c['pass'] for c in checks if 'envelope' in c['name'] or 'landing clear' in c['name']),'guardedLandings':guarded},'body':{'radiusM':.3,'heightM':1.8},'decks':{'lower':{'originZ':0,'walkingZ':.1875},'upper':{'originZ':3.1875,'walkingZ':3.375}},'parts':parts,'apertures':[{'id':'lower-roof-shaft','role':'lower-roof','boundsM':{'min':[2,2,3],'max':[4,4,3.1875]}},{'id':'upper-floor-shaft','role':'upper-floor','boundsM':{'min':[2,2,3.1875],'max':[4,4,3.375]}}],'pathM':measure['pathM'],'corridorFreeBoundsM':measure['corridorFreeBoundsM'],'landings':landings}
