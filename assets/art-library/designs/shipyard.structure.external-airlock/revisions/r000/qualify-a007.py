...
for inner_open,outer_open,expected in [(True,False,1),(False,True,1),(True,True,0)]:
 ...
 if inner_open or outer_open:
  start,end=(1,7) if inner_open and outer_open else (1,4) if inner_open else (4,7)
  body=m.Manifold.cylinder(1.8,.3/np.cos(np.pi/64),circular_segments=64).translate([0,1,.187501])
  swept=m.Manifold.batch_hull([body.translate([start,0,0]),body.translate([end,0,0])])
  worst=(open_structure ^ swept).volume()
  check(f'Accepted open-route body sweep inner={inner_open} outer={outer_open}',worst<1e-10,{'maxIntersectionM3':worst,'radiusM':.3,'heightM':1.8,'pathM':[[start,1],[end,1]],'method':'continuous convex-hull sweep, circumscribed64-sided radius proxy'})
...
out=Path(sys.argv[1]) if len(sys.argv)>1 else ROOT/'.runtime/native-external-airlock-r000/audit-a007.json'
