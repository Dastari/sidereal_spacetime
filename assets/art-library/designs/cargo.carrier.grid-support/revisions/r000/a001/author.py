...
def box(name,lo,hi,mat,col=COL,bevel=.006,parent=None):
 ...
 o=bpy.context.object;o.name='GEO-'+name;o.dimensions=tuple(b-a for a,b in zip(lo,hi));bpy.ops.object.transform_apply(location=False,rotation=False,scale=True)
...
for i,(instance,tag,offset) in enumerate(placements):
 appearance=['standard-small','standard-small-red','standard-small-blue','standard-small-amber'][i%4]
...
print(json.dumps({'carrierAssets':list(visual),'output':str(OUT),'device':'CPU','threads':2,'samples':24}))
