...
# Large native formations replace small repeated top boxes in selected regions.
for variant in range(2):
 offset=Vector(((8+variant)*4.,0,0));parts=[]
 pieces=[(0,0,1.6,1.45,-.15),(-.31,.19,.88,.91,.52),(.27,.23,.77,.65,.92),(.20,-.36,.97,.65,.20)] if variant==0 else [(0,0,1.55,1.4,-.22),(-.40,-.13,.68,.92,.83),(.33,.29,.86,.72,.42),(.13,-.35,.75,.63,.05)]
 for i,(x,y,w,d,h) in enumerate(pieces):
  o=mass('GEO-macro-broken-rock-%d-%d'%(variant,i),offset+Vector((x,y,0)),w,d,h,i);o.rotation_euler.z=.16*math.sin(i*2+variant);parts.append(o)
 forms.append(parts)
offset=Vector((40,0,0));parts=[]
for i in range(11):
 angle=i*math.tau/11;radius=.69+.055*math.sin(i*2.2)
 o=mass('GEO-vent-basin-broken-wall-%d'%i,offset+Vector((math.cos(angle)*radius,math.sin(angle)*radius,0)),.37,.48,.25+.24*math.sin(i*1.9),i);o.rotation_euler.z=angle;parts.append(o)
bpy.ops.mesh.primitive_cylinder_add(vertices=24,radius=.67,depth=.08,location=offset+Vector((0,0,-.58)));o=bpy.context.object;o.name='GEO-recessed-basalt-basin-floor';o.data.materials.append(materials[0]);parts.append(o)
parts.append(block('GEO-narrow-floor-vent',offset+Vector((0,0,-.532)),(.065,.65,.025),5))
bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2,radius=.065,location=offset+Vector((.01,.12,-.52)));o=bpy.context.object;o.name='GEO-point-hot-vent';o.scale=(.8,1,.35);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);o.data.materials.append(materials[5]);parts.append(o)
forms.append(parts)
for parts in forms:
 for o in parts:
