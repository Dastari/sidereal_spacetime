...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/volcanic-r019/kit.json','utf8')) as NativePlanetKit;
it('retains every regional surface and placement at all LODs within a bounded triangle budget',()=>{
 ...
 const geometry=(batch:typeof levels[number][number])=>batch.ranges.filter(r=>r.partId!=='volcanic-ground').map(r=>({id:r.partId,positions:Buffer.from(batch.positions.slice(r.firstTriangle*9,(r.firstTriangle+r.triangleCount)*9).buffer).toString('base64'),normals:Buffer.from(batch.normals.slice(r.firstTriangle*9,(r.firstTriangle+r.triangleCount)*9).buffer).toString('base64')}));
 ...
 expect(levels[0].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(127000);
 expect(levels[2].reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(50000);
