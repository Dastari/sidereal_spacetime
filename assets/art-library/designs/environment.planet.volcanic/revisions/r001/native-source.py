...
names=['basalt-dark','basalt-cliff','basalt-top','molten-core','sealed-rock'];colors=[(.012,.019,.039),(.038,.049,.080),(.105,.115,.155),(.35,.018,.001),(.003,.006,.014)];materials=[]
...
for variant in range(3):
 ...
 if variant==1:parts.append(block('GEO-local-basalt-step',offset+Vector((-.17,.12,.46)),(.62,.72,.24),2))
 forms.append(parts)
offset=Vector((12,0,0));forms.append([mass('GEO-buried-volcanic-pillar',offset+Vector((-.15,.04,0)),.95,.94,1.8,0),mass('GEO-attached-short-basalt-column',offset+Vector((.38,-.13,0)),.80,.74,1.05,1)])
...
for index,parts in enumerate(forms):
 ...
 kit['variants'].append({'name':['basalt-a','basalt-b','basalt-c','basalt-pillar','basalt-crater','molten-core','sealing-core'][index],'positions':positions,'indices':indices,'triangleMaterials':roles})
