...
names=['basalt-dark','basalt-cliff','basalt-top','molten-core','sealed-rock','hot-vent'];colors=[(.018,.010,.030),(.040,.025,.052),(.075,.062,.100),(.35,.018,.001),(.003,.006,.014),(.7,.08,.003)];materials=[]
for i,(name,color) in enumerate(zip(names,colors)):
 ...
 if i in (3,5):p.inputs['Emission Color'].default_value=(1,.12,.004,1);p.inputs['Emission Strength'].default_value=3
 materials.append(m)
def mass(name,offset,width,depth,height,variant):
 corners=[(-.66,-.38),(-.43,-.54),(.35,-.60),(.62,-.24),(.55,.43),(.17,.60),(-.47,.49),(-.61,.14)]
 if name.startswith("GEO-basalt-fragment"):corners=[(-.60,-.50),(.42,-.50),(.60,-.28),(.60,.50),(-.60,.50)]
 verts=[]
 for z,factor in [(-.9,1), (height-.008,1),(height,.985)]:
  for x,y in corners:verts.append((x*width*factor,y*depth*factor,z))
 n=len(corners);faces=[tuple(range(n-1,-1,-1)),tuple(range(2*n,3*n))]
...
for variant in range(3):
 offset=Vector((variant*4.,0,0));parts=[]
 parts.append(mass('GEO-basalt-fragment-main-%d'%variant,offset+Vector((-.10,.02,0)),1.12,1.04,[.18,.30,.14][variant],variant))
 parts.append(mass('GEO-basalt-fragment-upper-%d'%variant,offset+Vector((.27,-.13,0)),.61,.69,[.48,.68,.38][variant],variant))
 forms.append(parts)
...
for i in range(9):
 angle=i*math.tau/9;radius=.55+.065*math.sin(i*2.7);x,y=math.cos(angle)*radius,math.sin(angle)*radius
 o=mass('GEO-broken-crater-lip-%d'%i,offset+Vector((x,y,0)),.48,.39,.38+.24*math.sin(i*1.7),i);o.rotation_euler.z=angle+.15*math.sin(i);parts.append(o)
forms.append(parts)
...
# Exact native Blender-authored study surfaces; no regeneration or remeshing.
study_path=Path('/root/sidereal_spacetime/assets/art-library/planet-geology-studies/basalt-forms-lod-01/geometry.json')
study=json.loads(study_path.read_text())
for index,slug in enumerate(['broken-escarpment','fractured-plateau','breached-caldera'],8):
 offset=Vector((index*4.,0,0));g=study[slug]
 me=bpy.data.meshes.new('Native-'+slug);me.from_pydata(g['positions'],[],g['triangles']);me.update()
 for m in materials:me.materials.append(m)
 for polygon,role in zip(me.polygons,g['materialIndices']):polygon.material_index=role
 o=bpy.data.objects.new('GEO-study02-'+slug,me);scene.collection.objects.link(o);o.location=offset;parts=[o]
 if index==10:
  parts.append(block('GEO-narrow-floor-vent',offset+Vector((.025,.0,-.09)),(.038,.24,.018),5))
  bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=2,radius=.032,location=offset+Vector((.02,.07,-.08)));o=bpy.context.object;o.name='GEO-point-hot-vent';o.scale=(.8,1,.35);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);o.data.materials.append(materials[5]);parts.append(o)
 forms.append(parts)
# Replace repetitive prism filler with exact authored study ledges. Preserve native surfaces.
for index,slug in enumerate(['broken-escarpment','fractured-plateau','broken-escarpment']):
 for old in forms[index]:bpy.data.objects.remove(old,do_unlink=True)
 g=study[slug];me=bpy.data.meshes.new('Native-filler-'+slug);me.from_pydata(g['positions'],[],g['triangles']);me.update()
 for m in materials:me.materials.append(m)
 for polygon,role in zip(me.polygons,g['materialIndices']):polygon.material_index=role
 o=bpy.data.objects.new('GEO-native-filler-'+str(index),me);scene.collection.objects.link(o);o.location=(index*4.,0,0);forms[index]=[o]
for parts in forms:
 for o in parts:
...
for index,parts in enumerate(forms):
 ...
 kit['variants'].append({'name':['basalt-a','basalt-b','basalt-c','basalt-pillar','basalt-crater','molten-core','sealing-core','basalt-fall','macro-rock-a','macro-rock-b','vent-basin'][index],'positions':positions,'indices':indices,'triangleMaterials':roles})
