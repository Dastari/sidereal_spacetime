...
palette=[('planets--volcanic-moon-1-charcoal-crust', (0.14, 0.1, 0.18), 0.91), ('planets--volcanic-moon-1-purple-rim', (0.27, 0.2, 0.3), 0.87), ('planets--volcanic-moon-1-dark-cavity', (0.012, 0.008, 0.019), 0.97), ('planets--volcanic-moon-1-cooled-fracture', (0.045, 0.025, 0.061), 0.92), ('planets--volcanic-moon-1-molten-fault', (1.0, 0.11, 0.003), 0.57), ('planets--volcanic-moon-1-pale-purple-fracture', (0.35, 0.24, 0.34), 0.86)]
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-planets--volcanic-moon-1-r003'
 return obj
...
for name,parts in forms:
 ...
 for pi,(points,width) in enumerate(paths):
  ...
  for j,(x,y) in enumerate(points):
   ...
   if False:profile=[.70,1.45,5.5,3.6,1.15,.62] if name.endswith('-a') else [.65,1.05,4.2,2.65,.9,.62]
...
kit={'schema' :'sidereal.native-planet-kit.v1','layout':'volcanic-moon-craters','coveredReferenceIds':['planets--volcanic-moon-1'],'compositionRecipe':{'referenceId': 'planets--volcanic-moon-1', 'layoutSeed': 1523, 'scales': [0.41, 0.38, 0.43, 0.39, 0.4, 0.38, 0.42, 0.39, 0.43, 0.37, 0.41, 0.4], 'variantOffset': 1},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
