...
export function composeVolcanicMoonReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 const recipe=(kit as NativePlanetKit&{compositionRecipe?:{referenceId:string;layoutSeed:number;scales:number[];variantOffset:number;warmEdgeRegions?:number[]}}).compositionRecipe;
 if(!recipe||!['planets--volcanic-moon-1','planets--volcanic-moon-2'].includes(recipe.referenceId)||!Number.isInteger(recipe.layoutSeed)||recipe.scales.length!==12||recipe.scales.some(v=>!Number.isFinite(v)||v<.3||v>.46))throw new Error('Explicit volcanic moon composition recipe required');
 if(recipe.warmEdgeRegions&&(recipe.referenceId!=='planets--volcanic-moon-2'||recipe.warmEdgeRegions.length!==2||recipe.warmEdgeRegions[0]!==2||recipe.warmEdgeRegions[1]!==9))throw new Error('Explicit bounded warm-edge placements required');
 ...
 if(diagnostic)regions.push({n:unit([.452,.388,.794]),scale:.32,angle:0,name:'battered-region-a',id:'region-diagnostic'});
  ...
  regions.push({n:[Math.cos(angle)*r,y,Math.sin(angle)*r],scale:recipe.scales[i],angle:random()*Math.PI*2,name:recipe.warmEdgeRegions?.includes(i)?'battered-region-b-edge':(recipe.referenceId.endsWith('-2')?i%4:(i+recipe.variantOffset)%2)?'battered-region-b':'battered-region-a',id:`${recipe.referenceId}/crust-${i}`});
 }
