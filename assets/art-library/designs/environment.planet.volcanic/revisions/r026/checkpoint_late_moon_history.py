...
SPECS={'volcanic':{'current':25,'pin':23,'min':{'2':6}}}
def sha(path):
 h=hashlib.sha256()
...
def discover(family,spec):
 ...
 for p in sorted(OUT.iterdir()):
  ...
  if p.is_dir() and m and m[1] in spec['min'] and int(m[2])>=spec['min'][m[1]]:
   sources.append({'source':str(p.relative_to(ROOT)),'reference_id':f'planets--{family}-moon-{m[1]}','source_revision':int(m[2]),'trial_suffix':m[3] or None,'working_gate':'PASS — root and independent Astra exact seed38 two-angle and47px visual review; owner/hardware/publication gates remain open','files':sum(x.is_file()for x in p.rglob('*'))})
 ...
 extras=[str(p.relative_to(ROOT))for p in sorted((OUT/'volcanic-moon-2-r005').glob('*bank*'))if p.is_file()]
 for p in sorted(OUT.iterdir()):
  if str(p.relative_to(ROOT)) in {s['source']for s in sources}:continue
  if any(p.name.startswith(a)for a in aliases) and ('r006' in p.name) and (p.is_file()or any(k in p.name for k in ['audit','parity','composition'])):extras.append(str(p.relative_to(ROOT)))
 reports=[str(p.relative_to(ROOT))for p in sorted((OUT/'reviews').glob('*.md'))if any(a in p.name for a in aliases)]
...
def main():
 ...
 proposal={'schema':'sidereal.volcanic-r006-checkpoint-proposal.v1','prepared_at':now(),'status':'PREPARATION ONLY; no canonical writes until explicit root verdict','families':[discover(f,s)for f,s in SPECS.items()]}
 if not args.execute:
  (OUT/'reviews/volcanic-r006-checkpoint-proposal.json').write_text(json.dumps(proposal,indent=2)+'\n');print('Prepared proposal only; no canonical files touched');return
 ...
 with locked():
  ...
  for family in proposal['families']:
   ...
   d['revisions'].append({'revision':n,'created_at':now(),'stage':'in-progress','change':f'Preserve Volcanic moon2 native r006 and actual bank-hit evidence after root plus independent bounded visual PASS; main-world candidate remains pinned to r{pin:03}.','hypothesis':'Exact sources, PBR exports, captures and dated review verdicts preserve per-reference identity and remaining acceptance boundaries.','covered_reference_ids':refs,'evidence':evidence,'review':None})
   d['current_revision']=n;d['state']='in-progress';d['assigned_to']='root/planet-reference-20260914'
   d['feedback'].append({'revision':n,'author':'agent','text':f'Volcanic moon2 r006 bounded working visual gate PASS at seed38/two angles/47px; checkpoint only. Main-world pin remains r{pin:03}; prior approvals/mappings are exact. Attached review verdict states current visual gates; no owner sign-off or hardware transition acceptance inferred.','message_reference':'output/playwright/planet-reference-20260914/reviews/volcanic-r006-checkpoint-proposal.json','recorded_at':now(),'resolved_by_revision':None})
  ...
  (OUT/'reviews/volcanic-r006-checkpoint-result.json').write_text(json.dumps(result,indent=2)+'\n')
