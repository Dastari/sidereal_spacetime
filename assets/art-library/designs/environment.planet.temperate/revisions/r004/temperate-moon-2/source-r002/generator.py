...
palette=[('planets--temperate-moon-2-crust', (0.2, 0.29, 0.35), 0.91), ('planets--temperate-moon-2-rim', (0.46, 0.56, 0.59), 0.87), ('planets--temperate-moon-2-cavity', (0.011, 0.025, 0.031), 0.97), ('planets--temperate-moon-2-fracture-wall', (0.055, 0.11, 0.14), 0.92), ('planets--temperate-moon-2-cool-fault', (0.1, 0.25, 0.29), 0.91), ('planets--temperate-moon-2-pale-fracture', (0.66, 0.72, 0.65), 0.85), ('planets--temperate-moon-2-green-cap', (0.19, 0.37, 0.025), 0.9), ('planets--temperate-moon-2-sunlit-green-cap', (0.43, 0.57, 0.065), 0.88), ('planets--temperate-moon-2-blue-mineral-cover', (0.033, 0.26, 0.37), 0.88)]
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-planets--temperate-moon-2-r002'
 return obj
...
for form_name,parts in forms:
 ...
 for obj in parts:
  for face in obj.data.polygons:
   ...
   elif field > 0.56-.20 and q.x>.30:face.material_index=8
...
for name,parts in forms:
 ...
 for gi,(x,y) in enumerate([(-1.25,.48),(-1.02,.64),(-1.40,.76)][:2]):
  best=-10
  for obj in parts:
 ...
 for obj in parts:
  ...
  for face in obj.data.polygons:
   ...
   if face.normal.z>.58 and q.z>.10 and face.material_index in (0,1,5,6,7,8):
    if q.x < .55 or q.y>.57:face.material_index=7 if q.y>.68 else 6

kit={'schema':'sidereal.native-planet-kit.v1','layout':'hybrid-moon-craters','coveredReferenceIds':['planets--temperate-moon-2'],'compositionRecipe':{'referenceId': 'planets--temperate-moon-2', 'layoutSeed': 967, 'scales': [0.4, 0.38, 0.42, 0.39, 0.41, 0.38, 0.43, 0.39, 0.4, 0.37, 0.42, 0.4], 'variantOffset': 1},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
...
for name,parts in forms:
    ...
    for obj in parts:
        ...
        bm.free()
        had_native_uv=bool(mesh.uv_layers.active)
        if not had_native_uv:mesh.uv_layers.new(name='Native-rocky-UV')
        uv=mesh.uv_layers.active
        for p in ([] if had_native_uv else mesh.polygons):
