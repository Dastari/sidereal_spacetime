...
palette=[('planets--temperate-moon-1-crust', (0.22, 0.3, 0.38), 0.91), ('planets--temperate-moon-1-rim', (0.52, 0.58, 0.62), 0.87), ('planets--temperate-moon-1-cavity', (0.014, 0.023, 0.035), 0.97), ('planets--temperate-moon-1-fracture-wall', (0.065, 0.11, 0.17), 0.92), ('planets--temperate-moon-1-cool-fault', (0.12, 0.25, 0.28), 0.91), ('planets--temperate-moon-1-pale-fracture', (0.68, 0.72, 0.68), 0.85), ('planets--temperate-moon-1-green-cap', (0.24, 0.43, 0.035), 0.9), ('planets--temperate-moon-1-sunlit-green-cap', (0.48, 0.62, 0.075), 0.88), ('planets--temperate-moon-1-blue-mineral-cover', (0.045, 0.3, 0.45), 0.88)]
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-planets--temperate-moon-1-r002'
 return obj
...
for form_name,parts in forms:
 ...
 for obj in parts:
  for face in obj.data.polygons:
   ...
   elif field > 0.28-.20 and q.x>.30:face.material_index=8
...
for name,parts in forms:
 ...
 for gi,(x,y) in enumerate([(-1.25,.48),(-1.02,.64),(-1.40,.76)][:3]):
  best=-10
  for obj in parts:
 ...
 for obj in parts:
  ...
  for face in obj.data.polygons:
   ...
   if face.normal.z>.58 and q.z>.10 and face.material_index in (0,1,5,6,7,8):
    if q.x < .96 or q.y>.57:face.material_index=7 if q.y>.68 else 6

kit={'schema':'sidereal.native-planet-kit.v1','layout':'hybrid-moon-craters','coveredReferenceIds':['planets--temperate-moon-1'],'compositionRecipe':{'referenceId': 'planets--temperate-moon-1', 'layoutSeed': 857, 'scales': [0.4, 0.38, 0.42, 0.39, 0.41, 0.38, 0.43, 0.39, 0.4, 0.37, 0.42, 0.4], 'variantOffset': 1},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
