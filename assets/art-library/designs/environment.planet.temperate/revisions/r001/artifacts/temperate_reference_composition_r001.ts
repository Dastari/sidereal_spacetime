...
export function composeTemperateReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 const layouts=[{name:'continent-a',scale:.55,height:.38},{name:'continent-b',scale:.49,height:.47},{name:'continent-c',scale:.58,height:.31},{name:'continent-a',scale:.51,height:.38},{name:'continent-c',scale:.54,height:.31},{name:'continent-b',scale:.47,height:.47}];
 const grovePlan=[[-1.02,-.18],[-.75,.18],[-.55,-.37],[-.33,.16],[-.10,-.34],[.12,.21],[.38,-.28],[.62,.15],[.88,-.17],[-.56,.46],[.17,.48],[.57,.46]];
 for(let i=0;i<(diagnostic?1:layouts.length);i++){
  ...
  for(let g=0;g<grovePlan.length;g++){
   const [gx,gy]=grovePlan[g],groveScale=.15+[.015,.025,0,.020][g%4];
   emit(g%4===3?'small-grove':'tree-grove',`forest-${i}-${g}`,v=>{
    const lx=gx*spec.scale+v[0]*groveScale,ly=gy*spec.scale+v[1]*groveScale,x=lx*c-ly*ss,z=lx*ss+ly*c,h=1+spec.height*spec.scale+v[2]*groveScale;
