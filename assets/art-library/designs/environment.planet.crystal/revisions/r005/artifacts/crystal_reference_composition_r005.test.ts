...
const kit=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/crystal-r005/kit.json','utf8')) as NativePlanetKit;
const old=JSON.parse(readFileSync('output/playwright/planet-reference-20260914/crystal-r003/kit.json','utf8')) as NativePlanetKit;
it('preserves previous major crystal transforms while adding two medium groups per region',()=>{
 const a=priorComposition(old,38,0),b=composeCrystalReference(kit,38,0);const extract=(level:typeof a)=>level.flatMap((batch,m)=>batch.ranges.filter(r=>r.partId.startsWith('colossal')).map(r=>({m,id:r.partId,positions:Buffer.from(batch.positions.slice(r.firstTriangle*9,(r.firstTriangle+r.triangleCount)*9).buffer).toString('base64')})));
 expect(extract(b)).toEqual(extract(a));const ids=new Set(b.flatMap(batch=>batch.ranges.map(r=>r.partId)));expect([...ids].filter(id=>id.startsWith('medium'))).toHaveLength(36);
 expect(b.reduce((n,batch)=>n+batch.indices.length/3,0)).toBeLessThan(90000);
});
