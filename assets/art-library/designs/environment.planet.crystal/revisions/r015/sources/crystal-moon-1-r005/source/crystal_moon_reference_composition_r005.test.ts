...
for(const id of ['crystal-moon-1','crystal-moon-2']){
 const path=`output/playwright/planet-reference-20260914/${id}-r005/`,kit=JSON.parse(readFileSync(path+'kit.json','utf8'));
 it(`${id} retains exact native PBR and all attribute channels across LOD`,()=>{
  const prior=JSON.parse(readFileSync(path.replace('r005','r004')+'kit.json','utf8'));expect(kit.materials).toEqual(prior.materials);expect(kit.variants.map((v:any)=>v.name)).toEqual(['connected-crystalline-body']);
...
for(const id of ['crystal-moon-1','crystal-moon-2'])it(`${id} preserves original surface triangles and repairs only signed orientation`,()=>{
 const path=`output/playwright/planet-reference-20260914/${id}-r005/`,kit=JSON.parse(readFileSync(path+'kit.json','utf8')),prior=JSON.parse(readFileSync(path.replace('r005','r004')+'kit.json','utf8'));
 const a=prior.variants[0],b=kit.variants[0],report=JSON.parse(readFileSync(path+'orientation-validation.json','utf8'));
 expect(report.independentPanels).toBe(id.endsWith('-1')?86:26);expect(report.sourceWelded).toBe(false);expect(report.polygonsReversed).toBe(id.endsWith('-1')?636:200);
 expect(a.indices.length).toBe(b.indices.length);expect(a.triangleMaterials).toEqual(b.triangleMaterials);
 const flips=new Set(report.reversedSourcePolygons);
 for(let t=0;t<a.indices.length/3;t++){const reversed=flips.has(report.sourcePolygonPerTriangle[t]),order=reversed?[0,2,1]:[0,1,2];for(let k=0;k<3;k++){const ai=a.indices[t*3+order[k]],bi=b.indices[t*3+k];for(let axis=0;axis<3;axis++){expect(b.positions[bi*3+axis]).toBe(a.positions[ai*3+axis]);expect(b.normals[bi*3+axis]).toBe(a.normals[ai*3+axis]*(reversed?-1:1));}for(let axis=0;axis<2;axis++)expect(b.uvs[bi*2+axis]).toBe(a.uvs[ai*2+axis]);}}
 for(const panel of report.panels)expect(panel.positiveSignedVolume).toBeGreaterThan(0);
 for(const seed of [38,117,904]){const levels=[0,1,2].map(l=>composeCrystalMoonReference(kit,seed,l as 0|1|2));for(const level of levels.slice(1))expect(level).toEqual(levels[0]);}
});
