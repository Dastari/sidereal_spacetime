...
SPECS={'ice':{'current':27,'pin':26,'min':{'2':3}},'gas-giant':{'current':6,'pin':5,'min':{'1':5,'3':5}},'volcanic':{'current':24,'pin':23,'min':{'1':3,'2':3}},'crystal':{'current':14,'pin':13,'min':{'1':3,'2':3}}}
def sha(path):
 h=hashlib.sha256()
...
def discover(family,spec):
 ...
 for p in sorted(OUT.iterdir()):
  ...
  if p.is_dir() and m and m[1] in spec['min'] and int(m[2])>=spec['min'][m[1]]:
   sources.append({'source':str(p.relative_to(ROOT)),'reference_id':f'planets--{family}-moon-{m[1]}','source_revision':int(m[2]),'trial_suffix':m[3] or None,'files':sum(x.is_file()for x in p.rglob('*'))})
 ...
 for p in sorted(OUT.iterdir()):
  ...
  if any(p.name.startswith(a)for a in aliases) and ('moon' in p.name) and (p.is_file()or any(k in p.name for k in ['audit','parity','composition'])):extras.append(str(p.relative_to(ROOT)))
...
def main():
 ...
 proposal={'schema':'sidereal.late-moon-checkpoint-proposal.v1','prepared_at':now(),'status':'PREPARATION ONLY; no canonical writes until explicit root verdict','families':[discover(f,s)for f,s in SPECS.items()]}
 if not args.execute:
  (OUT/'reviews/late-moon-checkpoint-proposal.json').write_text(json.dumps(proposal,indent=2)+'\n');print('Prepared proposal only; no canonical files touched');return
 ...
 with locked():
  ...
  for family in proposal['families']:
   ...
   d['revisions'].append({'revision':n,'created_at':now(),'stage':'in-progress','change':f'Preserve late native moon candidates and meaningful failed/diagnostic history; main-world candidate remains pinned to r{pin:03}.','hypothesis':'Exact sources, PBR exports, captures and dated review verdicts preserve per-reference identity and remaining acceptance boundaries.','covered_reference_ids':refs,'evidence':evidence,'review':None})
   d['current_revision']=n;d['state']='in-progress';d['assigned_to']='root/planet-reference-20260914'
   d['feedback'].append({'revision':n,'author':'agent','text':f'Late moon preservation checkpoint only. Main-world pin remains r{pin:03}; prior approvals/mappings are exact. Attached review verdict states current visual gates; no owner sign-off or hardware transition acceptance inferred.','message_reference':'output/playwright/planet-reference-20260914/reviews/late-moon-checkpoint-proposal.json','recorded_at':now(),'resolved_by_revision':None})
  ...
  (OUT/'reviews/late-moon-checkpoint-result.json').write_text(json.dumps(result,indent=2)+'\n')
