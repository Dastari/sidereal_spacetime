"""Native r008: replace selected panel districts with connected bent fracture runs."""
...
old=BASE/f'crystal-moon-{moon}-r005';out=BASE/f'crystal-moon-{moon}-r008';assert not(out/'kit.blend').exists();out.mkdir(exist_ok=True)
...
for panel,region_id,points,center,normal in selected:
 ...
 for section,(low,high)in enumerate(zip(cuts,cuts[1:])):
  shift=[-.045,.065,-.025,.040,-.068,.018,.072,-.034,.051,-.015,.061,-.040][section]
  for band,(lower,upper,height)in enumerate([(-10,-region['width'],[-.045,.035,-.010,.078,.008,-.032,.052,-.012,.089,-.020,.055,.005][section]),(-region['width'],region['width'],[-.110,-.060,-.125,-.075][section%4]),(region['width'],10,[.065,-.020,.085,.025,-.040,.070,-.005,.055,-.025,.081,.015,-.030][section])]):
   topmesh=bmesh.new()
   ...
   bright=band==2 and section in [3,6,8] and region_id in [0,1] and center.z>-.45
...
gltf['nodes'][0]['extras']['authoring'] = 'native-crystal-moon-r008-selected-fractures'
...
print('R008_DONE',moon,'districts',len(selected),'parts',len(records),'triangles',len(roles))
