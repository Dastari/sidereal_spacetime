...
root=Path('/root/sidereal_spacetime/output/playwright/planet-reference-20260914');out=root/'crystal-moons-r007-parity';records=[]
for id in[1,2]:
 bpy.ops.wm.open_mainfile(filepath=str(root/f'crystal-moon-{id}-r007/kit.blend'));scene=bpy.context.scene;obj=next(o for o in bpy.data.objects if o.type=='MESH')
 kit=json.loads((root/f'crystal-moon-{id}-r007/kit.json').read_text());capture=json.loads((root/f'crystal-moon-{id}-r004/native-body-seed38.json').read_text());phase=((((38^kit['compositionRecipe']['layoutSeed'])*1664525)+1013904223)&0xffffffff)/4294967296*math.pi*2;c=math.cos(phase);s=math.sin(phase);alpha=capture['camera']['alpha'];beta=capture['camera']['beta'];radius=5.5
