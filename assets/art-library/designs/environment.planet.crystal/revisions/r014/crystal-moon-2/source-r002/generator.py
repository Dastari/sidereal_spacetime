...
def make(name,verts,faces,roles):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=bm.faces);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-planets--crystal-moon-2-r002';return o
...
for source_name in ['battered-region-a','battered-region-b']:
 ...
 for obj in target.objects:
  ...
  obj.name='GEO-crystal-'+obj.name;obj['role']='planet';obj['authoring']='native-planets--crystal-moon-2-r002';parts.append(obj)
...
for name, subdivisions in [('ground-sphere',5),('ground-sphere-medium',4),('ground-sphere-low',3)]:
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions, radius=.93)
...
for name,parts in forms:
 ...
 for obj in parts:
  for vertex in obj.data.vertices:
   vertex.co.z*=0.92
   vertex.co.x*=1.08;vertex.co.y*=1.08
...
kit={'schema':'sidereal.native-planet-kit.v1','layout':'crystal-moon-geology','coveredReferenceIds':['planets--crystal-moon-2'],'compositionRecipe':{'referenceId': 'planets--crystal-moon-2', 'layoutSeed': 2087, 'scales': [0.30, 0.28, 0.31, 0.27, 0.30, 0.28], 'variantOffset': 1, 'hero': 0.18},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
