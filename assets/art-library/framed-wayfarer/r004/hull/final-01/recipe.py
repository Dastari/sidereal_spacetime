...
class Module:
    ...
    def mesh(self, name, verts, faces, region='rail', bevel=0, group='ARMOR', surface_uv=None, material='enamel'):
        ...
        if bevel:
            ...
            if hasattr(self, 'mating_planes_y'):
                weights = data.attributes.new('bevel_weight_edge', 'FLOAT', 'EDGE')
                for edge in data.edges:
                    on_mate = any(all(abs(data.vertices[index].co.y - plane) < 1e-6 for index in edge.vertices)
                                  for plane in self.mating_planes_y)
...
def side(slug, variant, mats, pair=None, width=2, height=3, mirrored=False):
    ...
    panel_uv = lambda p: (atlas_span - (p.y + 1 + bay_shift) if mirrored else p.y + 1 + bay_shift, p.z)
    ...
    if width >= 1:
        ...
        m.box('cyan-rail-recess', .492, .499, max(rail_start+(0 if pair == 'right' else .025), lens_center - .36),
              min(rail_end-(0 if pair == 'left' else .025), lens_center + .36), .05 if low_clearance else .14,
              .125 if low_clearance else .255, 'black', .008)
        m.box('cyan-rail-lens', .499, .5, max(rail_start+(0 if pair == 'right' else .04), lens_center - .29),
...
def front(slug, mats):
    ...
    if slug in ('front-sill-straight', 'front-bow-transom', 'front-sill-diagonal45'):
        ...
        uv = lambda p: ((p-a).dot(tangent), p.z)
        ...
        for name, low, high in [('impact', 0, .3125), ('shoulder', 1.21875, 1.55)]:
            edge_block(m, 'wrapped-bow-' + name, a, b, .21875, length - .21875, -.375, .125, low, high,
                       'rail', .03125)
        ...
        for i, (start, end, region) in enumerate([(.265625, middle-.046875, 'service'),
            ...
            uv = lambda p, start=start: ((p-a).dot(tangent)-start+.125, p.z+.53125)
