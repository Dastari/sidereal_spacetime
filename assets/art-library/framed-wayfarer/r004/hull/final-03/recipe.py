...
if __name__ == '__main__' and '--' not in sys.argv:
    ...
    parser.add_argument('--full', action='store_true', help='Build full family after independent proof gate')
    ...
    command = [config['art']['blender'], '--background', '--factory-startup', '--threads', '4',
...
REGIONS = {
    ...
    'wide-red-service': (4, 7, 4, 3, (.47, .055, .075)),
    'bow-small': (0, 8.5, 2, 1.5, (.68, .71, .78)),
}
FRONT_IDS = {
...
def materials(out):
    ...
    for key, (x, y, w, h, color) in REGIONS.items():
        ...
        lu, lv = u - x, v - y
        if key in ('plain', 'service', 'wide-red-service', 'wide-plain', 'identity', 'wide-identity'):
            ...
            for cu in (left + .09, right - .09):
                for cv in (low + .14, high - .14):
                    ...
                    rough[seat] = .57
            if key in ('service', 'wide-red-service'):
            ...
            for course in (.625, 1.25, 1.875, 2.5, 3.125, 3.75):
                joint = inside & ((abs(lu - course) < .009) | (abs(lv - course) < .009))
                relief[joint] = -.0035; rgb[joint] *= .49; rough[joint] = .58
    ...
    # Single physical 2 m identity tiles retain the same planet/ring identity
    # as the broad paired bay, with room beside the fixed-size WF-01 text.
    single_disc = (u - 13.49) ** 2 + (v - 1.55) ** 2 < .19 ** 2
    rgb[single_disc] = (.80, .83, .89)
    single_ring = ((u - 13.49) / .285) ** 2 + ((v - 1.55 - (u - 13.49) * .35) / .070) ** 2
    rgb[(single_ring > .77) & (single_ring < 1.20)] = (.66, .08, .10)
    text_mask(rgb, 'WAYFARER', .68, 7.66, 6, (.04, .06, .10))
    text_mask(rgb, 'WAYFARER', .44, 9.00, 3, (.04, .06, .10))
...
class Module:
    ...
    def mesh(self, name, verts, faces, region='rail', bevel=0, group='ARMOR', surface_uv=None, material='enamel'):
        ...
        if bevel:
            ...
            if hasattr(self, 'mating_planes_y') or hasattr(self, 'mating_planes_x'):
                weights = data.attributes.new('bevel_weight_edge', 'FLOAT', 'EDGE')
                for edge in data.edges:
                    on_mate = any(all(abs(data.vertices[index].co[axis] - plane) < 1e-6 for index in edge.vertices)
                                  for axis, planes in [(0, getattr(self, 'mating_planes_x', [])),
                                                       (1, getattr(self, 'mating_planes_y', []))] for plane in planes)
                    weights.data[edge.index].value = 0 if on_mate else 1
...
def side(slug, variant, mats, pair=None, width=2, height=3, mirrored=False):
    m = Module(slug, mats); half = width / 2
    m.mating_planes_y = [-half, half]
    ...
    if height < 1.5:
        # Defined 187.5 mm low-clearance rail; retain depth and chamfer sizes.
        lower = [(.125, 0), (.4375, 0), (.5, .0625), (.5, .125),
                 (.4375, .1875), (.1875, .1875), (.125, .125)]
        top = [(.125, height-.1875), (.34375, height-.1875), (.5, height-.125),
               (.5, height-.0625), (.4375, height), (.125, height)]
    rail_start = -half + (0 if pair == 'right' else .125)
    rail_end = half - (0 if pair == 'left' else .125)
    lens_center = 1 if pair == 'left' else -1 if pair == 'right' else 0
    if width >= 1:
        pocket_start = max(rail_start+(0 if pair == 'right' else .025), lens_center-.36)
        pocket_end = min(rail_end-(0 if pair == 'left' else .025), lens_center+.36)
        if pocket_start > rail_start+1e-7:
            m.rail('deep-lower-impact-rail-before-pocket', lower, rail_start, pocket_start)
        low_z, high_z = lower[2][1], lower[3][1]
        cut_low = max(low_z, .05 if height < 1.5 else .14)
        cut_high = min(high_z, .125 if height < 1.5 else .255)
        recessed = lower[:3]
        if cut_low > low_z+1e-7: recessed.append((.5, cut_low))
        recessed += [(.46875, cut_low), (.46875, cut_high)]
        if cut_high < high_z-1e-7: recessed.append((.5, cut_high))
        recessed += lower[3:]
        m.rail('deep-lower-impact-rail-with-true-lens-pocket', recessed, pocket_start, pocket_end)
        if pocket_end < rail_end-1e-7:
            m.rail('deep-lower-impact-rail-after-pocket', lower, pocket_end, rail_end)
    else:
        m.rail('deep-lower-impact-rail', lower, rail_start, rail_end)
    m.rail('stepped-upper-armor-shoulder', top, rail_start, rail_end)
    ...
    for i, (a, b) in enumerate(edges):
        m.box(f'wrapped-connector-{i}-spine', .34375, .484375, a, b,
              low_band + .078125, height - high_band - .078125, 'connector', .015625)
        m.box(f'wrapped-connector-{i}-lower-shoe', CELL, .5, a, b, 0, low_band + .0625,
              'connector', .03125)
        m.box(f'wrapped-connector-{i}-upper-shoe', CELL, .5, a, b, height - high_band - .0625, height,
              'connector', .03125)
        if height >= 2.25:
    ...
    if region not in REGIONS: region = 'wide-plain' if pair else 'plain'
    bay_shift = 0 if pair != 'right' else 2
    atlas_span = 4 if pair else 2
    panel_uv = lambda p, normal: (atlas_span - (p.y + 1 + bay_shift) if mirrored else p.y + 1 + bay_shift, p.z)
    ...
    # Author one broad design in bay coordinates, then crop each independent
    # member to its exact owning interval. The physical member never scales.
    bay_offset = -1 if pair == 'left' else 1 if pair == 'right' else 0
    def bay_interval(a, b):
        return max(-half, a - bay_offset), min(half, b - bay_offset)
    def bay_box(name, x0, x1, a, b, z0, z1, region='connector', bevel=.015625, material='enamel'):
        a, b = bay_interval(a, b)
        if b - a > 1e-6:
            return m.box(name, x0, x1, a, b, z0, z1, region, bevel, material=material)
    def bay_rail(name, cross, a, b, region='metal'):
        a, b = bay_interval(a, b)
        if b - a > 1e-6: return m.rail(name, cross, a, b, region)
    extra = 1 if pair else 0
    if variant == 'vent':
        bay_box('vent-well', .282, .294, -.63 - extra, .63 + extra, .68, 2.21, 'black', .018)
        for i in range(6):
            z = .74 + i * .235
            bay_rail(f'deep-vent-louver-{i}', [(.292, z), (.4375, z + .035),
                                          (.4375, z + .112), (.292, z + .155)], -.57-extra, .57+extra)
        for i, (a, b) in enumerate([(-.68-extra, -.60-extra), (.60+extra, .68+extra)]):
            bay_box(f'vent-cassette-side-jamb-{i}', .28125, .40625, a, b, .625, 2.27, 'rail', .016)
    elif variant == 'red-service':
        bay_box('service-upper-hinge', .282, .34375, -.63-extra, -.40-extra, 1.94, 2.07, 'metal', .018)
        bay_box('service-lower-hinge', .282, .34375, -.63-extra, -.40-extra, .76, .89, 'metal', .018)
        bay_box('service-latch-recess', .282, .291, .47+extra, .65+extra, 1.23, 1.57, 'black', .008)
        bay_box('service-latch-grip', .29, .375, .53+extra, .59+extra, 1.27, 1.52, 'metal', .013)
    elif variant == 'utility':
        # Substantial removable service housing and vent pocket, with mapped
        # tooling on the pale cassette. These are exterior utility appliances.
        centers = [-.82, .82] if pair else [0]
        for i, center in enumerate(centers):
            bay_box(f'utility-{i}-deep-pocket', .282, .296, center-.43, center+.43, .72, 2.22, 'black', .016)
            bay_box(f'utility-{i}-framed-housing', .296, .4375, center-.36, center+.36, .80, 2.13, 'rail', .03125)
            bay_box(f'utility-{i}-pale-service-cover', .438, .46875, center-.28, center+.28, .93, 1.67, 'connector', .018)
            bay_box(f'utility-{i}-upper-recess', .438, .445, center-.26, center+.26, 1.79, 2.04, 'black', .008)
            bay_box(f'utility-{i}-status-lens', .445, .453125, center-.16, center+.16, 1.88, 1.94,
                    'black', 0, material='cyan' if i == 0 else 'amber')
            bay_box(f'utility-{i}-grip', .469, .5, center+.14, center+.19, 1.18, 1.43, 'metal', .009)
    if width >= 1:
        low_clearance = height < 1.5
        m.box('cyan-rail-recess', .469, .484375, max(rail_start+(0 if pair == 'right' else .025), lens_center - .36),
              min(rail_end-(0 if pair == 'left' else .025), lens_center + .36), .05 if low_clearance else .14,
              .125 if low_clearance else .255, 'black', .008)
        m.box('cyan-rail-lens', .484375, .5, max(rail_start+(0 if pair == 'right' else .04), lens_center - .29),
              min(rail_end-(0 if pair == 'left' else .04), lens_center + .29), .069 if low_clearance else .174,
              .100 if low_clearance else .218, 'black', 0, material='cyan')
...
def edge_block(m, name, a, b, start, end, d0, d1, low, high, region='connector', bevel=.015625, group='ARMOR', surface_uv=None):
    ...
    elif m.slug in ('front-sill-diagonal45', 'front-diagonal-cheek'):
        # Clip deep return corners to the inherited terminal mating planes;
        # the diagonal exterior face itself retains its authored sweep.
        planes = [(0, 0, 1), (1, 2 if m.slug == 'front-sill-diagonal45' else 2.075246572494507, -1),
                  (1, .0176776684820652 if m.slug == 'front-sill-diagonal45' else 0, 1)]
        for axis, limit, sign in planes:
            clipped = []
            for previous, current in zip(xy[-1:] + xy[:-1], xy):
                pin, cin = sign*(previous[axis]-limit) >= -1e-9, sign*(current[axis]-limit) >= -1e-9
                if pin != cin:
                    t = (limit-previous[axis])/(current[axis]-previous[axis])
                    clipped.append(tuple(previous[i]+t*(current[i]-previous[i]) for i in range(2)))
                if cin: clipped.append(current)
            xy = clipped
        assert len(xy) >= 3, ('Empty clipped front solid', name)
    if m.slug in ('front-sill-diagonal45', 'front-diagonal-cheek') and region == 'rail' and surface_uv is None:
        bottom = min(low(x, y) if callable(low) else low for x, y in xy)
        def surface_uv(p, normal):
            along, depth = (p-a).dot(tangent)-start, (p-a).dot(outward)-d0
            if abs(normal.z) > .5: return along, depth
            if abs(normal.dot(tangent)) > .5: return depth, p.z-bottom
            return along, p.z-bottom
...
def front(slug, mats):
    m = Module(slug, mats)
    if slug in ('front-sill-straight', 'front-bow-transom', 'front-sill-diagonal45'):
        diagonal = slug == 'front-sill-diagonal45'
        if diagonal:
            liner = [(0, .34375), (1.65625, 2), (1.625, 2), (0, .375)]
            a, b = Vector((.03125, .03125, 0)), Vector((1.96875, 1.96875, 0))
        else:
            liner = [(0, .34375), (2, .34375), (2, .375), (0, .375)]
            a, b = Vector((0, 0, 0)), Vector((2, 0, 0))
        m.xy_prism('retained-inner-sill-surface', liner, 0, 1.125, 'metal', 0, 'LINER')
        length = (b-a).length; tangent = (b-a).normalized()
        outer = .015625 if diagonal else .03125
        for name, low, high in [('impact', 0, .21875), ('shoulder', .875, 1.125)]:
            edge_block(m, 'opaque-sill-' + name, a, b, .125, length-.125, -.3125, outer,
                       low, high, 'rail', .025)
        for i, (start, end) in enumerate([(0, .125), (length-.125, length)]):
            edge_block(m, f'wrapped-sill-connector-{i}', a, b, start, end, -.3125, outer,
                       0, 1.125, 'connector', .025)
        edge_block(m, 'sill-cassette-reveal', a, b, .125, length-.125, -.28125, -.25,
                   .21875, .875, 'black', .008)
        region = 'bow-small' if slug == 'front-bow-transom' else 'wide-plain' if diagonal else 'plain'
        uv = lambda p, normal: ((p-a).dot(tangent), p.z)
        edge_block(m, 'opaque-sill-armor-cassette', a, b, .171875, length-.171875,
                   -.25, -.125, .265625, .828125, region, .025, surface_uv=uv)
    elif slug == 'front-bow-bumper':
        m.mating_planes_x = [0, 4]
        ...
        edge_block(m, 'wrapped-bow-shoulder', a, b, .21875, length-.21875, -.375, .125,
                   1.21875, 1.55, 'rail', .03125)
        for name, start, end in [('before', .21875, 1.31), ('after', 2.19, length-.21875)]:
            edge_block(m, 'wrapped-bow-impact-' + name, a, b, start, end, -.375, .125,
                       0, .3125, 'rail', .02)
        edge_block(m, 'bow-light-pocket-backing', a, b, 1.31, 2.19, -.375, .09375,
                   0, .3125, 'rail', .008)
        for name, low, high in [('lower', 0, .11), ('upper', .23, .3125)]:
            edge_block(m, 'bow-light-pocket-' + name, a, b, 1.31, 2.19, .09375, .125,
                       low, high, 'rail', .004)
        for j, (t0, t1) in enumerate([(0, .21875), (length - .21875, length)]):
            edge_block(m, f'bow-connector-{j}', a, b, t0, t1, -.375, .125, 0, 1.55,
                       'connector', .03125)
        right_return = [(3.75, CELL), (4, CELL), (4, .0625), (3.75, .625)]
        left_return = [(4-x, y) for x, y in reversed(right_return)]
        for name, polygon in [('right', right_return), ('left', left_return)]:
            # This narrow contact land stays planar; adjacent outer caps carry
            # chamfers. Weighted bevel on the thin mate generates sliver faces.
            m.xy_prism('fitted-bow-terminal-return-' + name, polygon, 0, 1.55,
                       'connector', 0)
        edge_block(m, 'bow-cassette-dark-reveal', a, b, .22, length - .22, -.28125, -.25,
                   .29, 1.24, 'black', .008)
        tangent = (Vector(b) - Vector(a)).normalized()
        uv = lambda p, normal: ((p - Vector(a)).dot(tangent) + .25, p.z)
        edge_block(m, 'opaque-bow-name-cassette', a, b, .28125, length - .28125, -.25, -.09375,
                   .34375, 1.1875, 'bow-name', .03125, surface_uv=uv)
        edge_block(m, 'bow-cyan-socket', a, b, 1.31, 2.19, .094, .109375, .11, .23, 'black', .003)
        obj = edge_block(m, 'bow-cyan-lens', a, b, 1.38, 2.12, .109375, .125, .15, .19, 'black', 0)
        obj.data.materials.clear(); obj.data.materials.append(mats['cyan'])
    elif slug == 'front-diagonal-cheek':
        m.mating_planes_x = [1]
        m.xy_prism('retained-inner-cheek-surface', [(2, 0), (2.03125, 0), (.03125, 2), (0, 2)],
                   0, 1.48, 'metal', 0, 'LINER')
        a, b = Vector((4, 0, 0)), Vector((1, 2, 0)); length = (b-a).length
        for name, low, high in [('impact', 0, .3125), ('shoulder', 1.21875, 1.55)]:
            edge_block(m, 'cheek-' + name, a, b, .21875, length-.21875, -.375, .0625,
                       low, high, 'rail', .03125)
        for i, (start, end) in enumerate([(0, .21875), (length-.21875, length)]):
            if i == 0:
                edge_block(m, f'wrapped-cheek-connector-{i}', a, b, start, end, -.375, .0625,
                           0, 1.55, 'connector', .03125)
            else:
                tangent = (b-a).normalized(); outward = Vector((tangent.y, -tangent.x, 0))
                aa = a+tangent*start
                # The terminal land meets the bumper on its existing X=4
                # datum (cheek X=1 after placement), Y2.03125..2.0625.
                polygon = [tuple(aa+outward*-.375)[:2], tuple(b+outward*-.375)[:2],
                           (1, 2), (1, 2.0625), tuple(b+outward*.0625)[:2],
                           tuple(aa+outward*.0625)[:2]]
                m.xy_prism('fitted-cheek-terminal-connector-land', list(reversed(polygon)),
                           0, 1.55, 'connector', .015625)
        edge_block(m, 'cheek-cassette-dark-reveal', a, b, .21875, length-.21875, -.3125, -.28125,
                   .3125, 1.21875, 'black', .008)
        middle = length/2
        for i, (start, end, region) in enumerate([(.265625, middle-.046875, 'service'),
                                                (middle+.046875, length-.265625, 'plain')]):
            tangent = (b-a).normalized()
            uv = lambda p, normal, start=start: ((p-a).dot(tangent)-start+.125, p.z+.53125)
            edge_block(m, f'opaque-cheek-cassette-{i}', a, b, start, end, -.28125, -.125,
                       .359375, 1.171875, region, .025, surface_uv=uv)
        edge_block(m, 'cheek-central-rib', a, b, middle-.046875, middle+.046875, -.25, .03125,
                   .34375, 1.1875, 'connector', .02)
        for i in range(4):
            z = .46 + .15625*i
            edge_block(m, f'cheek-deep-vent-blade-{i}', a, b, middle+.21875, length-.4375,
                       -.12, .015625, z, z+.0625, 'metal', .013)
    else:
        ...
        edge_block(m, 'shoulder-lower-impact-frame', a, b, start + .1875, end - .1875, -.28125, .0625,
                   0, .34375, 'rail', .025)
        edge_block(m, 'shoulder-sloped-upper-frame', a, b, start + .1875, end - .1875, -.28125, .0625,
...
def build_all(out_dir, full=False):
    ...
    if full:
        variants = ['plain', 'red-service', 'vent', 'utility', 'identity']
        requests = [(variant, role, 2, 3) for variant in variants for role in (None, 'left', 'right')]
        requests += [('plain', None, width, height) for width, height in [(1, 3), (.5, 3), (2, .75), (2, 1.5), (2, 2.25)]]
        requests = [(*row, mirrored) for row in requests for mirrored in (False, True)]
    else:
        requests = [(variant, pair, 2, 3, False) for variant, pair in
                    [('red-service', None), ('vent', None), ('plain', 'left'), ('plain', 'right')]]
    for variant, pair, width, height, mirrored in requests:
        slug = ('armor-' + variant + ('-pair-' + pair if pair else '') +
                f'-w{round(width*100):03}-h{round(height*100):03}' + ('-port' if mirrored else ''))
        m = side(slug, variant, mats, pair, width, height, mirrored)
        records.append(export(m, out, {'family': 'side', 'variant': variant, 'widthM': width, 'heightM': height,
                                       'pairRole': pair, 'visualBayWidthM': 4 if pair else width,
                                       'mirroredExterior': mirrored,
                                       'exteriorUvConvention': 'reversed U for reflected X placement; local Y unchanged' if mirrored else 'standard +X exterior',
                                       'heightProfile': 'low-clearance 187.5 mm rails' if height < 1.5 else 'full-size 437.5/500 mm rails',
    ...
    for slug, asset_id in FRONT_IDS.items():
        if not full and slug not in ('front-bow-bumper', 'front-shoulder-transition'): continue
    ...
    result = {'schema': 'sidereal.armor-cassette-hull-native.v1', 'revision': REVISION,
              'stage': 'complete-family-independent-review-candidate' if full else 'initial-independent-review-proof',
              ...
              'assemblyRecipe': {'portMirroredX': True, 'pairLeftMeans': 'lower local Y',
                    'baysPerSide': [{'variant': v, 'memberCentersY': ys} for v, ys in
                        [('red-service', [-8, -6]), ('utility', [-4, -2]), ('identity', [0, 2]),
                         ('vent', [4]), ('plain', [6, 8])]]} if full else None,
              'limits': ['No live publication or authority changes.',
              'Separate inboard liner and exterior armor groups; no opaque glazing cover.',
              'Complete-family independent visual review pending; proof-05 initial propagation gate passed.' if full else 'Independent visual review required before full-family propagation.',
...
if __name__ == '__main__':
    build_all(sys.argv[sys.argv.index('--') + 1], '--full' in sys.argv)
