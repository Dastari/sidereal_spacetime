async page => {
  ...
  const captures=[];
  for(const assembly of ['wayfarer','alternate-station','alternate-tug','heights','bulkheads','directions-32','junction-registry']){
    ...
    for(const [name,mode] of list){
      const metrics=await page.evaluate(([name,mode])=>window.review.view(name,mode),[name,mode]);
      const file=assembly+'-'+name+'-'+mode;
