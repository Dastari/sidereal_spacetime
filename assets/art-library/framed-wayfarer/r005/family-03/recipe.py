...
def bounded_identity_color_patch(old,mats):
 """Repaint only the identity graphic; physical surface UVs/normal/roughness stay metric."""
 import numpy as np
 image=next(node.image for node in mats['enamel'].node_tree.nodes if node.type=='TEX_IMAGE' and 'metric color atlas' in node.image.name)
 n=image.size[0];pixels=np.empty(n*n*4,dtype=np.float32);image.pixels.foreach_get(pixels);pixels=pixels.reshape(n,n,4)
 density=old.TEXELS_PER_METRE;u0,v0,width,height,_=old.REGIONS['identity'];x0,y0=round(u0*density),round(v0*density);w,h=round(width*density),round(height*density)
 region=pixels[y0:y0+h,x0:x0+w,:3].copy()
 # Original white/red marking pixels are distinct from the dark metric material.
 old_graphic=region[:,:,0]>.30
 region[old_graphic]=(.055,.073,.115)
 supersample=4;u,v=np.meshgrid((np.arange(w*supersample)+.5)/(density*supersample),(np.arange(h*supersample)+.5)/(density*supersample))
 patch=np.repeat(np.repeat(region,supersample,axis=0),supersample,axis=1)
 text=np.zeros(u.shape,dtype=bool);start_u,start_v=.14,1.32;cell_u,cell_v=.024,.035
 for index,ch in enumerate('WF-01'):
  for row,bits in enumerate(old.FONT[ch]):
   for col,bit in enumerate(bits):
    if bit=='1':
     x=start_u+(index*6+col)*cell_u;y=start_v+(6-row)*cell_v
     text|=(u>=x)&(u<x+cell_u)&(v>=y)&(v<y+cell_v)
 patch[text]=(.80,.83,.90)
 disc=(u-1.2)**2+(v-1.47)**2<.17**2
 patch[disc]=(.80,.83,.89)
 ring=((u-1.2)/.225)**2+((v-1.47-(u-1.2)*.35)/.055)**2
 ringmask=(ring>.77)&(ring<1.2);patch[ringmask]=(.66,.08,.10)
 pixels[y0:y0+h,x0:x0+w,:3]=patch.reshape(h,supersample,w,supersample,3).mean(axis=(1,3))
 image.pixels.foreach_set(pixels.ravel());image.save();image.pack()
 mask=text|disc|ringmask;ys,xs=np.where(mask)
 bounds=[float((xs.min())/(density*supersample)),float(ys.min()/(density*supersample)),float((xs.max()+1)/(density*supersample)),float((ys.max()+1)/(density*supersample))]
 return {'kind':'bounded-single-bay-color-graphic','text':'WF-01','graphicMetricUvBounds':bounds,'surfaceTexelsPerMetre':density,'graphicAntialiasSupersample':supersample,'normalAndRoughnessUnchanged':True,'note':'Graphic color pixels are composed inside one physical face inset. Underlying surface UV scale, normals and roughness are not normalized or stretched.'}
...
def build(out,full):
 ...
 identity_patch=bounded_identity_color_patch(old,mats)
 ...
 for record in manifest['models']:
  ...
  if p['kind']=='span':
   ...
   if panel1-panel0>.075 and z1-z0>.05:
    ...
    if variant=='identity':
     bounds=identity_patch['graphicMetricUvBounds'];safe=[.025,z0+.025,panel1-panel0-.025,z1-.025]
     assert safe[0]<=bounds[0] and safe[1]<=bounds[1] and safe[2]>=bounds[2] and safe[3]>=bounds[3],('IDENTITY_GRAPHIC_DOES_NOT_FIT',L,H,bounds,safe)
     record['graphicFit']={**identity_patch,'safeFaceMetricUvBounds':safe}
    if variant=='vent':
