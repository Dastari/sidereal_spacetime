 ...
 def boundary(self,name,endpoints32,heights=None,finishes=None,z=0.):
  ...
  for i in range(count):
   ...
   for j,size in enumerate(sizes):
    ident=self.span(size,heights[i],finishes[i])
...
def make_plan(full=False):
 ...
 if full:
  ...
  p.boundary('wayfarer',[[int(v*32) for v in q] for q in boundary],[3,3,3,1.5,1.5,1.5,1.5,1.5,3,3],['plain','identity','utility','plain','vent','plain','vent','plain','utility','red-service'],z=.1875)
