...
export function drawRenderBackendMenu(
  ...
  const width = (r.w - 6) / 2;
  (["webgl", "webgpu"] as const).forEach((backend, i) =>
    ui.button(
      "graphics-renderer-" + backend,
      backend === "webgl" ? "WebGL" : "WebGPU (experimental)",
      { x: r.x + i * (width + 6), y: r.y + 30, w: width, h: 34 },
      () => controls.set(backend),
      { selected: controls.state.requested === backend },
    ),
  );
  ui.text(
    `Active: ${controls.state.active === "webgpu" ? "WebGPU" : "WebGL"}`,
    r.x,
    r.y + 78,
    13,
    palette.muted,
  ...
  if (controls.state.reason)
    ui.paragraph(controls.state.reason, { ...r, y: r.y + 101, h: 36 }, 13);
