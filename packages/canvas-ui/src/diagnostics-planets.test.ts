...
it("shows worker build milliseconds and pending planet jobs without inventing a first sample", () => {
  ...
  const data = {
    ...
    renderHeight: 907,
    hardwareScale: 1,
    planetBuild: { pendingBuilds: 2 },
  } as RenderDiagnostics;
  const panel = createDiagnosticsUI(ui, () => data);
  panel.toggle();
  panel.draw();
  expect(text.mock.calls.some(([v]) => v === "Last planet build")).toBe(true);
  expect(text.mock.calls.some(([v]) => v === "Pending planet builds")).toBe(
    true,
  );
  data.planetBuild = { lastBuildMs: 127.234, pendingBuilds: 0 };
  text.mockClear();
  panel.draw();
  expect(text.mock.calls.some(([v]) => v === "127.23 ms")).toBe(true);
  panel.dispose();
});
