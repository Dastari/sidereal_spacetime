...
export const TILESET_INTERFACE_SCHEMA='sidereal.tileset-interface.v1' as const;
export const STRUCTURAL_LATTICE_PER_METER=32;
export const STRUCTURAL_MODULE_UNITS=64;
export const TILESET_INTERFACE_LIMITS={coordinate:8192,parts:2048,vertices:16,interfaces:8192} as const;
export type ConstructionRole='floor'|'roof'|'pressure-wall'|'partition'|'armor'|'external-system'|'interior-equipment'|'cargo'|'decoration';
export type DamageMode='voxel'|'entity-health'|'none';
export type LatticePoint3=[number,number,number];
export type NativeAssetPin={assetId:string;revision:string;sha256:string;nodePrefix?:string;sourceToNominal:{translation:LatticePoint3;quarterTurns:0|1|2|3;reflected:boolean}};
export type SealPatch={a:Point;b:Point;bottom:number;top:number;family:string;/** Approved physical rule; null is fitting evidence only. */definitionId:string|null};
export type NominalEdge={id:string;a:Point;b:Point;profile:string;bottom:number;top:number;seals:SealPatch[]};
export type NominalMount={id:string;family:string;position:LatticePoint3;normal:LatticePoint3;definitionId:string|null};
export type NominalPort={id:string;mountId:string;channel:ServiceChannel;medium:string;direction:'in'|'out'|'both';definitionId:string|null;sealedPenetrationDefinitionId:string|null};
export interface NominalPartInterface {
 ...
 damageAdapter:{id:string;revision:string;proxySha256:string;nativeClipAdapterId:string}|null;
...
export interface TilesetInterface {
 ...
 datums:{floorBottom:number;floorTop:number;roofBottom:number|null;roofTop:number|null;serviceBottom:number|null;serviceTop:number|null};
