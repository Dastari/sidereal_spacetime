...
const ionSpec = (variant: "standard" | "salvaged"): KindSpec => {
  ...
  return {
    ...
    sizes: ALL,
    sockets: ["rear", "face"],
    envelope: (i, s) => outward(SHIP_SIZE_CELLS[s], ION_LEN[i]),
...
const blockSpec: KindSpec = {
  ...
  sizes: ALL,
  sockets: ["rear", "face"],
  envelope: (i, s) => outward(SHIP_SIZE_CELLS[s], BLOCK_LEN[i], BLOCK_H[i]),
...
const resonanceSpec: KindSpec = {
  ...
  sizes: SML,
  sockets: ["rear", "face"],
  envelope: (i, s) => outward(SHIP_SIZE_CELLS[s], ION_LEN[i] * 0.85),
...
const solarSpec: KindSpec = {
  ...
  sizes: SMMD,
  sockets: ["top", "face"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], 0.5),
  massKg: [60, 160],
  hp: [40, 80],
...
function weaponSpec(k: WeaponKind): KindSpec {
  return {
    ...
    envelope: (_i, s) =>
      k.faceOnly
        ? outward(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s] * 2.1, SHIP_SIZE_CELLS[s] * 0.9)
        : topMount(
            SHIP_SIZE_CELLS[s],
            SHIP_SIZE_CELLS[s],
            r2(SHIP_SIZE_CELLS[s] * (k.heightScale ?? 0.85)),
            r2(SHIP_SIZE_CELLS[s] * (k.forward ?? 0.6)),
          ),
...
const scannerSpec: KindSpec = {
  ...
  sizes: SMMD,
  sockets: ["top", "face"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], r2(SHIP_SIZE_CELLS[s] * 1.8)),
  massKg: [90, 220],
  hp: [50, 100],
  crew: { automation: "computer", station: "sensor" },
...
const beaconSpec: KindSpec = {
  ...
  sizes: SMMD,
  sockets: ["top"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], r2(SHIP_SIZE_CELLS[s] * 2)),
  massKg: [70, 180],
  hp: [40, 90],
  crew: { automation: "computer" },
...
const tractorSpec: KindSpec = {
  ...
  sizes: SML,
  sockets: ["top", "face", "bottom"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], r2(SHIP_SIZE_CELLS[s] * 0.85), r2(SHIP_SIZE_CELLS[s] * 0.3)),
...
const miningSpec: KindSpec = {
  ...
  sizes: SML,
  sockets: ["top", "face", "bottom"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], r2(SHIP_SIZE_CELLS[s] * 0.85), r2(SHIP_SIZE_CELLS[s] * 0.7)),
...
const droneBaySpec: KindSpec = {
  ...
  sizes: MDLG,
  sockets: ["top", "bottom"],
  envelope: (_i, s) => topMount(SHIP_SIZE_CELLS[s], SHIP_SIZE_CELLS[s], r2(SHIP_SIZE_CELLS[s] * 0.6)),
...
const cargoDoorSpec: KindSpec = {
  ...
  cells: (i) => [[2, 4, 6][i], 1],
  envelope: (i) => edge([2, 4, 6][i], 0.25, [2.75, 3.25, 3.375][i]),
...
const dockingPortSpec: KindSpec = {
  ...
  envelope: (i) => edge([2, 3][i], [0.6, 0.8][i], [2.75, 3.25][i]),
  massKg: [700, 1500],
  hp: [320, 540],
  armor: [6, 8],
  destroyedEffect: "air-leak",
