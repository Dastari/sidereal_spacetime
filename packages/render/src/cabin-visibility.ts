...
export function isCabinVisual(node: Pick<TransformNode, "name" | "metadata">) {
  ...
  return (
    ["floor", "equipment", "cargo"].includes(metadata?.category) ||
...
export function createCabinVisibility(
  ...
  placements: readonly {
    ...
    lighting: {
      ...
      setCabinVisible?: (value: boolean) => void;
    };
  }[],
  ...
  const fixtureLights = placements.filter((p) => isCabinVisual(p.node));
  let visible = true;
  return {
    update(
      next: boolean,
      ...
      for (const placement of fixtureLights) {
        ...
        if (
          placement.lighting.setPowered &&
          placement.lighting.setCabinVisible
        ) {
