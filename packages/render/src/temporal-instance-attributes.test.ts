...
import {
  createTemporalInstanceAttributes,
...
test("linear temporal velocity binds all previous instance columns on original PBR and standard materials", () => {
  ...
  for (const material of materials) {
    ...
    plugin.getAttributes(attributes, scene, mesh);
    expect(attributes.slice(5)).toEqual([
      "previousWorld0",
      "previousWorld1",
      "previousWorld2",
      "previousWorld3",
    ...
    plugin.getAttributes(regular, scene, mesh);
    expect(regular).toEqual(["position"]);
    prepareTemporalInstanceAttributes(scene);
    expect(
      material.pluginManager!.getPlugin("TemporalInstanceAttributes"),
    ).toBe(plugin);
  ...
  expect(
    late.pluginManager?.getPlugin("TemporalInstanceAttributes"),
  ).toBeTruthy();
  registry.dispose();
  scene.dispose();
  engine.dispose();
