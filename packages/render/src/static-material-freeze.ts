...
export function createStaticMaterialFreeze(scene: Scene) {
        ...
        for (const wrapper of binding._drawWrappers)
          if (wrapper) {
            if (wrapper.defines && typeof wrapper.defines !== "string")
              wrapper.defines.markAllAsDirty();
    ...
    for (const mesh of scene.meshes) {
      ...
      for (const sub of mesh.subMeshes ?? []) {
        ...
        if (entry.epoch !== epoch) {
          ...
          entry.eligible =
            ...
            !material
              ...
              .some(
                (texture) =>
                  texture.animations.length > 0 ||
                  texture.getClassName() === "VideoTexture",
              );
        }
        entry.eligible &&= !!eligible;
        const binding =
          mesh instanceof InstancedMesh
            ? (mesh.sourceMesh.subMeshes[sub._id] ?? sub)
            : sub;
        ...
        if (mesh.isEnabled() && mesh.isVisible && mesh.visibility > 0) {
          ...
          for (const wrapper of binding._drawWrappers)
            if (wrapper?.defines && typeof wrapper.defines !== "string")
              entry.dirty ||= wrapper.defines.isDirty;
        }
      }
    }
    for (const [material, entry] of entries) {
  ...
  return {
    prepare() {
      scene.onBeforeRenderObservable.makeObserverBottomPriority(before);
    },
    invalidate,
    dispose() {
      if (disposed) return;
