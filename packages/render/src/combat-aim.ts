...
export function createCombatAim(
  ...
  function aim(localX: number, localY: number, range = 60) {
    ...
    const dx = local.x - localX,
      dy = -local.z - localY,
      length = Math.hypot(dx, dy);
    if (length < 0.05) return;
    const ratio = Math.min(1, range / length);
    local.x = localX + dx * ratio;
    local.z = -localY - dy * ratio;
    return {
      angle: Math.atan2(dx, dy),
      target: Vector3.TransformCoordinates(local, ship.getWorldMatrix()),
    };
  }
  return {
    meshes: [beam, dot],
    ...
    update(
      ...
      endpoint = ray.end;
      beam.position.copyFrom(muzzle.position.add(endpoint).scale(0.5));
      beam.scaling.set(
        0.012,
        0.012,
        Vector3.Distance(muzzle.position, endpoint),
      );
