...
export function poolAuthoredMaterials(
  ...
  for (const material of materials) {
    ...
    if (containers.some(c => c.materials.includes(material) && c.animationGroups.length)) continue;
    if (!(material instanceof PBRMaterial) || material.getClassName() !== "PBRMaterial" ||
        material.getActiveTextures().length || material.animations?.length || material.isFrozen ||
        material.imageProcessingConfiguration !== scene.imageProcessingConfiguration ||
        material.clipPlane || material.clipPlane2 || material.clipPlane3 ||
        material.clipPlane4 || material.clipPlane5 || material.clipPlane6 ||
        material.metadata?.mutableMaterial || material.onBindObservable.hasObservers() ||
        material.onUnBindObservable.hasObservers()) continue;
    ...
    const builtins = new Set([material.brdf, material.clearCoat, material.iridescence,
      material.anisotropy, material.sheen, material.subSurface, material.detailMap]);
    const plugins = (material.pluginManager as unknown as { _plugins?: unknown[] })?._plugins ?? [];
    if (plugins.some(p => !builtins.has(p as typeof material.brdf)) || typeof material.customShaderNameResolve === "function") continue;
    const users = [...meshes].filter(m => m.material === material);
    if (!users.length || users.some(m => !immutableSources.has(m as Mesh) ||
        !["equipment", "cargo"].includes(m.metadata?.role) || m.skeleton || m.morphTargetManager ||
        m.animations.length || m.bakedVertexAnimationManager || m.metadata?.mutableMaterial || m.metadata?.cutawayFade ||
        material.needAlphaBlendingForMesh(m) || material.needAlphaTestingForMesh(m))) continue;
    // Never adopt a material used by another scene owner or a MultiMaterial.
    if (scene.meshes.some(m => !meshes.has(m) && m.material === material) ||
        [...scene.multiMaterials, ...containers.flatMap(c => c.multiMaterials)]
          .some(m => m.subMaterials.includes(material))) continue;
  ...
  for (const group of groups.values()) {
    ...
    for (const container of containers)
      container.materials = container.materials.filter(m => !originals.has(m as PBRMaterial));
    for (const material of group) material._parentContainer = null;
    owned.add(canonical);
    for (const mesh of meshes)
      if (originals.has(mesh.material as PBRMaterial)) mesh.material = canonical;
