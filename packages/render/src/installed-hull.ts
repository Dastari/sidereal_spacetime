...
export async function loadInstalledHull(scene: Scene, parent: TransformNode) {
  ...
  const entries = manifest.entries.filter((e) => e.placements.length),
    prototypes = await loadEquipmentPrototypes(
      scene,
      entries.map((e) => e.asset),
    );
  const placements = entries.flatMap((e) =>
    e.placements.map((p) => {
      ...
      for (const mesh of result.meshes) {
        mesh.metadata = {
          ...
          cutawayFade: e.asset.category === "roof",
          shadowStructural: e.asset.category === "roof",
        };
        if (e.asset.category === "roof")
          mesh.name = "GEO-roof-hull-" + p.id + "--" + mesh.name;
      }
      return result;
    }),
  );
