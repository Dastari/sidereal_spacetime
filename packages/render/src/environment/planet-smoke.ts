...
export function createPlanetSmoke(scene: Scene, name: string, recipe: PlanetRecipe, lod: number, prepared?: PackedPlanetGeometry) {
  const coverage = planetEffects(recipe).smoke;
  if (lod >= 2 || coverage <= 0) return undefined;
  const geometry = prepared ?? buildPlanetClouds({seed: recipe.seed + 909, coverage, radius: 1.145, detail: 48,
