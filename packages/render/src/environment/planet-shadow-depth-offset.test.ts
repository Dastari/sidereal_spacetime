...
test("shadow slope offset restores prior engine state on each mesh, unbind and disposal", () => {
  ...
  vi.spyOn(engine, "setZOffset").mockImplementation((value) => {
    slope = value;
  });
  vi.spyOn(engine, "setZOffsetUnits").mockImplementation((value) => {
    units = value;
  });
  ...
  try {
    ...
    shadows
      .getShadowMap()!
      .onAfterUnbindObservable.notifyObservers(shadows.getShadowMap()!);
    expect([slope, units]).toEqual([3, 7]);
    shadows.onBeforeShadowMapRenderMeshObservable.notifyObservers(mesh);
    dispose();
    expect([slope, units]).toEqual([3, 7]);
