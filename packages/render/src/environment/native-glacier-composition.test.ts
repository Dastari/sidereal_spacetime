...
const kit:NativePlanetKit={schema:"sidereal.native-planet-kit.v1",layout:"clustered-glaciers",materials:[{name:"snow",linearColor:[1,1,1],roughness:.8}],variants:["snow-cluster-a","snow-cluster-b","snow-cluster-c","ice-bluff-a","ice-bluff-b","ice-canyon","sealing-core"].map(name=>({...form,name}))};
describe("native volumetric glacier assembly",()=>{
 it("repeats seeded native volume placement and distributes bounded hero regions",()=>{
  ...
  expect(a.tiles).toHaveLength(4096);expect(a.triangles).toBeLessThan(150000);
  const fraction=a.tiles.filter(v=>v>=3).length/a.tiles.length;
  expect(fraction).toBeGreaterThan(.18);expect(fraction).toBeLessThan(.32);
