...
export function composeNativeGlacialInterior(kit: NativePlanetKit, seed: number, coverage: number, cells: 6 | 10 | 16 = 16, relief = .55): NativePlanetComposition {
  ...
  for(let face=0;face<6;face++)for(let row=0;row<cells;row++)for(let col=0;col<cells;col++){
    const u=-1+(col+.5+(random()-.5)*.64)*step,v=-1+(row+.5+(random()-.5)*.64)*step;
    ...
    const yaw=Math.round((Math.sin(field[0]*9)+Math.cos(field[2]*7))*1.3)*.22,tangent=t.map((x,j)=>x*Math.cos(yaw)+b[j]*Math.sin(yaw)) as V,bitangent=cross(n,tangent);
    ...
    const boundary=1-.082*Math.min(1.35,coverage/.55)+ripple;
    const basin=Math.max(0,(alignment-boundary)/(1-boundary));
    const branch=Math.abs(field[2]+.13*Math.sin(field[0]*7+field[1]*4));
    const branch2=Math.abs(field[0]+.15*Math.sin(field[1]*6-field[2]*4));
    const channel=coverage>0 && ((branch<.095 && Math.abs(field[0])>.20) || branch2<.09);
    const smallCrater=coverage>0 && Math.abs(field[0])>.35 && Math.abs(field[1])>.35 && Math.abs(field[2])>.35 && Math.abs(field[0])+Math.abs(field[1])+Math.abs(field[2])>1.715;

    const ice=coverage>0 && (basin>0 || channel || smallCrater);
    ...
    if(ice){
      const ridge=!smallCrater && (!channel || (branch2>.055 && branch>.055)) && basin<.5;
      const spires=ridge && basin>.1 && Math.sin(field[0]*17)+Math.cos(field[1]*15+field[2]*9)>1.35;
      variant=ridge?(spires?4:3):5;
      radial=(ridge?.83:.68)+region;
      const height=ridge?(spires?.12:.14+.020*Math.sin(field[0]*13+field[2]*17)):.075;
      scale=[cellSize*1.24,cellSize*1.24,height*(.65+relief*.35/.55)];
    }else{
