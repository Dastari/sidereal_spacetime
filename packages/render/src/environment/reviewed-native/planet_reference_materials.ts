...
import {
  ...
  type ReferenceTransmissionRole,
...
export function referenceMaterial(scene: Scene, role: ReferenceMaterialRole) {
  const material = new PBRMaterial(role.name, scene);
  ...
  material.metallic = role.metallic ?? 0;
  material.maxSimultaneousLights = 4;
  if (role.emissiveColor)
    material.emissiveColor = Color3.FromArray(role.emissiveColor);
  material.emissiveIntensity = role.emissiveStrength ?? 1;
  if (role.ior !== undefined) material.indexOfRefraction = role.ior;
  const coat = role.clearcoatFactor ?? role.clearCoat?.intensity ?? 0;
  material.clearCoat.isEnabled = coat > 0;
  material.clearCoat.intensity = coat;
  material.clearCoat.roughness =
    role.clearcoatRoughnessFactor ?? role.clearCoat?.roughness ?? 0;
  material.alpha =
    role.alphaMode === "BLEND" || role.alphaMode === "MASK"
      ? (role.alpha ?? 1)
      : 1;
  material.backFaceCulling = !role.doubleSided;
  material.twoSidedLighting = role.doubleSided ?? false;
  material.useAlphaFromAlbedoTexture =
    role.useTextureAlpha ??
    (role.alphaMode === "BLEND" || role.alphaMode === "MASK");
  material.alphaCutOff = role.alphaCutoff ?? 0.5;
  material.transparencyMode =
    role.alphaMode === "BLEND"
