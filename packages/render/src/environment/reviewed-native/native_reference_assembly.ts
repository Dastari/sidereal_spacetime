...
export const unit = (v: Vec): Vec => {
  ...
  if (r < 1e-12) throw new Error("Degenerate native direction");
  return v.map((x) => x / r) as Vec;
};
...
export function nativeAssembly(kit: NativePlanetKit) {
  ...
  function emit(
    ...
    const variant = kit.variants.find((v) => v.name === name);
    if (!variant) throw new Error(`Missing native variant ${name}`);
    const starts = batches.map((b) => b.indices.length / 3);
    for (let i = 0; i < variant.indices.length; i += 3) {
      const b = batches[variant.triangleMaterials[i / 3]],
        p = [0, 1, 2].map((k) =>
          transform(
      ...
      const n = unit(
          cross(
            p[1].map((v, k) => v - p[0][k]) as Vec,
            p[2].map((v, k) => v - p[0][k]) as Vec,
          ),
        ),
        start = b.positions.length / 3;
      for (const v of p) {
        b.positions.push(...v);
        b.normals.push(...(smooth ? unit(v) : n));
      }
      b.indices.push(start, start + 1, start + 2);
    }
    batches.forEach((b, m) => {
      const count = b.indices.length / 3 - starts[m];
      if (count)
        b.ranges.push({
          firstTriangle: starts[m],
          triangleCount: count,
          partId: id,
        });
    });
...
export function tangent(
  ...
  return (v: Vec): Vec => {
    const x = (v[0] * c - v[1] * s) * scale,
      y = (v[0] * s + v[1] * c) * scale,
      h = 1 + altitude + v[2] * heightScale;
    if (radial) {
      const direction = unit(
        n.map((k, j) => k + east[j] * x + north[j] * y) as Vec,
      );
...
export function globeAnchor(index: number, count: number, phase: number): Vec {
  const y = 1 - (2 * (index + 0.5)) / count,
    a = index * 2.399963229728653 + phase,
    r = Math.sqrt(1 - y * y);
