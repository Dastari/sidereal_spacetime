...
export function composeVolcanicReference(
  kit: NativePlanetKit,
  seed: number,
  lod: 0 | 1 | 2,
  diagnostic = false,
) {
  if (!Number.isFinite(seed) || ![0, 1, 2].includes(lod))
    throw new Error("Invalid volcanic composition parameters");
  ...
  function emit(
    ...
    const mesh = kit.variants.find((v) => v.name === name) as
    ...
    if (
      ...
      throw new Error("Missing native volcanic normals or UVs");
    if (
      mesh.indices.length % 3 ||
      mesh.triangleMaterials.length !== mesh.indices.length / 3
    )
      throw new Error("Invalid volcanic topology");
    const starts = batches.map((b) => b.indices.length / 3);
    for (let i = 0; i < mesh.indices.length; i += 3) {
      const batch = batches[mesh.triangleMaterials[i / 3]];
      if (!batch) throw new Error("Invalid volcanic material role");
      ...
      if (
        source.some((v) => v.length !== 3 || v.some((x) => !Number.isFinite(x)))
      )
        throw new Error("Invalid volcanic vertex");
      const positions = source.map(transform),
        normals =
          mesh.normals?.length === mesh.positions.length
            ? source.map((p, k) =>
                deformedNormalAt(
    ...
    batches.forEach((b, i) => {
      const triangleCount = b.indices.length / 3 - starts[i];
      if (triangleCount)
        b.ranges.push({ firstTriangle: starts[i], triangleCount, partId });
    });
  }
  emit(
    lod === 0
      ...
      : lod === 1
        ...
        : "ground-sphere-low",
    "volcanic-ground",
    (v) => unit(v).map((x) => x * 0.997) as Vec,
    true,
  );
  const random = seeded(seed),
    phase = random() * Math.PI * 2,
    count = diagnostic ? 1 : 18;
  for (let i = 0; i < count; i++) {
    const anchor: Vec = diagnostic
        ? [0.452, 0.388, 0.794]
        : globeAnchor(i, count, phase),
      scale = diagnostic ? 0.26 : [0.32, 0.29, 0.35, 0.31, 0.3, 0.34][i % 6],
      angle = diagnostic ? 0 : random() * Math.PI * 2;
    emit(
      `volcanic-region-${["a", "b", "c"][i % 3]}`,
      `volcanic-region-${i}`,
      tangent(anchor, scale, angle, 0, true, scale),
    );
  }
