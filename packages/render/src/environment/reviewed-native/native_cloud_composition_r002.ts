...
const ANCHOR_ORDER = [
  6, 9, 1, 14, 4, 11, 0, 15, 3, 12, 7, 8, 2, 13, 5, 10,
] as const;
 ...
 */
export function composeNativeClouds(
  kit: NativePlanetKit,
  seed: number,
  coverage: number,
  phase = 0,
  ...
  if (![seed, coverage, phase].every(Number.isFinite))
    throw new Error("Native cloud inputs must be finite");
  for (const name of new Set(BANKS))
    if (!kit.variants.some((v) => v.name === name))
      throw new Error(`Missing native cloud variant ${name}`);
  const assembly = nativeAssembly(kit),
    random = seeded((seed >>> 0) ^ 0x6d2b79f5),
    count =
      coverage <= 0
        ? 0
        : Math.max(
            2,
            Math.ceil(Math.min(1, coverage) * NATIVE_CLOUD_BANK_LIMIT),
          );
  const directionPhase = random() * Math.PI * 2,
    c = Math.cos(phase),
    s = Math.sin(phase);
  for (let i = 0; i < count; i++) {
    ...
    const scale = 0.15 + random() * 0.11,
      rotation = random() * Math.PI * 2,
      altitude = 0.099 + random() * 0.004;
    const name = BANKS[i % BANKS.length],
      heightScale = name === "cloud-towering-bank" ? 0.058 : 0.065;
    const place = tangent(
      globeAnchor(ANCHOR_ORDER[i], NATIVE_CLOUD_BANK_LIMIT, directionPhase),
      scale,
      rotation,
      altitude,
      true,
    ...
    assembly.emit(name, `cloud-bank-${String(i).padStart(3, "0")}`, (v) => {
      const p = place(v);
      return [c * p[0] + s * p[2], p[1], -s * p[0] + c * p[2]] as Vec;
    });
  }
