...
export function composeHybridMoonReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 if(!recipe||!['planets--temperate-moon-1','planets--temperate-moon-2','planets--ocean-moon-1','planets--ocean-moon-2'].includes(recipe.referenceId)||!Number.isInteger(recipe.layoutSeed)||recipe.scales.length!==12||recipe.scales.some(v=>!Number.isFinite(v)||v<.3||v>.46))throw new Error('Explicit hybrid moon composition recipe required');
