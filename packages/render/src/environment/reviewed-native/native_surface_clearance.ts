...
export function nativeSurfaceFloor(mesh: NativePlanetKit["variants"][number]) {
  const cells = new Map<string, { a: Vec; b: Vec; c: Vec; den: number }[]>(),
    size = 0.15;
  for (let i = 0; i < mesh.indices.length; i += 3) {
    ...
    for (
      let x = Math.floor(Math.min(a[0], b[0], c[0]) / size);
      x <= Math.floor(Math.max(a[0], b[0], c[0]) / size);
      x++
    )
      for (
        let y = Math.floor(Math.min(a[1], b[1], c[1]) / size);
        y <= Math.floor(Math.max(a[1], b[1], c[1]) / size);
        y++
      ) {
        const key = `${x},${y}`,
          list = cells.get(key) || [];
