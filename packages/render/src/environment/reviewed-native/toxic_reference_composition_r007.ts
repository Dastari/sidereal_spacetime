...
export function composeToxicReference(
  ...
  const sourceGround = variant("ground-sphere");
  let relief = 0;
  for (let i = 0; i < sourceGround.positions.length; i += 3)
    relief = Math.max(
      relief,
      Math.hypot(
        sourceGround.positions[i],
        sourceGround.positions[i + 1],
        sourceGround.positions[i + 2],
      ) - 1,
    );
  const substrate = (v: Vec): Vec => {
    ...
    for (const region of regions) {
      ...
      if (Number.isFinite(z))
        depression = Math.max(
          depression,
          0.012 + region.scale * (0.07 - z) + relief,
        );
    }
