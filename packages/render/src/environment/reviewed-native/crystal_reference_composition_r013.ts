...
export function composeCrystalReference(
  ...
  if (!diagnostic) {
    ...
    const covered = existingEmitterTips
      .filter((p) => Math.hypot(...p) > surface(unit(p)) + 0.005)
      .map(unit);
    const candidates = Array.from({ length: 1536 }, (_, i) => {
      const y = 1 - (2 * (i + 0.5)) / 1536,
        theta = phase + i * 2.399963229728653,
        r = Math.sqrt(1 - y * y);
      return { i, n: [Math.cos(theta) * r, y, Math.sin(theta) * r] as Vec };
    }).filter(
      (v) =>
        !regions.some(
          (region) =>
    ...
    for (let bridge = 0; bridge < 24; bridge++) {
      ...
      for (const candidate of candidates) {
        if (selected.includes(candidate.i)) continue;
      ...
      const east = unit(
        ...
        north = cross(n, east),
        rotation =
          ((Math.imul(seed + i + 31013, 1664525) >>> 0) / 4294967296) *
          Math.PI *
          2,
        c = Math.cos(rotation),
        s = Math.sin(rotation),
        bridgeScale = 0.19 + (i % 3) * 0.018;
      emit("medium-cluster", `bridge-crystals-${bridge}`, (v) => {
