...
export function composeGasReference(kit: NativePlanetKit) {
  ...
  for (const raw of kit.variants) {
    ...
    if (
      variant.normals?.length !== variant.positions.length ||
      variant.uvs?.length !== (variant.positions.length / 3) * 2
    )
      throw new Error(`Native gas attributes missing: ${variant.name}`);
    ...
    for (let i = 0; i < variant.indices.length; i += 3) {
      ...
      for (let k = 0; k < 3; k++) {
        const index = variant.indices[i + k];
        b.positions.push(
          ...rotate(variant.positions.slice(index * 3, index * 3 + 3) as Vec),
        );
        b.normals.push(
          ...rotate(variant.normals.slice(index * 3, index * 3 + 3) as Vec),
        );
        b.uvs.push(...variant.uvs.slice(index * 2, index * 2 + 2));
      }
