...
export function composeTemperateReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 const batches=kit.materials.map(()=>({positions:[] as number[],normals:[] as number[],uvs:[] as number[],indices:[] as number[],ranges:[] as Range[]}));
 const variant=(name:string)=>{const value=kit.variants.find(v=>v.name===name);if(!value)throw new Error(`Missing temperate native variant ${name}`);return value;};
 ...
 function emit(name:string,id:string,transform:(v:Vec)=>Vec,smooth=false){
  ...
  if(mesh.normals?.length!==mesh.positions.length||mesh.uvs?.length!==mesh.positions.length/3*2)throw new Error('Missing authored temperate normals or UVs');
 ...
 const layouts=Array.from({length:14},(_,i)=>({name:['continent-a','continent-b','continent-c'][i%3],scale:[.38,.31,.34,.29,.35,.32,.33][i%7],height:[.22,.27,.19][i%3]}));
 ...
 const grovePlan=[[-.89,.06,.10],[-.65,.12,.10],[-.76,.31,.10],[-.61,-.09,.10],[-.95,-.10,.10],[.03,.32,.16],[.21,.39,.16],[.28,.18,.16],[.03,.51,.16],[.74,-.13,.07],[.92,-.19,.07],[.75,-.34,.07],[-.33,-.38,0],[.43,-.37,0]];
 for(let i=0;i<(diagnostic?1:layouts.length);i++){
  const spec=layouts[i],y=1-2*(i+.5)/layouts.length,theta=phase+i*2.39996323,r=Math.sqrt(1-y*y),n:Vec=diagnostic?unit([.452,.388,.794]):[Math.cos(theta)*r,y,Math.sin(theta)*r],rotation=diagnostic?0:random()*Math.PI*2;
  ...
  for(let g=0;g<grovePlan.length;g++){
   const [gx,gy,raised]=grovePlan[g],groveScale=.070+[.015,.027,0,.020,.010][(g+i)%5],groveAngle=random()*Math.PI*2,gc=Math.cos(groveAngle),gs=Math.sin(groveAngle),vertical=.72+random()*.45;
   emit(g%4===3?'small-grove':'tree-grove',`forest-${i}-${g}`,v=>{
    const lx=gx*spec.scale+(v[0]*gc-v[1]*gs)*groveScale,ly=gy*spec.scale+(v[0]*gs+v[1]*gc)*groveScale,x=lx*c-ly*ss,z=lx*ss+ly*c,h=1+(spec.height+raised)*spec.scale+v[2]*groveScale*vertical;
    const d=unit([n[0]+east[0]*x+north[0]*z,n[1]+east[1]*x+north[1]*z,n[2]+east[2]*x+north[2]*z]);return [d[0]*h,d[1]*h,d[2]*h];
   });
  }
 }
