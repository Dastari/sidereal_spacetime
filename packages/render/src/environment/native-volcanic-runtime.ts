...
export const loadNativeVolcanicKit = createNativeVolcanicKitLoader(
...
export function createNativeVolcanicKitLoader(revision: string) {
  if (!/^volcanic-r\d{3}$/.test(revision))
    throw new Error("Invalid native volcanic revision");
  let pending: Promise<NativePlanetKit> | undefined;
  return () =>
    (pending ??= (async () => {
      const response = await fetch(`/assets/planets/${revision}/kit.json`);
      if (!response.ok)
        throw new Error(`Native volcanic kit HTTP${response.status}`);
      const kit = (await response.json()) as NativePlanetKit;
      if (kit.layout !== "volcanic-geology")
        throw new Error("Unsupported native volcanic layout");
      composeNativePlanet(kit, 0, 0.55, 0.55);
      return kit;
    })().catch((error) => {
      pending = undefined;
      throw error;
...
export function canUseNativeVolcanic(recipe: PlanetRecipe) {
  const effects = planetEffects(recipe),
    defaults = planetEffects(planetRecipe("volcanic", recipe.seed));
  return (
    recipe.style === "volcanic" &&
    recipe.cloudCoverage === 0 &&
    effects.vegetation === 0 &&
    effects.crystalCoverage === 0 &&
    effects.volcanicCoverage === defaults.volcanicCoverage &&
    effects.smoke === defaults.smoke &&
    JSON.stringify(recipe.palette) === JSON.stringify(PLANET_PALETTES.volcanic)
  );
}
  ...
  return {
    get(seed: number, lod: PlanetLOD, coverage = 0.55, relief = 0.55) {
      if (
        ...
        throw new Error("Invalid native volcanic cache recipe");
      ...
      value = composeNativeVolcanic(
        kit,
        seed,
        coverage,
        ([20, 12, 8] as const)[lod],
        relief,
      );
      entries.set(key, value);
      while (entries.size > NATIVE_VOLCANIC_CACHE_LIMIT)
...
export function createNativeVolcanicWorldPlanet(
  scene: Scene,
  name: string,
  recipe: PlanetRecipe,
  lod: PlanetLOD,
  kit: NativePlanetKit,
  cache: ReturnType<typeof createNativeVolcanicCache>,
  revision: string,
) {
  ...
  const planet = createNativeVolcanicPlanet(
    scene,
    name,
    kit,
    recipe.seed,
  ...
  const smoke = createPlanetSmoke(scene, name, recipe, nativeLod);
  if (smoke) smoke.parent = planet.root;
  planet.root.metadata = {
    role: "planet",
    ...planet.root.metadata,
    lod: nativeLod,
    actualCells: geometry.diagnostics?.actualCells,
  ...
  return {
    ...
    updateWeather(age: number, reducedMotion: boolean) {
      planet.update(age, reducedMotion);
