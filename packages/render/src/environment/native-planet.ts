...
import {planetEffects,PLANET_PALETTES,type PlanetRecipe} from "../../../content/src/environment";
...
export function canUseNativeIce(recipe:PlanetRecipe){
 ...
 return recipe.style==="ice" && recipe.cloudCoverage===0 && recipe.emission===0 && effects.vegetation===0 && effects.volcanicCoverage===0 && effects.crystalCoverage===0 && effects.smoke===0 && JSON.stringify(recipe.palette)===JSON.stringify(PLANET_PALETTES.ice);
}
...
export function createNativePlanetCache(kit:NativePlanetKit){
 ...
 return {
  get(seed:number,lod:PlanetLOD,coverage=.55,relief=.55){
   if(!Number.isInteger(seed)||seed<0||seed>4294967295||![0,1,2].includes(lod)||!Number.isFinite(coverage)||coverage<0||coverage>1||!Number.isFinite(relief)||relief<0||relief>1)throw new Error("Invalid native cache recipe");
   ...
   value=composeNativeGlacialInterior(kit,seed,coverage,([16,10,6] as const)[lod],relief);
   entries.set(key,value);
   while(entries.size>NATIVE_PLANET_CACHE_LIMIT)entries.delete(entries.keys().next().value!);
   return value;
  },
...
export function createNativeIcePlanet(scene:Scene,name:string,recipe:PlanetRecipe,lod:PlanetLOD,kit:NativePlanetKit,cache:NativePlanetCache){
 const nativeLod=Math.max(lod,recipe.resolution<48?2:recipe.resolution<64?1:0) as PlanetLOD;
 const coverage=Math.max(0,Math.min(1,.55+(recipe.terrain-.65)*.22+(recipe.seaLevel-.475)*.2));
 const root=new TransformNode(name+"-native-ice",scene),geometry=cache.get(recipe.seed,nativeLod,coverage,recipe.mountains);
