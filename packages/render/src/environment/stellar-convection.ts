  ...
  override getCustomCode(type: string): Record<string, string> | null {
    ...
    if (type === "fragment")
      return {
        ...
        ${
          this.tiled
            ? `finalEmissive*=.5+.5*channel;
        finalEmissive+=vec3(4.2,1.3,.035)*channel*channel*smoothstep(-.35,.5,sin(sp.z*7.+st*.3)+thermal*.4)*(.24+.76*smoothstep(-.4,.6,thermal));
