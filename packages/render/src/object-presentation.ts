...
export function createObjectPresentation(
        ...
        (mesh) =>
          ...
          (mesh.metadata?.partId === id ||
            mesh.metadata?.trianglePlacements?.some(
              (r: { placementId: string }) => r.placementId === id,
            )),
      )
    )
  ...
  const click = (event: PointerEvent) => {
    ...
    const candidate = picked
      ? (placementAtFace(picked, hit!.faceId) as string | undefined)
      : undefined;
  ...
  function lights(
    ...
    const missing = [...lightStates.keys()]
    ...
    for (const state of [...states, ...missing]) {
      ...
      const placement = installed.find(
        (p) => p.node.metadata?.partId === state.placementId,
      );
      if (!placement) continue;
      lightStates.set(state.placementId, state.enabled);
      if (placement.lighting.setPowered)
        placement.lighting.setPowered(state.enabled);
      else
        for (const light of placement.lighting.lights)
  ...
  return {
    ...
    dispose() {
      ...
      for (const [mesh, source] of sources)
        if (!mesh.isDisposed()) mesh.material = source;
      for (const material of offMaterials.values())
