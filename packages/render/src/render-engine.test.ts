...
import { createRenderBackendPreference, readRenderBackend, RENDER_BACKEND_STORAGE_KEY } from "./render-backend";
...
test("backend preference defaults to WebGL and separates selection, active fallback and reload", () => {
  ...
  pref.set("webgpu");
  expect(pref.snapshot()).toMatchObject({requested:"webgpu",active:"webgl",reloadRequired:true});
  expect(data.get(RENDER_BACKEND_STORAGE_KEY)).toBe("webgpu");
  const fallback = createRenderBackendPreference("webgl",readRenderBackend(storage),storage,"Unavailable");
...
test("engine selection loads WebGPU only on request and falls back without retaining a failed engine", async () => {
  const engine = new NullEngine(), webgpu = vi.fn(async()=>undefined), webgl = vi.fn(()=>engine);
  ...
  const selected = await chooseRenderEngine("webgpu",{webgl,webgpu:async()=>engine});
  expect(selected.active).toBe("webgpu");
  const locked = await chooseRenderEngine("webgpu",{webgpu:async()=>{throw Error("device failed");},webgl:()=>{throw Error("context locked");}});
  expect(locked).toMatchObject({engine:undefined,reloadWithWebGL:true,active:"webgl"});
