...
test("snapshot admission resets on draw membership changes and excludes animated or temporal frames", () => {
  ...
  ready.mockReturnValue(true);tick();
  expect(snapshot.snapshot().armed).toBe(true);
  scene.getActiveMeshes().push(mesh);vi.spyOn(scene,"getActiveIndices").mockReturnValue(36);
  scene.onAfterRenderObservable.notifyObservers(scene);
  Object.defineProperty(engine,"snapshotRendering",{value:true,writable:true});
  Object.defineProperty(engine,"snapshotRenderingMode",{value:1,writable:true});
  Object.defineProperty(engine,"_snapshotRendering",{value:{play:true,_allBundleLists:[{numDrawCalls:2},{numDrawCalls:3}]}});
  scene.getActiveMeshes().reset();
  expect(snapshot.activeStats()).toMatchObject({drawCalls:5,activeMeshes:1,activeIndices:36,meshesByRole:{equipment:{active:1}}});
...
test("snapshot revisions observe transforms, material state, geometry edits and external draw revisions", () => {
  ...
  const map=shadow.getShadowMap()!;map.refreshRate=0;map._shouldRender();
  previous=revision.capture(scene,1);map._shouldRender();expect(revision.capture(scene,1)).toBe(previous);
  map.resetRefreshCounter();expect(revision.capture(scene,1)).toBeGreaterThan(previous);
  const multi=new MultiMaterial("authored-surfaces",scene);multi.subMaterials=[material];mesh.material=multi;
  previous=revision.capture(scene,1);material.alpha=.8;
  expect(revision.capture(scene,1)).toBeGreaterThan(previous);
...
test("snapshot and Flight caches hand off geometry ownership and invalidate edits before replay", () => {
  ...
  vi.spyOn(scene,"freezeActiveMeshes").mockImplementation((_skip,success)=>{success?.();return scene;});
  const driver={enableSnapshotRendering:vi.fn(),disableSnapshotRendering:vi.fn(),updateMesh:vi.fn(),updateMeshesForEffectLayer:vi.fn(),dispose:vi.fn()};
  const snapshot=createFastSnapshot(scene,()=>driver),flight=createFlightActiveSet(scene);
  const frame={ready:true,reducedMotion:false,temporal:false,displayRevision:0};
  const tick=()=>{const candidate=snapshot.prepare(frame);flight.prepare(!candidate);scene.onBeforeRenderObservable.notifyObservers(scene);};
  for(let cycle=0;cycle<2;cycle++){
    for(let i=0;i<5;i++)tick();expect(flight.snapshot().frozen).toBe(true);
    frame.reducedMotion=true;for(let i=0;i<4;i++)tick();
    expect(snapshot.snapshot().armed).toBe(true);expect(flight.snapshot().frozen).toBe(false);
    mesh.updateVerticesData("position",mesh.getVerticesData("position")!);
    expect(snapshot.snapshot().armed).toBe(false);expect(driver.disableSnapshotRendering).toHaveBeenCalled();
    for(let i=0;i<4;i++)tick();expect(snapshot.snapshot().armed).toBe(true);
    snapshot.invalidate();expect(snapshot.snapshot().armed).toBe(false);
    frame.reducedMotion=false;
  }
  snapshot.prepare(frame);flight.prepare(false);
  expect(mesh.geometry!.onGeometryUpdated).toBe(original);
  snapshot.dispose();flight.dispose();scene.dispose();engine.dispose();
});

test("the real snapshot helper preserves hidden morph configuration and instance culling flags", () => {
  const engine=new NullEngine(),scene=new Scene(engine);new FreeCamera("camera",new Vector3(0,0,-5),scene);
  Object.defineProperty(engine,"isWebGPU",{value:true});vi.spyOn(scene,"isReady").mockReturnValue(true);
  const source=CreateBox("prototype",{},scene);source.metadata={role:"hull",partId:"prototype"};source.isVisible=false;
  const instance=Object.assign(source.createInstance("placement"),{ignoreCameraMaxZ:true});instance.metadata={role:"hull",partId:"placement"};
  const unflagged=source.createInstance("unflagged");unflagged.metadata={role:"hull",partId:"unflagged"};
  instance.ignoreCameraMaxZ=true;
  const variant=CreateBox("hidden-variant",{},scene);variant.metadata={role:"crew",partId:"variant"};variant.setEnabled(false);
  variant.ignoreCameraMaxZ=true;
  const manager=new MorphTargetManager(scene),target=new MorphTarget("pose",0,scene);
  target.setPositions(variant.getVerticesData("position")!);manager.addTarget(target);variant.morphTargetManager=manager;
  const influences=manager.numMaxInfluencers,owner=createFastSnapshot(scene);
  for(let i=0;i<4;i++){owner.prepare({ready:true,reducedMotion:true,temporal:false,displayRevision:0});scene.onBeforeRenderObservable.notifyObservers(scene);}
  expect(owner.snapshot().armed).toBe(true);
  expect(manager.numMaxInfluencers).toBe(influences);expect(variant.ignoreCameraMaxZ).toBe(true);expect(instance.ignoreCameraMaxZ).toBe(true);
  expect(Object.hasOwn(unflagged,"ignoreCameraMaxZ")).toBe(false);
  source.ignoreCameraMaxZ=true;
  expect(owner.prepare({ready:true,reducedMotion:true,temporal:false,displayRevision:0})).toBe(false);
  expect(owner.snapshot()).toMatchObject({armed:false,reason:"Per-mesh far-plane override"});
