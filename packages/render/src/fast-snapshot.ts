...
export function createFastSnapshot(scene: Scene, driverFactory?: () => Driver) {
  ...
  let recorded:
    ...
    | undefined;
  const disable = () => {
    if (armed) driver?.disableSnapshotRendering();
    armed = false;
    stable = 0;
  };
            ...
            : scene.meshes.some(
                  (m) =>
                    ...
                    (m instanceof InstancedMesh
                      ? m.sourceMesh.ignoreCameraMaxZ
                      : m instanceof Mesh && m.ignoreCameraMaxZ),
                )
              ? "Per-mesh far-plane override"
              : scene.meshes.some(
                    (m) =>
                      m.isEnabled() && m.isVisible && !!m.morphTargetManager,
                  )
                ? "Morph targets require qualification"
                : "";
      ...
      if (!driver) {
        if (driverFactory) driver = driverFactory();
          ...
          const flags = scene.meshes.map((mesh) => {
            const flagged = mesh as typeof mesh & {
              ignoreCameraMaxZ?: boolean;
            };
            return [
              flagged,
              flagged.ignoreCameraMaxZ,
              Object.hasOwn(flagged, "ignoreCameraMaxZ"),
            ] as const;
          });
            ...
            scene.meshes.flatMap((mesh) =>
              mesh.morphTargetManager
                ? [
                    [
                      mesh.morphTargetManager,
                      mesh.morphTargetManager.numMaxInfluencers,
                    ] as const,
                  ]
                : [],
            ),
          ...
          try {
            ...
            for (const [mesh, ignore, owned] of flags) {
              if (owned) mesh.ignoreCameraMaxZ = ignore;
              else delete mesh.ignoreCameraMaxZ;
            }
            ...
            for (const [manager, influencers] of morphs)
              manager.numMaxInfluencers = influencers;
          }
        }
      }
      driver.enableSnapshotRendering();
      armed = true;
    }
    if (armed) {
      for (const mesh of scene.meshes)
        if (
          ...
          driver?.updateMesh(mesh);
      for (const layer of scene.effectLayers ?? [])
        driver?.updateMeshesForEffectLayer(layer, false);
    }
    reason = armed
      ? engine.snapshotRendering
        ? ""
        : "Preparing bundles"
      : "Waiting for stable scene";
  });
  const after = scene.onAfterRenderObservable.add(() => {
    ...
    if (armed && engine.snapshotRenderingMode !== 1)
      recorded = {
    ...
    activeStats: () => {
      ...
      const replay = (
        engine as unknown as {
          _snapshotRendering?: {
            play: boolean;
            _allBundleLists: { numDrawCalls: number }[];
          };
        }
      )._snapshotRendering;
      ...
      return {
        ...
        ...(bundles?.length
          ? {
              drawCalls: bundles.reduce(
                (sum, bundle) => sum + bundle.numDrawCalls,
                0,
              ),
            }
          : {}),
