...
export function createAssemblyLayoutPreview(
  ...
  let lastId = "";
  let measuredDocument: LayoutDocument | undefined;
  let measuredVisibility = "";
  return {
    getCamera: view.getCamera,
    setMeasuring: view.setMeasuring,
    measureAt: view.measureAt,
    clearMeasurements: view.clearMeasurements,
    removeMeasurementPoint: view.removeMeasurementPoint,
    ...
    update(
      ...
      if (measuredDocument !== doc || measuredVisibility !== visibility) {
        view.clearMeasurements();
        measuredDocument = doc;
