...
export function createPoseState() {
  ...
  return {
    step(input: PoseIntent, delta: number) {
      if (![input.yaw, input.pitch, input.facing, delta].every(Number.isFinite))
        throw new Error("Non-finite pose intent");
      const dt = clamp(delta, 0, 0.1),
        p = EQUIPMENT_POSE_PROFILES[input.profile];
      if (!initialized) {
        heading = input.facing;
        yaw = input.facing;
        initialized = true;
      }
      if (input.itemId !== itemId) {
        itemId = input.itemId;
        sequence = input.shotSequence;
        recoil = 0;
      } else if (input.shotSequence !== undefined) {
        if (
          sequence !== undefined &&
          input.shotSequence > sequence &&
          !input.hidden &&
          !input.seated
        )
          recoil = 1;
        if (sequence === undefined || input.shotSequence > sequence)
          sequence = input.shotSequence;
      }
      const target =
        input.active &&
      ...
      const t = input.reducedMotion
        ...
        : 1 -
          Math.exp(
            -dt / (target > weight ? p.acquireSeconds : p.releaseSeconds),
          );
      ...
      if (input.seated) {
        ...
        const difference = angle(requested - heading),
          amount = clamp(
            (Math.abs(difference) - p.turnStart) / (p.turnFull - p.turnStart),
            0,
            1,
          ),
        ...
        pitch = approach(
          pitch,
          target
            ? clamp(input.pitch, -p.maxUpperBodyPitch, p.maxUpperBodyPitch)
            : -0.45,
          p.aimRate,
          dt,
        );
      }
      recoil = exponential(recoil, 0, 0.09, dt);
      return {
        heading,
        yaw,
        pitch,
