...
export async function createVoxelCrewVisual(
  ...
  const clips = new Map<string, AnimationGroup>(container.animationGroups.map((g) => [g.name, g]));
  ...
  const armedLoops = (clip: string) => /\.(idle_armed|walk_armed|run_armed|aim)$/.test(clip);
  const loops = (clip: VoxelCrewAction) => (clip.includes(".") ? armedLoops(clip) : voxelCrewLoops(clip));
  ...
  const update = (input: VoxelCrewMotion) => {
    ...
    if (armedClass && !motion.seated && !motion.dead && !motion.downed && !motion.climbing) {
      const has = (c: string) => clips.has(`${armedClass}.${c}`);
      const name = (c: string) => `${armedClass}.${c}` as VoxelCrewAction;
      const aimClip = motion.combat && !motion.sprinting && has("aim") ? name("aim") : undefined;
      if (motion.moving) {
        const loco = motion.sprinting && has("run_armed") ? name("run_armed") : has("walk_armed") ? name("walk_armed") : undefined;
        const speedRatio = voxelCrewSpeedRatio(motion.sprinting ? "run" : "walk", motion);
        if (loco) layers = aimClip ? { lower: loco, upper: aimClip, speedRatio } : { full: loco, speedRatio };
      } else if (has("idle_armed")) layers = { full: aimClip ?? name("idle_armed"), speedRatio: 1 };
    }
    ...
    if (motion.shotSequence !== undefined && lastShot !== undefined && motion.shotSequence !== lastShot && weapon !== "none")
      startOneShot(
        armedClass && clips.has(`${armedClass}.shoot`)
          ...
          : weapon === "rifle" ? "shoot_rifle" : "shoot_pistol",
        motion,
      );
  ...
  const apply = (motion: VoxelCrewMotion, restartOneShot = false) => {
    ...
    if ("full" in layers) {
      if (shot?.layer === "full") {
        ...
        wanted.push({ clip: layers.full, mask: "lower", loop: loops(layers.full), speed: layers.speedRatio });
        wanted.push({ clip: shot.clip, mask: "upper", loop: false, speed: 1, restart: restartOneShot });
      } else wanted.push({ clip: layers.full, mask: "full", loop: loops(layers.full), speed: layers.speedRatio });
    } else {
      wanted.push({ clip: layers.lower, mask: "lower", loop: true, speed: layers.speedRatio });
      if (shot && shot.layer === "upper")
        wanted.push({ clip: shot.clip, mask: "upper", loop: false, speed: 1, restart: restartOneShot });
      else wanted.push({ clip: layers.upper, mask: "upper", loop: loops(layers.upper), speed: 1 });
    }
  ...
  return {
    ...
    addClips(source: AssetContainer) {
      ...
      for (const group of source.animationGroups) {
        const clone = group.clone(group.name, (target: { name?: string }) =>
          (target?.name && (joints.get(target.name) ?? nodes.get(target.name))) || target,
        );
        clone.stop();
