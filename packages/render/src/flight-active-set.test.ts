...
test("Flight caches the real active list without freezing mesh transforms and drops it on changes", () => {
  ...
  const mesh = CreateBox("part", {}, scene);
  ...
  vi.spyOn(material, "isReady").mockReturnValue(true);
  vi.spyOn(material, "isReadyForSubMesh").mockReturnValue(true);
  vi.spyOn(mesh, "render").mockReturnValue(mesh);
  const freeze = vi.spyOn(scene, "freezeActiveMeshes"),
    owner = createFlightActiveSet(scene);
  const geometryCallback = mesh.geometry!.onGeometryUpdated;
  const frame = (flight = true) => {
    owner.prepare(flight);
    scene.render();
  };
  for (let i = 0; i < 6; i++) frame();
  expect(owner.snapshot().frozen).toBe(true);
  expect(freeze).toHaveBeenCalledTimes(1);
  expect(freeze.mock.calls[0][0]).toBe(true);
  expect(freeze.mock.calls[0][3]).toBe(false);
  expect(mesh.isWorldMatrixFrozen).toBe(false);
  expect(scene.getActiveMeshes().data[0]).toBe(mesh);
  mesh.position.x = 2;
  frame();
  expect(owner.snapshot().frozen).toBe(false);
  for (let i = 0; i < 4; i++) frame();
  expect(owner.snapshot().frozen).toBe(true);
  mesh.setEnabled(false);
  frame();
  expect(scene.getActiveMeshes().length).toBe(0);
  ...
  expect(mesh.metadata.partId).toBe("part");
  owner.dispose();
  scene.dispose();
  engine.dispose();
});
...
test("late ready callbacks cannot re-freeze after a view change or disposal", () => {
  ...
  const release = vi.spyOn(scene, "unfreezeActiveMeshes"),
    owner = createFlightActiveSet(scene);
  const frame = () => {
    owner.prepare(true);
...
test("active-list revision covers geometry, material queue classification, camera and placement replacement", () => {
  ...
  const material = new StandardMaterial("material", scene);
  mesh.material = material;
  const revision = createActiveSetRevision();
  let value = revision.capture(scene);
  expect(revision.capture(scene)).toBe(value);
  for (const change of [
    () => {
      material.alpha = 0.4;
    },
    ...
    () => {
      mesh.metadata.partId = "replacement";
    },
    () => {
    ...
    () => {
      mesh
        .getBoundingInfo()
        .reConstruct(new Vector3(-4, -4, -4), new Vector3(4, 4, 4));
    },
  ]) {
    change();
    scene.incrementRenderId();
    const next = revision.capture(scene);
