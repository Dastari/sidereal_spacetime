...
test("static sphere culling is conservative under root movement and restores the original strategy", () => {
  ...
  const camera=new FreeCamera("camera",new Vector3(0,0,-20),scene);camera.setTarget(Vector3.Zero());
  camera.minZ=.1;camera.maxZ=100;camera.getViewMatrix(true);camera.getProjectionMatrix(true);
  const planes=Frustum.GetPlanes(camera.getTransformationMatrix());
  const mesh=CreateBox("long-hull",{width:12,height:1,depth:1},scene);
  mesh.metadata={role:"hull",partId:"hull"};mesh.parent=root;
  mesh.cullingStrategy=AbstractMesh.CULLINGSTRATEGY_OPTIMISTIC_INCLUSION;
  const cache=cacheStaticTransforms(root,[mesh]);
  const bounds=mesh.getBoundingInfo(),boxTest=vi.spyOn(bounds.boundingBox,"isInFrustum");
  let rejected=0;
  for (const x of [-200,-30,-10,0,10,30,200]) {
    root.position.x=x;root.rotation.y=x*.03;cache.refresh();
    const standard=bounds.isInFrustum(planes,AbstractMesh.CULLINGSTRATEGY_STANDARD);
    boxTest.mockClear();
  ...
  cache.dispose();expect(mesh.cullingStrategy).toBe(AbstractMesh.CULLINGSTRATEGY_OPTIMISTIC_INCLUSION);
  scene.dispose();engine.dispose();
});
