...
class TemporalInstanceAttributes extends MaterialPluginBase {
  ...
  override isCompatible(language: ShaderLanguage) {return language === ShaderLanguage.WGSL || language === ShaderLanguage.GLSL;}
  override getAttributes(attributes: string[], scene: Scene) {
    if (!scene.needsPreviousWorldMatrices || !attributes.includes("world0")) return;
    for (let i=0;i<4;i++) {
...
export function prepareTemporalInstanceAttributes(scene: Scene, materials: Iterable<Material> = scene.materials) {
  for (const material of materials) {
    if (!(material instanceof StandardMaterial || material instanceof PBRBaseMaterial)) continue;
    if (!material.pluginManager?.getPlugin("TemporalInstanceAttributes")) new TemporalInstanceAttributes(material);
  }
}
...
export function createTemporalInstanceAttributes(scene: Scene) {
  ...
  const observer=Material.OnEventObservable.add(material=>{
    if (material.getScene() === scene) prepareTemporalInstanceAttributes(scene,[material]);
  },MaterialPluginEvent.Created);
  return {dispose(){Material.OnEventObservable.remove(observer);}};
}
