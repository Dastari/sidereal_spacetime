...
export function prefabFlightInput(
  ...
  // IFCS phase 3 (live authority) has no per-actuator `supply` input; see feat/prefab-ships for phase 4+.
  state: { fittings: readonly FlightFitting[]; cargo?: readonly FlightCargoMass[]; crew?: readonly FlightCrewMass[] },
): FlightDefinitionInput {
  return {
