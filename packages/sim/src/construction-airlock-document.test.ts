...
test('actual accepted door orientations retain closed barriers and supported one-door-at-a-time routes',async()=>{
 const {readFileSync}=await import('node:fs');const {createPublishedNativeExternalAirlockCompiler}=await import('./construction-airlock-plan');const {nativeAirlockCollision}=await import('./construction-airlock-document');const {sweepDeckCircle,canOccupyDeck}=await import('./construction-collision');
 const d=createNativeAirlockDocument(),plan=createPublishedNativeExternalAirlockCompiler(readFileSync('assets/art-library/designs/shipyard.structure.external-airlock/revisions/r000/audit-a007.json'))(d.layout.id);
 const frame=(inner:number,outer:number)=>nativeAirlockCollision(d,plan,[{openingId:d.airlockRoom.innerDoorId,fraction:inner,sealRetraction:inner},{openingId:d.airlockRoom.outerDoorId,fraction:outer,sealRetraction:outer}]);
 const loc=(x:number)=>({shipId:plan.instanceId,deckId:d.airlockRoom.deckId,position:[x,1] as [number,number]});
 expect(sweepDeckCircle(frame(0,0),loc(1),[3,0],.3).position[0]).toBeLessThan(2);
 expect(sweepDeckCircle(frame(1,0),loc(1),[3,0],.3).position[0]).toBeCloseTo(4);
 expect(sweepDeckCircle(frame(0,1),loc(4),[3,0],.3).position[0]).toBeCloseTo(7);
 expect(canOccupyDeck(frame(0,1),loc(7),.3)).toBe(true);expect(canOccupyDeck(frame(0,1),loc(8.1),.3)).toBe(false);
});
