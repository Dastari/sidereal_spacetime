...
export type InventoryLocation = Pick<
  GridItem,
  "containerId" | "equipmentSlot" | "x" | "y" | "rotated"
>;
...
const MAX_ITEMS = 128,
  MAX_CONTAINERS = 152,
  MAX_DEPTH = 6;
export function itemSize(item: GridItem, definition: GridDefinition) {
  return item.rotated
...
export function inventoryMass(
  ...
  const itemMass = (id: string, path: string[] = []): number => {
    ...
    for (const container of snapshot.containers.filter(
      (c) => c.parentItemId === id,
    ))
      mass += containerMass(container, [...path, id]);
    memo.set(id, mass);
    return mass;
  };
  const containerMass = (
    ...
    container.kind === "liquid"
      ...
      : snapshot.items
          ...
          .reduce((sum, item) => sum + itemMass(item.id, path), 0);
  for (const item of snapshot.items) itemMass(item.id);
  return {
    itemMass: (id: string) => itemMass(id),
    containerMass: (id: string) => {
      const c = snapshot.containers.find((c) => c.id === id);
      if (!c) throw new Error("Unknown container");
      return containerMass(c);
    },
  };
}
export function validateInventory(
  ...
  for (const container of snapshot.containers) {
    if (
      ...
      !snapshot.items.some((i) => i.id === container.parentItemId)
    )
      throw new Error("Container has no parent item");
    if (container.kind !== "grid" && container.kind !== "liquid")
      throw new Error("Unknown storage kind");
    if (
      container.kind === "grid" &&
      (!Number.isInteger(container.width) ||
        !Number.isInteger(container.height) ||
        container.width < 1 ||
        container.height < 1 ||
        container.width > 16 ||
        container.height > 16)
    ...
    if (
      ...
      (!Number.isFinite(container.amountLitres) ||
        ...
        !(container.liquidType in liquidDensity))
    )
  ...
  for (const item of snapshot.items) {
    ...
    if (item.equipmentSlot) {
      if (item.equipmentSlot !== definition.equipSlot)
        throw new Error("Item does not fit this equipment slot");
      if (slots.has(item.equipmentSlot))
        throw new Error("Equipment slot occupied");
      slots.add(item.equipmentSlot);
      continue;
    }
    const container = containers.get(item.containerId);
    if (!container || container.kind !== "grid")
      throw new Error("Items require a grid container");
    ...
    if (
      ...
      item.x < 0 ||
      item.y < 0 ||
      item.x + size.width > container.width ||
      item.y + size.height > container.height
    )
      throw new Error("Item extends outside the container");
    for (const other of snapshot.items) {
      ...
      if (
        item.x < other.x + otherSize.width &&
        item.x + size.width > other.x &&
        item.y < other.y + otherSize.height &&
        item.y + size.height > other.y
      )
        throw new Error("Inventory items overlap");
    }
  }
  const mass = inventoryMass(snapshot, definitions, liquidDensity);
  for (const container of snapshot.containers)
    if (mass.containerMass(container.id) > container.maxMassKg + 1e-8)
      throw new Error("Container payload limit exceeded");
  const carried =
    mass.containerMass(pocketsId) +
    snapshot.items
      .filter((i) => i.equipmentSlot)
      .reduce((sum, item) => sum + mass.itemMass(item.id), 0);
  if (carried > carryLimitKg + 1e-8)
    throw new Error("Character carry limit exceeded");
  return carried;
}
    ...
    for (let y = 0; y < container.height; y++)
      for (let x = 0; x < container.width; x++) {
        const width = rotated ? definition.height : definition.width,
          height = rotated ? definition.width : definition.height;
        if (
          x + width > container.width ||
          y + height > container.height ||
          occupied.some(
            (o) =>
              x < o.item.x + o.size.width &&
              x + width > o.item.x &&
              y < o.item.y + o.size.height &&
              y + height > o.item.y,
          )
        )
          continue;
