import { transitionAirlock, type AirlockSide, type AirlockPressure } from "./construction-topology";
import { sealedDoorMotionStep, validateSealedDoorMotion, type SealedDoorMotion, type SealMotionPolicy } from "./construction-seal-motion";
...
const closed = (d: SealedDoorMotion) => d.hingeFraction === 0 && d.sealRetraction === 0 && !d.targetOpen;
function validate(state: NativeAirlockState) {
  for (const side of sides) validateSealedDoorMotion(state[side]);
  if (state.pumpTarget !== null && !sides.includes(state.pumpTarget)) throw Error("Airlock: invalid pump target");
  if (state.inner.targetOpen && state.outer.targetOpen) throw Error("Airlock: conflicting opening targets");
}
function mechanical(state: NativeAirlockState, ctx: NativeAirlockContext) {
  return {
    inner: { open: !closed(state.inner), obstructed: ctx.motion.inner.hingeObstructed || ctx.motion.inner.sealObstructed, sealIntact: ctx.sealsQualified.inner },
    outer: { open: !closed(state.outer), obstructed: ctx.motion.outer.hingeObstructed || ctx.motion.outer.sealObstructed, sealIntact: ctx.sealsQualified.outer },
    powered: ctx.powered, chamberBreached: !ctx.chamberIntact, pumpTarget: state.pumpTarget,
  };
}
...
export function requestNativeAirlock(state: NativeAirlockState, ctx: NativeAirlockContext, command: NativeAirlockCommand, authorized: boolean):
  { ok: true; state: NativeAirlockState } | { ok: false; reason: string } {
  validate(state);
  if (!authorized) return { ok: false, reason: "permission" };
  const decision = transitionAirlock(mechanical(state, ctx), ctx.pressure, command);
  if (!decision.ok) return decision;
  const next = { ...state, inner: { ...state.inner }, outer: { ...state.outer } };
  next.pumpTarget = decision.state.pumpTarget;
  if (command.kind === "open" || command.kind === "close") next[command.side].targetOpen = command.kind === "open";
...
export function stepNativeAirlock(state: NativeAirlockState, ctx: NativeAirlockContext, seconds: number): NativeAirlockState {
  validate(state);
  const next = { ...state, inner: { ...state.inner }, outer: { ...state.outer } };
  if (next.pumpTarget !== null) {
    const check = transitionAirlock(mechanical(state, ctx), ctx.pressure, { kind: "startPump", side: next.pumpTarget });
...
export function nativeAirlockPassageAllowed(state: NativeAirlockState, side: AirlockSide, gate: {
  authorized: boolean; supportedRoute: boolean; destinationExposureAllowed: boolean;
}) {
  validate(state);
  return gate.authorized && gate.supportedRoute && gate.destinationExposureAllowed && state[side].hingeFraction === 1 && state[side].sealRetraction === 1 && !state[side].blocked;
}
