...
test("moving an existing raised panel retains height while its authored plane snaps to the wall", () => {
  const { doc, compiled } = fixture();
  for (const asset of panels) {
    const proposed = {
      ...placement(asset.id, 4.4),
      position: [4.4, 2, 1.1875] as [number, number, number],
    };
    const raised = snapHullAttachment(
      proposed,
      asset,
      doc,
      compiled,
    ...
    expect(raised.part.position).toEqual([4, 2, 1.1875]);
    expect(
      snapHullAttachment(raised.part, asset, doc, compiled, "deck", 2, {
        preserveHeight: true,
      })!.part,
    ).toEqual(raised.part);
    expect(proposed.position).toEqual([4.4, 2, 1.1875]);
  }
});
