...
import { compilePressureTopology, type PressureStructure } from "./construction-topology";
...
export const NATIVE_EXTERNAL_AIRLOCK_AUDIT_SHA256 = "eb7eab8523d68906a302a84cf06a5b2a507229d15e89fb96f66d1ade8b6e0aa6";
interface CompositeAudit {
  schema: string; pass: boolean;
...
export function createPublishedNativeExternalAirlockCompiler(auditBytes: Uint8Array) {
  ...
  return {
    ...
    installation, installationFingerprint: hash(new TextEncoder().encode(JSON.stringify(installation))),
    doors: (["inner", "outer"] as const).map(side => ({ id: `${instanceId}:${side}`, side, originM: (side === "inner" ? [2, 0, 0] : [6, 2, 0]) as [number, number, number], quarterTurns: side === "inner" ? 1 : 3 })),
    cells: [
      { id: `${instanceId}:interior`, volumeM3: audit.freeVolumes.filter(v => v.x < 2).reduce((n, v) => n + v.freeM3, 0) },
      { id: `${instanceId}:chamber`, volumeM3: audit.freeVolumes.filter(v => v.x >= 2).reduce((n, v) => n + v.freeM3, 0) },
    ],
    waitingPositionsM: { interior: [1, 1, .1875], chamber: [4, 1, .1875], exterior: [7, 1, .1875] },
    sealedFloorCount: 12, exteriorFloorCount: 4,
...
export function acceptNativeExternalAirlockInstallation(plan: NativeExternalAirlockPlan, actual: readonly NativeRoomInstalledPart[]) {
  if (actual.length !== plan.installation.length) throw Error("Airlock installation count changed");
  const byId = new Map(actual.map(p => [p.id, p]));
  if (byId.size !== actual.length) throw Error("Airlock installation duplicate identity");
  for (const p of plan.installation) {
    const a = byId.get(p.id);
    if (!a || a.source !== p.source || a.sha256 !== p.sha256 || a.nodePrefix !== p.nodePrefix || a.quarterTurns !== p.quarterTurns || JSON.stringify(a.originM) !== JSON.stringify(p.originM)) throw Error("Airlock installed source or transform changed");
  }
...
export function nativeExternalAirlockPressure(plan: NativeExternalAirlockPlan, deckId: string, state: NativeAirlockState, policy: { id: string; openConductance: number; unsealedConductance: number }) {
  if (plan.auditSha256 !== NATIVE_EXTERNAL_AIRLOCK_AUDIT_SHA256 || !deckId || !policy.id || !Number.isFinite(policy.openConductance) || !Number.isFinite(policy.unsealedConductance) || policy.unsealedConductance <= 0 || policy.openConductance < policy.unsealedConductance || policy.openConductance > 1) throw Error("Airlock pressure: invalid proof or gameplay policy");
  for (const side of ["inner", "outer"] as const) validateSealedDoorMotion(state[side]);
  const [interior, chamber] = plan.cells;
  const structure: PressureStructure = {
    cells: [{ ...interior, deckId, faces: ["shell", "inner"] }, { ...chamber, deckId, faces: ["shell", "inner", "outer"] }],
    boundaries: [
      ...plan.cells.map(c => ({ id: `${c.id}:shell`, a: { cellId: c.id, faceId: "shell" }, b: null, kind: "sealed" as const, pressureDefinitionId: plan.auditSha256 })),
      ...(["inner", "outer"] as const).map(side => {
        const d = state[side], sealed = d.hingeFraction === 0 && d.sealRetraction === 0;
        return { id: `${plan.instanceId}:${side}`, a: { cellId: side === "inner" ? interior.id : chamber.id, faceId: side }, b: side === "inner" ? { cellId: chamber.id, faceId: "inner" } : null,
          ...(sealed ? { kind: "sealed" as const, pressureDefinitionId: plan.auditSha256 } : { kind: "flow" as const, conductanceMolesPerSecondPa: policy.unsealedConductance + (policy.openConductance - policy.unsealedConductance) * d.hingeFraction, sourceDefinitionId: policy.id }) };
      }),
    ],
  };
