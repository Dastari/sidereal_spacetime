...
it("wraps contact headings to (-pi, pi] without changing angular momentum", () => {
  for (const [heading, omega] of [
    [Math.PI, 3],
    [-Math.PI, 0],
    [-Math.PI + 0.01, -3],
    [200 * Math.PI, 0],
  ]) {
    const result = stepContacts([body("ship", 0, { heading, omega })], 1 / 60)
      .bodies[0];
    expect(result.heading).toBeGreaterThan(-Math.PI);
    expect(result.heading).toBeLessThanOrEqual(Math.PI);
    expect(Math.sin(result.heading)).toBeCloseTo(
      Math.sin(heading + omega / 60),
      12,
    );
    expect(Math.cos(result.heading)).toBeCloseTo(
      Math.cos(heading + omega / 60),
      12,
    );
...
it("contact ordering uses plain stable identity comparison", () => {
  ...
  expect(
    stepContacts(
      ids.map((id, i) => body(id, i * 10)),
      1 / 60,
    ).bodies.map((b) => b.id),
  ).toEqual([...ids].sort());
});

it("uses lateral and longitudinal capsule offsets relative to the physical COM", () => {
  const result = stepContacts(
    [
      body("hull", 0, {
        lateralOffset: 5,
      ...
      body("rock", 7, { y: 2, vx: -10 }),
    ],
    1 / 60,
  );
  expect(result.impacts).toBeGreaterThan(0);
  expect(result.bodies[0].omega).toBeGreaterThan(0);
  expect(
    result.bodies.reduce((sum, b) => sum + b.massKg * b.vx, 0),
  ).toBeCloseTo(-10000, 8);
});

it("detects the sweep of a circle whose geometric center is offset from COM", () => {
  const result = stepContacts(
    [
      body("hull", 0, {
        lateralOffset: 5,
      ...
      body("rock", 5, { y: 1.8, radius: 0.5 }),
    ],
    1 / 60,
  );
  expect(result.impacts).toBeGreaterThan(0);
  expect(result.bodies[0].omega).toBeLessThan(12);
  expect(result.bodies[1].vy).toBeGreaterThan(0);
  expect(() =>
    stepContacts([body("invalid", 0, { lateralOffset: NaN })], 1 / 60),
  ).toThrow();
});
