...
function flight(f: ReturnType<typeof fixture>, shipId: string) {
  if (f.db.constructionFlightDirty.shipId.find(shipId))
    compileShipFlight(f.db as any, shipId, (id) => readConstructionFlightInput(f.ctx, id));
  return resolveShipFlightDefinition(
    {
      binding: (id: string) => f.db.constructionFlightBinding.shipId.find(id),
      constructionInstanceExists: (id: string) => !!f.db.constructionInstance.id.find(id),
      currentInstanceRevision: (id: string) => f.db.constructionInstance.id.find(id)?.revision,
      fittings: (id: string) => f.db.constructionFlightFitting.by_ship.filter(id),
...
test("operator assigns fed.s.wren at a berth: complete, owned, active, boarded ship", () => {
  ...
  expect(() => assignPrefabShip(f.ctx, { ...args, expectedCatalogRevision: "ship-components-v1@0" })).toThrow(
    "catalog revision",
  );
  ...
  expect(location).toMatchObject({ instanceId: shipId, deckId: instance.spawnDeckId });
  expect(f.db.gameShipAccess.shipId.find(shipId)).toMatchObject({
    characterId: a.actor.id,
    lifecycle: "active",
    templateSha256: FED_WREN_PIN.blueprintSha256,
  });
  expect(f.db.worldAdmission.characterId.find(a.actor.id)).toMatchObject({ shipId });
  ...
  const ledger = JSON.parse(f.db.shipOperatorOperation.operationId.find("assign-wren-01").summaryJson);
  ...
  expect(() =>
    assignPrefabShip(f.ctx, { ...args, operationId: "assign-wren-02", expectedCharacterShipId: shipId }),
  ).toThrow("already has a ship");
});

test("fed.s.wren honours an explicit pose in an existing system and refuses unknown systems", () => {
  const { f, b } = wiped();
  ...
  expect(() =>
    assignPrefabShip(f.ctx, {
      ...
      spawnPoseJson: JSON.stringify({ kind: "at", systemId: "no-such-system", x: 5000, y: 0, heading: 0 }),
    }),
  ).toThrow("Unknown spawn system");
  const pose = { kind: "at", systemId, x: 5000, y: -2500, heading: 1.25 };
  assignPrefabShip(f.ctx, { ...base, operationId: "assign-wren-at", spawnPoseJson: JSON.stringify(pose) });
  const shipId = f.db.character.id.find(b.actor.id).shipId;
  expect(f.db.shipWorldMotion.shipId.find(shipId)).toMatchObject({ systemId, x: 5000, y: -2500, heading: 1.25 });
  expect(f.db.ship.id.find(shipId)).toMatchObject({ name: "Wren", heading: 1.25 });
  expect(flight(f, shipId).status).toBe("ready");
});

test("fed.s.wren can be the configured starter prefab for new characters", () => {
  const f = fixture();
