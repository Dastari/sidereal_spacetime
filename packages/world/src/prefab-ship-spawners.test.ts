...
test("only fed.s.wren is registered as a non-legacy prefab spawner", () => {
  expect(REGISTERED_PREFAB_PINS.map((p) => p.prefabId)).toEqual(["fed.s.wren"]);
...
test("fed.s.wren pins equal the current grammar/catalog derivation (golden)", () => {
...
test("the spawned fed.s.wren instance document stays under the 1 MiB budget", () => {
