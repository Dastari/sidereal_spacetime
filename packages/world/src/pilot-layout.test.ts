...
function fixture(occupied = true) {
  ...
  let station = {
    ...
    localX: 0,
    localY: 6,
    occupantId: occupied ? "crew" : undefined,
    operational: true,
  };
  let input = {
    characterId: "crew",
  ...
  const ctx = {
    ...
    db: {
      ...
      station: {
        shipId: { find: () => station },
        id: {
          update: (v: typeof station) => {
            station = v;
          },
        },
      },
      character: {
        by_owner: { filter: () => [actor] },
        id: {
          find: () => actor,
          update: (v: typeof actor) => {
            actor = v;
          },
        },
      },
  ...
  return {
    ctx: ctx as unknown as Parameters<typeof alignPilotLayout>[0],
    get: () => ({ actor, station, input, receipt }),
  };
}
describe("pilot fixture migration", () => {
  it("preserves identities, occupancy and operational state, clears commands, records once", () => {
    ...
    expect(first.station).toMatchObject({
      id: "helm",
      occupantId: "crew",
      operational: true,
      localY: 10.25,
  ...
  it("upgrades the recorded revision-one seat without replacing its identity or grant", () => {
    ...
    f.ctx.db.pilotLayoutReceipt.insert({
      shipId: "ship",
      revision: 1,
      stationId: "helm",
      previousX: 0,
    ...
    expect(f.get().station).toMatchObject({
      id: "helm",
      occupantId: "crew",
      operational: true,
      localY: 10.25,
    ...
    expect(f.get().receipt).toMatchObject({
      revision: 3,
      stationId: "helm",
      previousY: 10,
    });
  ...
  it("permits walking from the old deck to the relocated station and keeps crew inside diagonal walls", () => {
    let p = { x: 0, y: 6 };
    for (let i = 0; i < 100; i++)
      p = walk(p.x, p.y, 0, 1, [], false, constrainLabDeck);
    expect(p.y).toBeGreaterThan(10);
    for (let i = 0; i < 500; i++)
      p = walk(p.x, p.y, 1, 1, [], true, constrainLabDeck);
