...
export function installPrefabShip(ctx: Context, actor: PrefabCharacterRow, request: PrefabShipRequest): { shipId: string; deckId: string } {
  ...
  let placement: { systemId: string; x: number; y: number; serverTick: bigint };
  if (request.pose.kind === "berth") {
    const system = ensureCanonicalSystem(ctx.db);
    placement = { systemId: system.id, ...reserveBerth(ctx.db, system.id, model.hull.radius + model.hull.halfLength), serverTick };
  } else {
    // Explicit poses must name an existing simulated system; nothing here creates one.
    if (!ctx.db.worldSystem.id.find(request.pose.systemId))
      throw new SenderError("Unknown spawn system " + request.pose.systemId);
    placement = { systemId: request.pose.systemId, x: request.pose.x, y: request.pose.y, serverTick };
  }
