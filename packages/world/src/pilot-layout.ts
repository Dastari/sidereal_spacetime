...
export function alignPilotLayout(ctx: Context, shipId: string) {
  ...
  if (!ship || !ship.owner.isEqual(ctx.sender))
    throw new SenderError("Ship unavailable");
  ...
  if (
    station.localX !== 0 ||
    ![6, 10, PILOT_LAYOUT.station.y].includes(station.localY)
  )
    throw new SenderError("Pilot fixture requires explicit migration review");
  const occupant = station.occupantId
    ? ctx.db.character.id.find(station.occupantId)
    : undefined;
  if (
    station.occupantId &&
    (!occupant ||
      occupant.shipId !== shipId ||
      !occupant.owner.isEqual(ctx.sender))
  )
  ...
  for (const actor of ctx.db.character.by_owner.filter(ctx.sender)) {
    ...
    const point =
      actor.id === occupant?.id
        ? PILOT_LAYOUT.station
        : repairLabDeckPosition(actor.localX, actor.localY);
    commitFlightCharacter(
      ctx,
      { ...actor, localX: point.x, localY: point.y, sprinting: false },
  ...
  const nextReceipt = {
    shipId,
    revision: PILOT_LAYOUT.revision,
    stationId: station.id,
    previousX: station.localX,
    previousY: station.localY,
    appliedMicros: ctx.timestamp.microsSinceUnixEpoch,
  };
