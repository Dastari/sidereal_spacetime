...
import type { CompiledFlightActuator } from "@sidereal/sim/flight-definition";
type PhysicalViewContext = FlightViewContext & {
  db: {
...
export function ownAuthoredFlightPhysics(ctx: PhysicalViewContext) {
  ...
  return bindings
    .filter((b) => b.owner.isEqual(ctx.sender))
    .map((b) => {
      const c = ctx.db.constructionFlightCompiled.shipId.find(b.shipId),
        pending = !!ctx.db.constructionFlightDirty.shipId.find(b.shipId);
      return {
        shipId: b.shipId,
        revision: c?.revision ?? 0n,
        status: pending ? "pending" : (c?.status ?? "pending"),
        reason: pending || !c ? "flight-compilation-pending" : c.reason,
        massKg: c?.massKg ?? 0,
        centerX: c?.centerX ?? 0,
        centerY: c?.centerY ?? 0,
        inertiaKgM2: c?.inertiaKgM2 ?? 0,
        envelopeJson: c?.envelopeJson ?? "null",
        definitionHash: c?.definitionHash ?? "",
      };
    });
}
export const authoredFlightActuatorProjection = t.row(
  "AuthoredFlightActuator",
  {
    id: t.string().primaryKey(),
    shipId: t.string(),
    placedObjectId: t.string(),
...
export function ownAuthoredFlightActuators(
  ctx: PhysicalViewContext & {
    db: { actuatorOutput: { id: Find<{ throttle: number }> } };
  },
) {
  const a = actor(ctx);
  if (
    !a ||
    !hasAcceptedAuthoredFlight(ctx, a) ||
    ctx.db.constructionFlightDirty.shipId.find(a.shipId)
  )
    return [];
  ...
  try {
    actuators = JSON.parse(c.actuatorsJson);
  } catch {
    return [];
  }
  ...
  return actuators
    .filter((a) => fittings.get(a.id)?.installed)
    .map((device) => ({
      id: device.id,
      shipId: a.shipId,
      placedObjectId: device.placedObjectId,
      x: device.x,
      y: device.y,
      nozzleX: device.nozzleX,
      nozzleY: device.nozzleY,
      height: device.height,
      exhaustX: device.exhaustX,
      exhaustY: device.exhaustY,
      throttle:
        ctx.db.actuatorOutput.id.find(a.shipId + ":" + device.id)?.throttle ??
        0,
    }));
}
