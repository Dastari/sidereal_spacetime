...
test("fixture migration retains the original container and creates only empty stable links once", () => {
  ...
  const db = {
    ...
    inventoryContainer: {
      ...
      insert: (row: typeof legacy) => {
        containers.push(row);
        return row;
      },
    },
    storageBinding: {
      id: { find: (id: string) => bindings.find((row) => row.id === id) },
      insert: (row: (typeof bindings)[number]) => bindings.push(row),
    },
    inventoryState: {
      characterId: {
        find: () => state,
        update: (row: typeof state) => {
          state = row;
        },
      },
    },
  ...
  } as unknown as Parameters<typeof seedStorage>[0];
  seedStorage(ctx, "actor");
  expect(bindings).toHaveLength(4);
  expect(containers).toHaveLength(4);
  expect(
    bindings.find(
      (row) => row.placementId === LAB_STORAGE_FIXTURES[0].placementId,
    )?.containerId,
  ).toBe("existing-crate");
  expect(containers[0]).toBe(legacy);
  expect(state.revision).toBe(13n);
  const ids = containers.map((row) => row.id);
  seedStorage(ctx, "actor");
  expect(containers.map((row) => row.id)).toEqual(ids);
  expect(state.revision).toBe(13n);
  // The empty read-only item fixture ensures seeding cannot mint, move or delete contents.
});

test("container views inherit reach and wall occlusion, including nested contents", async () => {
  const { inventoryContainersView, inventoryItemsView } =
  ...
  const ctx = {
    ...
    db: {
      ...inventoryMetadataTestTables(),
      character: { by_owner: { filter: () => [actor] } },
      inventoryContainer: {
        by_character: { filter: () => [container, child] },
      },
      inventoryItem: { by_character: { filter: () => [item] } },
      storageBinding: { by_character: { filter: () => [] } },
    },
  } as unknown as Parameters<typeof inventoryContainersView>[0];
  expect(inventoryContainersView(ctx)).toEqual([]);
  expect(inventoryItemsView(ctx)).toEqual([]);
  actor.localY = 1.1;
  expect(inventoryContainersView(ctx).map((row) => row.id)).toEqual([
    "world",
    "nested",
  ]);
  expect(inventoryItemsView(ctx)[0].id).toBe("pack");
  actor.localY = 4;
  expect(inventoryContainersView(ctx)).toEqual([]);
  actor.localY = 1.1;
...
test("bound cargo reach follows the authored floor arrangement without rewriting persisted coordinates", async () => {
  ...
  const containers = LAB_STORAGE_FIXTURES.map((f, i) => ({
    id: `crate-${i}`,
    characterId: "actor",
    shipId: "ship",
    parentItemId: "",
  ...
  const ctx = {
    ...
    db: {
      ...inventoryMetadataTestTables(),
      character: { by_owner: { filter: () => [actor] } },
      inventoryContainer: { by_character: { filter: () => containers } },
      inventoryItem: { by_character: { filter: () => [] } },
  ...
  actor.localX = -2.2;
  expect(inventoryContainersView(ctx).map((row) => row.placementId)).toEqual([
    LAB_STORAGE_FIXTURES[1].placementId,
    LAB_STORAGE_FIXTURES[3].placementId,
  ]);
  expect(
    containers.every((row) => row.localX === 100 && row.localY === 100),
  ).toBe(true);
});
