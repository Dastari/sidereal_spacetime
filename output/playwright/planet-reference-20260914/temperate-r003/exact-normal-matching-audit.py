...
out=Path(__file__).resolve().parent;raw=(out/'kit.json').read_bytes();kit=json.loads(raw);v=next(x for x in kit['variants']if x['name']=='continent-a');expected=[]
...
g,b=read_glb(out/'continent-a.glb');actual,_=glb_corners(g,b);bins=collections.defaultdict(list)
...
report={'method':'Maximum bipartite matching, preserves corner multiplicity/material boundaries; original strict tolerances unchanged','kitSha256':hashlib.sha256(raw).hexdigest(),'variant':'continent-a','expectedCorners':len(expected),'actualCorners':len(actual),'matchedCorners':len(matched),'unmatchedExpected':missing,'unmatchedActual':len(actual)-len(matched),'positionTolerance':1e-5,'normalTolerance':1e-4,'uvTolerance':1e-5,'maximumNormalDelta':max(math.dist(expected[i][2],actual[j][2])for j,i in matched.items()),'kitUnchanged':raw==(out/'kit.json').read_bytes()}
(out/'exact-normal-matching-audit.json').write_text(json.dumps(report,indent=2));print(report);assert not missing and len(matched)==len(actual)
