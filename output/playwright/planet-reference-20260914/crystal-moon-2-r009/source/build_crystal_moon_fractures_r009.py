"""Native r009: replace selected panel districts with connected bent fracture runs."""
...
old=BASE/f'crystal-moon-{moon}-r005';out=BASE/f'crystal-moon-{moon}-r009';assert not(out/'kit.blend').exists();out.mkdir(exist_ok=True)
...
for panel in panels:
 ...
 for index,region in enumerate(regions):
  ...
  if center.normalized().dot(anchor)>.05 and min(values)<region['width']+.20 and max(values)>-region['width']-.20:
...
for panel,region_id,points,center,normal in selected:
 ...
 cuts=[-10,-.92,-.75,-.55,-.38,-.23,-.07,.11,.25,.45,.58,.78,10]
 sections=[(0,-10,-1.15),(12,-1.15,-1.04),(13,-1.04,-.92)]+[(i,cuts[i],cuts[i+1])for i in range(1,11)]+[(11,.78,.875),(14,.875,.965),(15,.965,1.055),(16,1.055,1.15),(17,1.15,10)]
 for section,low,high in sections:
  shift=[-.045,.065,-.025,.040,-.068,.018,.072,-.034,.051,-.015,.061,-.040,-.045,-.045,-.040,-.040,-.040,-.040][section]
  for band,(lower,upper,height)in enumerate([(-10,-region['width'],[-.045,.035,-.010,.078,.008,-.032,.052,-.012,.089,-.020,.055,.005,.065,-.025,.082,-.020,.060,-.015][section]),(-region['width'],region['width'],[-.110,-.060,-.125,-.075][section%4]),(region['width'],10,[.065,-.020,.085,.025,-.040,.070,-.005,.055,-.025,.081,.015,-.030,-.015,.070,.070,-.035,.055,-.030][section])]):
   topmesh=bmesh.new()
   ...
   for contour in contours:
    ...
    while changed and len(contour)>3:
     ...
     for i,p in enumerate(contour):
      a=p-contour[i-1];b=contour[(i+1)%len(contour)]-p
      if a.cross(b).length<1e-7 and a.dot(b)>=0:contour.pop(i);changed=True;break
   ...
   bright=band==2 and section in [3,6,8,14] and region_id in [0,1] and center.z>-.45
   top_role=5 if band==1 else(6 if bright else(4 if (panel['panel']+section)%9==0 else 3))
   ...
   for face in mesh.polygons:
    coords=[mesh.vertices[i].co for i in face.vertices];side_x=max([coords[(i+1)%len(coords)]-p for i,p in enumerate(coords)],key=lambda q:q.length).normalized();side_y=face.normal.cross(side_x).normalized()
    for loop in face.loop_indices:
     p=mesh.vertices[mesh.loops[loop].vertex_index].co;uv.data[loop].uv=(p.dot(u)*1.4+.5,p.dot(v)*1.4+.5)if abs(face.normal.dot(normal))>.9 else(p.dot(side_x)*2,p.dot(side_y)*2)
   ...
   records.append({'partId':part,'region':region_id,'sourcePanel':panel['panel'],'section':section,'band':band,'height':height,'runAxis':list(run),'authoredNormal':list(normal),'runLimits':[low,high],'firstTriangle':first,'triangleCount':len(mesh.loop_triangles),'topMaterial':top_role})
...
gltf['nodes'][0]['extras']['authoring'] = 'native-crystal-moon-r009-selected-fractures'
...
print('R009_DONE',moon,'districts',len(selected),'parts',len(records),'triangles',len(roles))
