"""Native glacial shafts under thick irregular snow. Isolated r025 only.
Run: blender -b --python scripts/art_library/build_ice_native_kit_r025.py -- NEW_OUTPUT
"""
...
def mesh_object(name,verts,faces,roles,smooth=None):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=1,use_grid_fill=True);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-planets--ice-moon-1-r002';return o
...
region=shaft('snow-cut-region',0,1.07)
# Smaller broken cavity; preserve its depth while freeing room for unequal banks.
for vertex in region.data.vertices:
 vertex.co.x*=.68;vertex.co.y*=.68
 if vertex.co.z>-.05:vertex.co.z-=.035
...
def snow_shelf(name,outline,z,thickness):
 ...
 for j in range(2):
  for i in range(n):faces.append((j*n+i,j*n+(i+1)%n,(j+1)*n+(i+1)%n,(j+1)*n+i));roles.append((2 if i%3 else 3)if j==0 else 0)
 faces.append(tuple(range(2*n,3*n)));roles.append(0)
 return mesh_object(name,verts,faces,roles)
parts=[region,
 buttress('hero-blue-column',.77,-.43,.16,.58,.15,5),
...
# Coherent intermediate banks, with thick blue walls under broken white crust.
for i,(outline,z,depth)in enumerate([
 ([(-1.43,-.71),(-1.14,-.86),(-.94,-.71),(-.81,-.51),(-.90,-.23),(-1.19,-.18),(-1.48,-.31)],.27,.34),
 ([(-1.36,.02),(-1.02,-.06),(-.85,.12),(-.97,.31),(-.84,.52),(-1.15,.66),(-1.46,.45)],.40,.44),
 ([(-.78,.66),(-.54,.57),(-.28,.64),(-.23,.96),(-.49,1.17),(-.86,1.05),(-.95,.86)],.28,.31),
 ([(.03,.59),(.33,.44),(.62,.54),(.71,.88),(.44,1.09),(.08,.94),(-.02,.77)],.38,.43),
 ([(-.71,-1.09),(-.42,-1.19),(-.15,-.99),(-.17,-.73),(-.36,-.59),(-.64,-.72)],.34,.40),
 ([(.10,-.86),(.30,-.99),(.53,-.84),(.61,-.63),(.42,-.43),(.15,-.52)],.21,.29)
 ]):parts.append(snow_shelf('intermediate-stepped-bank-'+str(i),outline,z,depth))
for group,(x,y)in enumerate([(-.97,.27),(-.55,.72),(.34,.63),(-.36,-.80)]):
 for j,(dx,dy,r,h)in enumerate([(-.10,.02,.11,.43),(.09,.05,.10,.55),(.03,-.12,.105,.31)]):parts.append(buttress('bank-attached-blue-cluster-'+str(group)+'-'+str(j),x+dx,y+dy,r,h,.27*j+group*.18,4+j%3))
...
# Replace the two broad snowplain caps with unequal native districts.
# Low blue foundations retain genuine solid terrain through the opened gaps.
gorgeparts=[gorge_solid('gorge-deep-open-floor',lower+list(reversed(upper)),-.40,4),
 gorge_solid('gorge-north-recessed-ice-bed',upper+[(1.88,.69),(1.27,.92),(.72,.77),(.18,1.02),(-.36,.83),(-.89,1.03),(-1.52,.82),(-1.90,.63)],-.24,2),
 gorge_solid('gorge-south-recessed-ice-bed',list(reversed(lower))+[(-1.88,-.73),(-1.24,-.98),(-.78,-.76),(-.21,-1.03),(.43,-.89),(1.03,-1.02),(1.58,-.72),(1.87,-.61)],-.27,2)]
districts=[
 ([(-1.84,.32),(-1.58,.19),(-1.31,.25),(-1.22,.48),(-1.40,.76),(-1.74,.67),(-1.90,.52)],.28),
 ([(-1.13,.31),(-.96,.20),(-.68,.21),(-.47,.39),(-.53,.67),(-.78,.84),(-1.07,.71)],.12),
 ([(-.32,.46),(-.10,.35),(.18,.40),(.39,.55),(.25,.88),(-.03,.82),(-.35,.68)],.34),
 ([(.62,.24),(.94,.29),(1.11,.39),(1.46,.43),(1.63,.64),(1.35,.77),(1.10,.69),(.87,.79),(.64,.63)],.19),
 ([(-1.84,-.47),(-1.56,-.35),(-1.31,-.47),(-1.25,-.65),(-1.43,-.84),(-1.73,-.76)],.16),
 ([(-1.03,-.48),(-.78,-.38),(-.50,-.42),(-.36,-.62),(-.50,-.88),(-.84,-.80),(-1.10,-.68)],.35),
 ([(-.16,-.39),(.16,-.27),(.41,-.39),(.56,-.58),(.32,-.78),(.02,-.83),(-.21,-.61)],.11),
 ([(.77,-.31),(1.04,-.23),(1.31,-.36),(1.62,-.35),(1.79,-.53),(1.55,-.69),(1.25,-.77),(.91,-.65)],.27)]
for i,(outline,height) in enumerate(districts):
 gorgeparts.append(gorge_solid('gorge-unequal-snow-district-'+str(i),outline,height,0))
 # A smaller shoulder shares the bank volume and exposes an unequal step.
 if i in (1,3,5):
  cx=sum(x for x,y in outline)/len(outline);cy=sum(y for x,y in outline)/len(outline);shoulder=[(cx+(x-cx)*.55,cy+(y-cy)*.57)for x,y in outline]
  gorgeparts.append(snow_shelf('gorge-district-thick-shoulder-'+str(i),shoulder,height+.11,.15))
for group,(x,y)in enumerate([(-1.16,.61),(.49,.63),(-.31,-.72),(.66,-.54)]):
 for j,(dx,dy,r,h)in enumerate([(-.08,.02,.105,.25),(.10,-.035,.12,.39),(.04,.12,.085,.16)]):gorgeparts.append(buttress('gorge-exposed-gap-cluster-'+str(group)+'-'+str(j),x+dx,y+dy,r,h,.32*j+.15*group,4+j%3))
...
# Selected gorge walls become compact blue clusters, leaving the center open.
for side,xs in [(1,[-1.30,-.58,.44]),(-1,[-.91,.81,1.38])]:
 for group,x in enumerate(xs):
  y=(.18*math.sin(x*2.5)) + side*.27
  for j,(dx,dy,r,h)in enumerate([(-.10,0,.13,.24),(.09,.035,.11,.36),(.015,-.035,.095,.13)]):gorgeparts.append(buttress('gorge-cluster-'+str(side)+'-'+str(group)+'-'+str(j),x+dx,y+side*dy,r,h,.31*j,4+j%3))
...
for fi,(region_name,region_parts)in enumerate(forms):
 ...
 for k in range(180):
  ...
  obj['role']='planet';obj['authoring']='native-planets--ice-moon-1-r002';region_parts.append(obj)

# Native ground relief addresses actual picked exposed substrate, not only borders.
# Closed compact unequal stepped districts, authored once and retained at allLOD.
ground_districts=[]
for i in range(72):
 z=1-2*(i+.5)/72;theta=i*2.399963229728653+.37;r=math.sqrt(1-z*z);n=Vector((math.cos(theta)*r,math.sin(theta)*r,z));east=n.cross(Vector((0,0,1)) if abs(z)<.94 else Vector((0,1,0))).normalized();north=n.cross(east)
 width=[.25,.20,.28,.23,.26][i%5];depth=[.24,.28,.21,.26][i%4];height=[.052,.076,.037,.063][i%4]
 outline=[(-.92*width,-.70*depth),(-.19*width,-.98*depth),(.59*width,-.79*depth),(.91*width,-.18*depth),(.62*width,.26*depth),(.78*width,.72*depth),(.10*width,.96*depth),(-.66*width,.58*depth),(-.88*width,.11*depth)]
 # Unequal broken footprint variants, not identical disks over a white base.
 if i%4==1:outline=[(x*(.88 if j in (0,1,6)else 1),y*(.76 if j in(2,3,4)else 1))for j,(x,y)in enumerate(outline)]
 if i%4==2:outline.insert(4,(.27*width,.13*depth))
 if i%4==3:outline=[(x+.11*y,y-.08*x)for x,y in outline]
 objects=[snow_shelf('ground-stepped-district-'+str(i),outline,height,height+.055)]
 if i%3!=1:objects.append(snow_shelf('ground-thick-shoulder-'+str(i),[(x*.48-.025,y*.53+.01)for x,y in outline],height+.031,.065))
 if i%3==1:
  for j,(cx,cy,r,h)in enumerate([(width*.56,-depth*.12,.046,height+.052),(width*.39,-depth*.31,.035,height+.023)]):
   column=[(cx+math.cos(a*math.tau/5+.2*j)*r,cy+math.sin(a*math.tau/5+.2*j)*r*.83)for a in range(5)]
   objects.append(snow_shelf('ground-grouped-blue-shaft-'+str(i)+'-'+str(j),column,h,h+.06))
 for obj in objects:
  ...
  bm.to_mesh(obj.data);bm.free()
  # The editable local shelf is embedded into the native authored sphere.
  for vertex in obj.data.vertices:
   x,y,h=vertex.co;direction=(n+east*x+north*y).normalized();vertex.co=direction*(1+h)
  bm=bmesh.new();bm.from_mesh(obj.data);bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(obj.data);bm.free();obj.data.update();obj.data.normals_split_custom_set([tuple(n.vector)for n in obj.data.corner_normals]);obj['groundPlacementId']='native-ground-district-'+str(i)
  ground_districts.append(obj)
for name,subdivisions in[('ground-sphere',6),('ground-sphere-medium',5),('ground-sphere-low',4)]:
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions,radius=.982);o=bpy.context.object;o.name='GEO-'+name;o.data.materials.append(materials[2]);o['role']='planet';o['authoring']='native-planets--ice-moon-1-r002'
 for p in o.data.polygons:p.use_smooth=True
 forms.append((name,[o,*ground_districts]))
kit={'schema':'sidereal.native-planet-kit.v1','layout':'ice-moon-glacial','coveredReferenceIds':['planets--ice-moon-1'],'compositionRecipe':{'referenceId': 'planets--ice-moon-1', 'layoutSeed': 1297, 'scales': [0.34, 0.29, 0.35, 0.3, 0.33, 0.28, 0.34, 0.31, 0.33, 0.3], 'variantOffset': 1},'uvConvention':'glTF corner UV, Blender V flipped; invertY=false','materials':definitions,'variants':[]}
# Native powder and facet optical finish remains ordinary PBR.
for role in range(5):
 image=bpy.data.images.new('Ice25-albedo-'+str(role),width=256,height=256,alpha=True);orm=bpy.data.images.new('Ice25-roughness-'+str(role),width=256,height=256,alpha=True);orm.colorspace_settings.name='Non-Color';pixels=[];rp=[]
 for y in range(256):
  for x in range(256):
   u=x/256;v=y/256
...
validation['sha256']={p.name:hashlib.sha256(p.read_bytes()).hexdigest()for p in out.iterdir()if p.is_file()};(out/'validation.json').write_text(json.dumps(validation,indent=2));print('ICE_R025_DONE',validation['variants'])
