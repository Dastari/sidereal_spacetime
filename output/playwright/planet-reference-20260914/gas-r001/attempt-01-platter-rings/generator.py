...
def gas_color(u,v):
 ...
 lat=v+.012*math.sin(u*math.tau*3+v*19)+.006*math.sin(u*math.tau*9-v*23)
 ...
 detail=.86+.14*math.sin(lat*178+math.sin(u*math.tau*5)*2.2)
 base=tuple(c*detail for c in base)
 for cx,cy,rx,ry,strength in [(.62,.44,.145,.075,2.8),(.24,.68,.08,.042,2.4)]:
  dx=((u-cx+.5)%1-.5)/rx;dy=(v-cy)/ry;r=math.hypot(dx,dy)
  if r<1:
   a=math.atan2(dy,dx);rings=.5+.5*math.sin(r*31-a*1.6+r*r*5)
...
def material(name,color,roughness,alpha=1,texture=None):
 ...
 if texture:
  tex=m.node_tree.nodes.new('ShaderNodeTexImage');tex.image=image;m.node_tree.links.new(tex.outputs['Color'],p.inputs['Base Color']);d['baseColorTexture']='gas-albedo.png';d['textureColorSpace']='sRGB';d['invertY']=False
 materials.append(m);definitions.append(d);return m
bodymat=material('gas-authored-PBR',(1,1,1),.66,texture=True)
material('ring-inner-dust',(.30,.07,.38),.90,.47)
material('ring-main-lavender',(.42,.18,.52),.86,.65)
material('ring-outer-dust',(.26,.07,.36),.93,.30)
...
for index,(inner,outer,role) in enumerate([(1.19,1.34,1),(1.40,1.65,2),(1.72,1.86,3)]):
 ...
 for j in range(segments):
  ...
  if index==2 and j%59 in range(4,13):continue
  faces.append((j*2,((j+1)%segments)*2,((j+1)%segments)*2+1,j*2+1))
 mesh=bpy.data.meshes.new('Ring-dust-'+str(index));mesh.from_pydata(verts,[],faces);mesh.materials.append(materials[role]);obj=bpy.data.objects.new('GEO-ring-dust-'+str(index),mesh);scene.collection.objects.link(obj);forms.append(('ring-dust-'+str(index),[obj]))
