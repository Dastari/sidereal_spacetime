...
export function composeIceMoonReference(kit:NativePlanetKit,seed:number,_lod:0|1|2,diagnostic:boolean|'gorge'=false){
 ...
 if(!recipe||!['planets--ice-moon-1','planets--ice-moon-2'].includes(recipe.referenceId)||![6,8].includes(recipe.scales.length))throw new Error('Explicit glacial moon recipe required');
