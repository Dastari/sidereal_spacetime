...
const ids=['temperate-moon-1','temperate-moon-2','ocean-moon-1','ocean-moon-2'];
for(const id of ids){
...
it('keeps the large crater floor exposed in all48 seeded regional placements',()=>{
 const engine=new NullEngine(),scene=new Scene(engine);scene.useRightHandedSystem=true;
 try{for(const id of ids){const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r001/kit.json`,'utf8'));const meshes=composeHybridMoonReference(kit,38,0).map((b,i)=>{const mesh=new Mesh(`${id}-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial('moon-role',scene);const data=new VertexData();Object.assign(data,b);data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
...
it('preserves native foundation geometry and cavity pigment while adding only upper regional cover',()=>{
 ...
 for(const id of ids){const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r001/kit.json`,'utf8')) as NativePlanetKit;
