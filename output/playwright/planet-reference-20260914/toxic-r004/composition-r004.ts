...
export function composeToxicReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 if(diagnostic)regions.push({n:unit([.452,.388,.794]),scale:.28,angle:0,name:'toxic-basin-group-a',id:'region-diagnostic'});
  ...
  regions.push({n:[Math.cos(angle)*r,y,Math.sin(angle)*r],scale:[.36,.30,.40,.33][i%4],angle:random()*Math.PI*2,name:['toxic-basin-group-a','toxic-basin-group-b','toxic-basin-group-c'][i%3],id:`region-${i}`});
 }
 ...
 const substrate=(v:Vec):Vec=>{
  ...
  for(const region of regions){
   ...
   if(Number.isFinite(z))depression=Math.max(depression,.012+region.scale*(.07-z));
