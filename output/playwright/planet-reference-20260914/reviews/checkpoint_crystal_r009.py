...
def discover(family,spec):
 ...
 extras=[str(p.relative_to(ROOT))for p in sorted((ROOT/'scripts/art_library').glob('*crystal*'))if p.is_file() and 'moon' in p.name and any(r in p.name for r in ['r006','r007','r008','r009'])]
 for p in sorted(OUT.iterdir()):
  if str(p.relative_to(ROOT)) in {s['source']for s in sources}:continue
  if any(p.name.startswith(a)for a in aliases) and ('moon' in p.name) and any(r in p.name for r in ['r006','r007','r008','r009']) and (p.is_file()or any(k in p.name for k in ['audit','parity','composition'])):extras.append(str(p.relative_to(ROOT)))
 reports=[str(p.relative_to(ROOT))for p in sorted((OUT/'reviews').glob('*.md'))if 'crystal-moon' in p.name and any(r in p.name for r in ['r006','r007','r008','r009'])]
...
def main():
 ...
 proposal={'schema':'sidereal.crystal-r009-checkpoint-proposal.v1','prepared_at':now(),'status':'PREPARATION ONLY; no canonical writes until explicit root verdict','families':[discover(f,s)for f,s in SPECS.items()]}
 if not args.execute:
  (OUT/'reviews/crystal-r009-checkpoint-proposal.json').write_text(json.dumps(proposal,indent=2)+'\n');print('Prepared proposal only; no canonical files touched');return
 ...
 with locked():
  ...
  for family in proposal['families']:
   ...
   d['revisions'].append({'revision':n,'created_at':now(),'stage':'in-progress','change':f'Preserve Crystal moon native r006–r009, failed attempts, terminal-interval evidence and exact actual review verdict; main-world candidate remains pinned to r{pin:03}.','hypothesis':'Exact sources, PBR exports, captures and dated review verdicts preserve per-reference identity and remaining acceptance boundaries.','covered_reference_ids':refs,'evidence':evidence,'review':None})
   d['current_revision']=n;d['state']='in-progress';d['assigned_to']='root/planet-reference-20260914'
   d['feedback'].append({'revision':n,'author':'agent','text':f'Late moon preservation checkpoint only. Main-world pin remains r{pin:03}; prior approvals/mappings are exact. Attached review verdict states current visual gates; no owner sign-off or hardware transition acceptance inferred.','message_reference':'output/playwright/planet-reference-20260914/reviews/crystal-r009-checkpoint-proposal.json','recorded_at':now(),'resolved_by_revision':None})
  ...
  (OUT/'reviews/crystal-r009-checkpoint-result.json').write_text(json.dumps(result,indent=2)+'\n')
