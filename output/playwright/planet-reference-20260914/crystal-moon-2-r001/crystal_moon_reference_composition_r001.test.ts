...
import {composeCrystalMoonReference} from './crystal_moon_reference_composition_r001';
...
it('retains complete native regional surfaces UVs normals and identities at every LOD',()=>{
 ...
 expect(a.reduce((n,b)=>n+b.indices.length/3,0)).toBeLessThan(172000);
});
...
it('keeps the large crater floor exposed in all18 seeded regional placements',()=>{
 const engine=new NullEngine(),scene=new Scene(engine);scene.useRightHandedSystem=true;
 try{for(const id of ids){const kit=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r001/kit.json`,'utf8'));const meshes=composeCrystalMoonReference(kit,38,0).map((b,i)=>{const mesh=new Mesh(`${id}-${i}`,scene);mesh.metadata={role:'planet',materialRole:i,triangleRanges:b.ranges};mesh.material=new PBRMaterial('moon-role',scene);const data=new VertexData();Object.assign(data,b);data.applyToMesh(mesh);mesh.computeWorldMatrix(true);return mesh;});
