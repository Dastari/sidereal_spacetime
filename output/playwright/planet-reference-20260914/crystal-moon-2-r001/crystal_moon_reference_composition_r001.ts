...
export function composeCrystalMoonReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 if(!recipe||!['planets--crystal-moon-1','planets--crystal-moon-2'].includes(recipe.referenceId)||![8,10].includes(recipe.scales.length))throw new Error('Explicit crystalline moon recipe required');
