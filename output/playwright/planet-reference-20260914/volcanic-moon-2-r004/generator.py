...
palette=[('planets--volcanic-moon-2-charcoal-crust', (0.078, 0.047, 0.095), 0.91), ('planets--volcanic-moon-2-purple-rim', (0.18, 0.11, 0.19), 0.87), ('planets--volcanic-moon-2-dark-cavity', (0.006, 0.003, 0.01), 0.97), ('planets--volcanic-moon-2-cooled-fracture', (0.025, 0.013, 0.037), 0.92), ('planets--volcanic-moon-2-molten-fault', (1.0, 0.095, 0.002), 0.57), ('planets--volcanic-moon-2-pale-purple-fracture', (0.25, 0.13, 0.24), 0.86)]
...
def mesh_object(name,verts,faces,roles):
 ...
 obj['role']='planet';obj['authoring']='native-planets--volcanic-moon-2-r003'
 return obj
...
# Preserve the exact normal B source; only two recipe placements use its
...
for name,parts in forms:
 ...
 if name.endswith('-b') or name.endswith('-b-edge'):paths=paths[:1]
 ...
 for pi,(points,width) in enumerate(paths):
  ...
  for j,(x,y) in enumerate(points):
   ...
   # Only selected junctions broaden; other fault tips stay narrow and cold districts survive.
   ...
   if True:profile=[.70,1.45,5.5,3.6,1.15,.62] if name.endswith('-a') else [.65,1.05,4.2,2.65,.9,.62]
   if name.endswith('-b-edge'):profile=[.8,2.5,9.2,7.2,2.4,.72]
   ...
   a,b=sides
   verts.extend([(a[0],a[1],a[2]-.014),(b[0],b[1],b[2]-.014),a,b])
...
kit={'schema' :'sidereal.native-planet-kit.v1','layout':'volcanic-moon-craters','coveredReferenceIds':['planets--volcanic-moon-2'],'compositionRecipe':{'referenceId': 'planets--volcanic-moon-2', 'layoutSeed': 1637, 'scales': [0.38, 0.4, 0.39, 0.41, 0.37, 0.4, 0.38, 0.42, 0.39, 0.37, 0.41, 0.38], 'variantOffset': 1},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
kit['compositionRecipe']['warmEdgeRegions']=[2,9]
kit['materials'][4].update(emissiveColor=[1,.095,.002],emissiveStrength=2.2)
...
validation={'publication':'isolated volcanic moon2 r004 two hot-edge candidate; visual review pending','coordinates':'Blender Z up, formation roots buried below Z0; ground sphere radius1','partsManifold':True,'lodContract':'Dominant crater variants retained unchanged at all levels; ground-sphere high/medium/low are authored originals; small outcrop density may decrease; no decimation.','variants':[]}
for name,parts in forms:
