...
def mesh_object(name,verts,faces,roles,smooth=None):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bmesh.ops.triangulate(bm,faces=list(bm.faces));bmesh.ops.subdivide_edges(bm,edges=list(bm.edges),cuts=1,use_grid_fill=True);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-ice-r026';return o
...
for fi,(region_name,region_parts)in enumerate(forms):
 ...
 for k in range(180):
  ...
  obj['role']='planet';obj['authoring']='native-ice-r026';region_parts.append(obj)
...
ground_districts=[]
for i in range(42):
 z=1-2*(i+.5)/42;theta=i*2.399963229728653+.37;r=math.sqrt(1-z*z);n=Vector((math.cos(theta)*r,math.sin(theta)*r,z));east=n.cross(Vector((0,0,1)) if abs(z)<.94 else Vector((0,1,0))).normalized();north=n.cross(east)
...
for name,subdivisions in[('ground-sphere',5),('ground-sphere-medium',4),('ground-sphere-low',3)]:
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=subdivisions,radius=.982);o=bpy.context.object;o.name='GEO-'+name;o.data.materials.append(materials[2]);o['role']='planet';o['authoring']='native-ice-r026'
...
kit['opticalPolicy']={'scope':'Selected named tall shard vertical sides only; opaque snow caps and surrounding geology','transmissiveRoleIds':[5,6],'opaqueRoleIds':[0,1,2,3,4],'selectedNativeObjects':optical_selected,'selectedNativeFaceCounts':optical_counts}
