...
import{composeCrystalMoonReference}from'./crystal_moon_reference_composition_r006';
...
for(const id of ['crystal-moon-1','crystal-moon-2']){
 const path=`output/playwright/planet-reference-20260914/${id}-r006/`,kit=JSON.parse(readFileSync(path+'kit.json','utf8'));
 it(`${id} retains exact native PBR and all attribute channels across LOD`,()=>{
  const prior=JSON.parse(readFileSync(path.replace('r006','r005')+'kit.json','utf8'));expect(kit.materials).toEqual(prior.materials);expect(kit.variants.map((v:any)=>v.name)).toEqual(['connected-crystalline-body']);
...
it('r006 contains selective native lip notches while preserving broad quiet facets',()=>{for(const id of ['crystal-moon-1','crystal-moon-2']){const p=JSON.parse(readFileSync(`output/playwright/planet-reference-20260914/${id}-r006/native-panel-plan.json`,'utf8'));const chipped=p.filter((v:any)=>v.chippedEdges.length>0);expect(chipped.length).toBeGreaterThan(3);expect(chipped.length).toBeLessThan(p.length/2);}});

for(const id of ['crystal-moon-1','crystal-moon-2'])it(`${id} preserves exact parent surfaces and identifies new native parts at every LOD`,()=>{
 const path=`output/playwright/planet-reference-20260914/${id}-r006/`,kit=JSON.parse(readFileSync(path+'kit.json','utf8')),prior=JSON.parse(readFileSync(path.replace('r006','r005')+'kit.json','utf8')),a=prior.variants[0],b=kit.variants[0];
 for(const channel of ['positions','normals','uvs','indices','triangleMaterials'])expect(b[channel].slice(0,a[channel].length)).toEqual(a[channel]);
 const parts=kit.nativePartRanges;expect(parts[0].triangleCount).toBe(a.indices.length/3);expect(parts.reduce((n:number,p:any)=>n+p.triangleCount,0)).toBe(b.indices.length/3);expect(new Set(parts.map((p:any)=>p.partId)).size).toBe(parts.length);
 for(const seed of [38,117,904]){const levels=[0,1,2].map(l=>composeCrystalMoonReference(kit,seed,l as 0|1|2));for(const level of levels.slice(1))expect(level).toEqual(levels[0]);const seen=new Set(levels[0].flatMap(b=>b.ranges.map(r=>r.partId)));expect(seen).toEqual(new Set(parts.map((p:any)=>p.partId)));}
});
