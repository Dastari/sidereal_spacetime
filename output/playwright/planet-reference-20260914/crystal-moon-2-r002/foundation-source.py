...
def make(name,verts,faces,roles):
 ...
 bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=bm.faces);bm.to_mesh(mesh);bm.free();o['role']='planet';o['authoring']='native-planets--crystal-moon-2-r001';return o
...
for source_name in ['battered-region-a','battered-region-b']:
 ...
 for obj in target.objects:
  ...
  obj.name='GEO-crystal-'+obj.name;obj['role']='planet';obj['authoring']='native-planets--crystal-moon-2-r001';parts.append(obj)
...
kit={'schema':'sidereal.native-planet-kit.v1','layout':'crystal-moon-geology','coveredReferenceIds':['planets--crystal-moon-2'],'compositionRecipe':{'referenceId': 'planets--crystal-moon-2', 'layoutSeed': 2087, 'scales': [0.36, 0.32, 0.38, 0.33, 0.37, 0.31, 0.36, 0.34], 'variantOffset': 1, 'hero': 0.27},'materials':[{'name':name,'linearColor':list(color),'roughness':roughness} for name,color,roughness in palette],'variants':[]}
