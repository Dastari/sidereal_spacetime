...
old=Path('output/playwright/planet-reference-20260914')/(rid.removeprefix('planets--')+'-r002')
...
bm=bmesh.new();bmesh.ops.create_icosphere(bm,subdivisions=2 if second else 3,radius=1)
bmesh.ops.join_triangles(bm,faces=list(bm.faces),angle_face_threshold=math.pi,angle_shape_threshold=math.pi)
for v in bm.verts:
 ...
 else:v.co=n*(.98+.025*math.sin(theta*3+n.z*6))
...
for i,p in enumerate(bm.faces):
 ...
 gap=.965 if not second else .975;rise=(.012+r.random()*.026)if not second else(.008+r.random()*.020)
 low=[c+(v-c)*1.012-normal*.072 for v in points]
 ...
 face(cap,top_role)
 panel_records.append({'panel':i,'luminousEdgeRoute':luminous,'topRole':top_role,'height':rise,'normal':list(normal),'center':list(c)})
bm.free()
...
obj['role']='planet';obj['partId']=rid+'/native-body';obj['authoring']='native-crystal-moon-r003';obj['morphology']='angular-connected-mass'if second else'round-fractured-crystal-plates'
