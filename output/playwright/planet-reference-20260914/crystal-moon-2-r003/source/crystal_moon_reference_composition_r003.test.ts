...
for(const id of ['crystal-moon-1','crystal-moon-2']){
 const path=`output/playwright/planet-reference-20260914/${id}-r003/`,kit=JSON.parse(readFileSync(path+'kit.json','utf8'));
 it(`${id} retains exact native PBR and all attribute channels across LOD`,()=>{
  const prior=JSON.parse(readFileSync(path.replace('r003','r002')+'kit.json','utf8'));expect(kit.materials).toEqual(prior.materials);expect(kit.variants.map((v:any)=>v.name)).toEqual(['connected-crystalline-body']);
