...
export function composeCrystalReference(kit:NativePlanetKit,seed:number,lod:0|1|2,diagnostic=false){
 ...
 // Cover the largest angular gaps in the FINAL visible emitter population.
 // Existing forty authored clusters remain unchanged; buried tips cannot falsely
 // satisfy coverage. A dense global candidate lattice has no camera dependency.
 if(!diagnostic){
 const surface=nativeRadialSurface(batches,[0,1,2,8]);
 const covered=existingEmitterTips.filter(p=>Math.hypot(...p)>surface(unit(p))+.005).map(unit);
 const candidates=Array.from({length:1536},(_,i)=>{const y=1-2*(i+.5)/1536,theta=phase+i*2.399963229728653,r=Math.sqrt(1-y*y);return{i,n:[Math.cos(theta)*r,y,Math.sin(theta)*r]as Vec};}).filter(v=>!regions.some(region=>Math.hypot(...v.n.map((x,k)=>x-region.n[k]))<region.scale*.75));
 const selected:number[]=[];
 for(let bridge=0;bridge<24;bridge++){
  let best:typeof candidates[number]|undefined,bestScore=-1;
  for(const candidate of candidates){if(selected.includes(candidate.i))continue;const score=Math.min(...covered.map(p=>1-(candidate.n[0]*p[0]+candidate.n[1]*p[1]+candidate.n[2]*p[2])));if(score>bestScore){bestScore=score;best=candidate;}}
  if(!best)throw new Error('Native bridge coverage candidates exhausted');
  const {i,n}=best,foot=surface(n);if(!Number.isFinite(foot))throw new Error('Bridge crystal has no final native terrain support');
  selected.push(i);covered.push(n);
  const east=unit(cross(Math.abs(n[1])>.94?[1,0,0]:[0,1,0],n)),north=cross(n,east),rotation=((Math.imul(seed+i+31013,1664525)>>>0)/4294967296)*Math.PI*2,c=Math.cos(rotation),s=Math.sin(rotation),bridgeScale=.19+(i%3)*.018;
  emit('medium-cluster',`bridge-crystals-${bridge}`,v=>{const x=(v[0]*c-v[1]*s)*bridgeScale,z=(v[0]*s+v[1]*c)*bridgeScale,h=foot+v[2]*bridgeScale;return n.map((q,k)=>q*h+east[k]*x+north[k]*z)as Vec;});
 }
 }
