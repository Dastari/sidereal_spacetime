import { StellarConvection } from "./stellar-convection";
  ...
  try {
    ...
    const loading = (async () => {
      ...
      if (bytes.byteLength > 8 * 1024 * 1024)
        throw new Error("Star asset exceeds byte budget");
      if (bytesToHex(sha256(bytes)) !== REVIEWED_YELLOW_STAR.sha256)
    ...
    for (const material of container.materials) {
      if (
        ...
        material.metadata?.gltf?.extras?.stellarSurface === true
      )
        convection.push(new StellarConvection(material, true));
    }
    let suppressedNativeEffects = 0;
    let activeFlares = 0;
    for (const node of [...container.transformNodes, ...container.meshes]) {
      ...
      const legacyEffect =
        Number.isFinite(extra.flarePhase) ||
        (typeof extra.partId === "string" &&
          extra.partId.startsWith("ejecta-"));
      // Preserve the native source and placement identity, but retire the
      // persistent whisker/cube VFX in favour of rare corona-rooted eruptions.
      if (legacyEffect) {
      ...
      node.metadata = {
        ...node.metadata,
        role: legacyEffect ? "effect" : "environment",
        stellarLegacyEffect: legacyEffect,
        bodyId: options.bodyId,
        celestialKind: "star",
      ...
      if (node instanceof Mesh && node.getTotalIndices()) {
        ...
        if (extra.stellarSurface === "hexagonal-convection-v1") {
          // Vertex displacement reaches at most .047 native radii. Expand
          // CPU culling bounds once; the GPU never writes geometry back.
          const bounds = node.getBoundingInfo(),
            padding = new Vector3(0.05, 0.05, 0.05);
          bounds.reConstruct(
            bounds.boundingBox.minimum.subtract(padding),
            bounds.boundingBox.maximum.add(padding),
          );
        }
        if (node.material && !legacyEffect)
    ...
    return {
      ...
      update(time: number) {
        if (!disposed) {
          ...
          activeFlares = stellarEruptionState(time).strength > 0 ? 1 : 0;
        }
      },
      dispose,
    };
