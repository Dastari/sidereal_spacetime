...
export function createStellarCorona(scene: Scene, bodyId: string) {
  const mesh = CreatePlane(`stellar-corona:${bodyId}`, { size: 4.3 }, scene);
...
void main(){
 vec2 p=(vUV-.5)*4.3;float r=length(p);float a=atan(p.y,p.x);
 ...
 float reach=1.20+.62*warp+.24*flow;
 float base=smoothstep(.90,.985,r);
 float outer=base*(1.-smoothstep(1.02,reach,r));
 float hot=base*exp(-pow((r-.995)/(.035+.026*flow),2.));
 float wisp=base*exp(-max(r-1.015,0.)*(4.5-1.5*warp))*(.25+.75*flow);
 float fade=1.-smoothstep(1.85,2.1,r);
 ...
 float height=.35+.70*smoothstep(0.,.72,age);
 float h=(r-.97)/height;
 float bend=.42*sin(clamp(h,0.,1.)*3.8-age*1.6)*smoothstep(0.,.45,age)*smoothstep(0.,.35,h);
 float curtain=turbulence(vec2(da*15.+time*.34,h*6.-time*1.7)+warp*2.);
 float width=(.23+.10*curtain)*(1.-.65*smoothstep(.25,1.,h));
 float spread=exp(-pow((da-bend)/width,2.));
 float radial=smoothstep(.93,1.015,r)*(1.-smoothstep(.80+.15*curtain,1.08,h));
 float ragged=smoothstep(.15,.73,curtain);
 float strands=.5+.5*sin(h*24.+curtain*7.-time*2.);
 float veil=.12+.60*ragged*(.35+.65*strands);
 float plume=spread*radial*strength*veil;
 float rootHeat=plume*exp(-max(h,0.)*3.8);
 c+=orange*(plume*3.2+rootHeat*.6+plume*pow(strands,4.)*1.5);
