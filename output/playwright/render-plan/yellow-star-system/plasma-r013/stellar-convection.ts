  ...
  override getCustomCode(type: string): Record<string, string> | null {
    ...
    if (type === "fragment")
      return {
        ...
        float convection=.5+.5*sin(flow+st*1.1);
        float boil=smoothstep(.28,.83,convection);
        finalEmissive*=vec3(1.,.62,.22)*(.35+.25*granule+boil*2.3);
        finalEmissive+=vec3(1.,.28,.006)*pow(boil,5.)*.9;
