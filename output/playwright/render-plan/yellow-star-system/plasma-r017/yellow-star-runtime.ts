    ...
    for (const node of [...container.transformNodes, ...container.meshes]) {
      ...
      if (node instanceof Mesh && node.getTotalIndices()) {
        ...
        if (extra.stellarSurface === "hexagonal-convection-v1") {
          ...
          const bounds=node.getBoundingInfo(), padding=new Vector3(.05,.05,.05);
          bounds.reConstruct(bounds.boundingBox.minimum.subtract(padding),bounds.boundingBox.maximum.add(padding));
        }
