...
let frames=0;let start=performance.now();let fixedTime:number|undefined;let runtime:Awaited<ReturnType<typeof createYellowStarRuntime>>|undefined;
const controller=new AbortController();
Object.assign(window,{starReview:{scene,engine,glow,camera,resetTime:()=>{fixedTime=undefined;start=performance.now();},setTime:(time:number)=>{fixedTime=time;}}});
...
engine.runRenderLoop(()=>{runtime?.update(fixedTime ?? (performance.now()-start)/1000);(engine as any)._drawCalls.fetchNewFrame();scene.render();frames++;canvas.dataset.state=JSON.stringify({frames,ready:!!runtime,flareCount:runtime?.flareCount,activeFlares:runtime?.activeFlares,ejectaCount:runtime?.ejectaCount,convectionMaterials:runtime?.convectionMaterials,time:fixedTime ?? (performance.now()-start)/1000,glow:glow.isEnabled,lighting:scene.lightsEnabled,meshes:scene.meshes.length,instances:scene.meshes.filter(m=>!!(m as any).sourceMesh).length,drawCalls:(engine as any)._drawCalls.current});});
createYellowStarRuntime(scene,{bodyId:'yellow-review',radius:1,signal:controller.signal}).then(r=>{runtime=r;r.root.setEnabled(true);document.querySelector('#status')!.textContent=`Native star · rare animated eruptions`;}).catch(e=>{document.querySelector('#status')!.textContent=String(e);});
